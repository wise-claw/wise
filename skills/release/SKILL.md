---
name: release
description: Generic release assistant — analyzes repo release rules, caches them in .wise/RELEASE_RULE.md, then guides the release
level: 3
---

# Release Skill

A thin, repo-aware release assistant. On first run it inspects the project and CI to derive release rules, stores them in `.wise/RELEASE_RULE.md` for future use, then walks you through a release using those rules.

## Usage

```
/wise:release [version]
```
...
Check whether `.wise/RELEASE_RULE.md` exists.
...
Inspect the repo and answer the following. Write answers into `.wise/RELEASE_RULE.md`.
...
### Step 2 — Write `.wise/RELEASE_RULE.md`
...
- `.wise/RELEASE_RULE.md` is a local cache. Commit it to your repo if you want to share the derived rules with your team, or add it to `.gitignore` if you prefer it stays local.
