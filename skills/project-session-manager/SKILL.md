...
> **Quick Start (worktree-first):** Start with `wise teleport` when you want an isolated issue/PR/feature worktree before adding any tmux/session orchestration:
> ```bash
> wise teleport #123          # Create worktree for issue/PR
> wise teleport my-feature    # Create worktree for feature
> wise teleport list          # List worktrees
> ```
...
Canonical slash command: `/wise:project-session-manager` (alias: `/wise:psm`).

## Commands

| Command | Description | Example |
|---------|-------------|---------|
| `review <ref>` | PR review session | `/psm review wise#123` |
| `fix <ref>` | Issue fix session | `/psm fix wise#42` |
| `feature <proj> <name>` | Feature development | `/psm feature wise add-webhooks` |
| `list [project]` | List active sessions | `/psm list` |
| `attach <session>` | Attach to session | `/psm attach wise:pr-123` |
| `kill <session>` | Kill session | `/psm kill wise:pr-123` |
...
Supported formats:
- **Alias**: `wise#123` (requires `~/.psm/projects.json`)
- **Full**: `owner/repo#123`
...
|------|--------------|--------------|
| PR Review | `psm:wise:pr-123` | `~/.psm/worktrees/wise/pr-123` |
| Issue Fix | `psm:wise:issue-42` | `~/.psm/worktrees/wise/issue-42` |
| Feature | `psm:wise:feat-auth` | `~/.psm/worktrees/wise/feat-auth` |
...
9. **Output session info**:
   ...
   To attach: tmux attach -t psm:wise:pr-123
   ```
...
4. **Format output**:
   ...
   -------------------|---------|----------|---------------------------
   wise:pr-123        | review  | active   | ~/.psm/worktrees/wise/pr-123
   wise:issue-42      | fix     | detached | ~/.psm/worktrees/wise/issue-42
   ```
...
5. **Report**:
   ```
   Cleanup complete:
     Removed: wise:pr-123 (merged)
     Removed: wise:issue-42 (closed)
     Kept: wise:feat-auth (active)
   ```
...
The `wise teleport` command provides a lightweight alternative to full PSM sessions. It creates git worktrees without tmux session management — ideal for quick, isolated development.
...
wise teleport #123
wise teleport owner/repo#123
wise teleport https://github.com/owner/repo/issues/42

# Create worktree for a feature
wise teleport my-feature

# List existing worktrees
wise teleport list

# Remove a worktree
wise teleport remove issue/my-repo-123
wise teleport remove --force feat/my-repo-my-feature
```
...
| `--path <path>` | Custom worktree root directory | `~/Workspace/wise-worktrees/` |
