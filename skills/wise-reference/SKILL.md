---
name: wise-reference
description: WISE agent catalog, available tools, team pipeline routing, commit protocol, and skills registry. Auto-loads when delegating to agents, using WISE tools, orchestrating teams, making commits, or invoking skills.
user-invocable: false
---

# WISE Reference

Use this built-in reference when you need detailed WISE catalog information that does not need to live in every `CLAUDE.md` session.
...
- `wise team N:codex|gemini "..."`
- `wise ask <claude|codex|gemini>`
- `/ccg`
...
Invoke built-in workflows via `/wise:<name>`.
...
- `wise-plan` — planning workflow and `/plan`-safe alias
- `ralplan` — consensus planning workflow
- `sciwise` — science/research workflow
...
### Utility skills
- `ask`, `cancel`, `note`, `skillify`, `learner` (deprecated alias), `wise-setup`, `mcp-setup`, `hud`, `wise-doctor`, `trace`, `release`, `project-session-manager`, `skill`, `writer-memory`, `configure-notifications`
...
### Example
```text
feat(docs): reduce always-loaded WISE instruction footprint

Move reference-only orchestration content into a native Claude skill so
session-start guidance stays small while detailed WISE reference remains available.
