...
import { resolveWiseStateRoot } from '../../../scripts/lib/state-root.mjs';
...
export async function resolveSelfImprovePaths({ projectRoot = process.cwd(), topic = '', slug = '', sessionId = '' } = {}) {
  ...
  const wiseRoot = await resolveWiseStateRoot(resolvedProjectRoot);
  const baseRoot = join(wiseRoot, 'self-improve');
  ...
  // Falls back to WISE_SESSION_ID env var when explicit arg is absent.
  // Legacy layout (no topic/slug supplied, flat .wise/self-improve/ exists) is
  // preserved as-is — session scoping only applies to the topic-scoped layout.
  const rawSessionId = sessionId && sessionId.trim()
    ? sessionId.trim()
    : (process.env.WISE_SESSION_ID && process.env.WISE_SESSION_ID.trim() ? process.env.WISE_SESSION_ID.trim() : '');
...
function printHelp() {
  process.stdout.write(
    [
      ...
      '- New runs default to .wise/self-improve/topics/<topic-slug>/',
      '- Pass --session-id to isolate parallel runs: .wise/self-improve/topics/<slug>/sessions/<sid>/',
      '- Legacy flat .wise/self-improve/ is preserved only when no topic/slug is supplied and a flat layout already exists',
      '',
    ].join('\n'),
  );
}
