...
```
/wise:team N:agent-type "task description"
/wise:team "task description"
/wise:team ralph "task description"
```

### Parameters

- **N** - Number of teammate agents (1-20). Optional; defaults to auto-sizing based on task decomposition.
- **agent-type** - WISE agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
...
The lead writes handoffs to `.wise/handoffs/<stage-name>.md`.
...
3. **On team cancellation, handoffs survive** in `.wise/handoffs/` for session resume. They are not deleted by `TeamDelete`.
...
- **Resume:** restart from the last non-terminal stage using staged state + live task status. Read `.wise/handoffs/` to recover stage transition context.
- **Cancel:** `/wise:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.wise/handoffs/` are preserved for potential resume.
...
- Extract **agent-type**, validate it maps to a known WISE subagent
...
Write WISE state using the `state_write` MCP tool for proper session-scoped persistence:
...
On every stage transition, update WISE state:
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean WISE state** -- Remove `.wise/state/team-state.json`
...
- NEVER run team spawning/orchestration skills or commands (for example `$team`, `$ultrawork`, `$autopilot`, `$ralph`, `wise team ...`, `omx team ...`)
...
- `codex_worker`: Emphasize CLI API lifecycle (`wise team api ... --json`) and explicit failure ACKs with stderr.
...
Task decomposition:
#1 [codex_worker] Security review of current auth code -> output to .wise/research/auth-security.md
...
1. Spawn `Task(subagent_type="wise:planner", ...)` with task description + codebase context
...
1. User invokes `/team ralph "task"` or `/wise:team ralph "task"`
...
4. If architect approves: both modes complete, run `/wise:cancel`
...
| ----------------------- | ------------------------------------------------------------------ | -------------------------------------- |
| **Storage**             | JSON files in `~/.claude/teams/` and `~/.claude/tasks/`            | SQLite in `.wise/state/swarm.db`        |
...
The `/wise:cancel` skill handles team cleanup:
...
When `WISE_RUNTIME_V2=1` is set, the team runtime uses an event-driven architecture instead of the legacy done.json polling watchdog:
...
- **Event log**: All team events are appended to `.wise/state/team/{teamName}/events.jsonl`
- **Worker status files**: Workers write status to `.wise/state/team/{teamName}/workers/{name}/status.json`
...
When `WISE_TEAM_SCALING_ENABLED=1` is set, the team supports mid-session scaling:
...
Optional settings live in `.claude/wise.jsonc` (project) or `~/.config/claude-wise/config.jsonc` (user). Project values override user values; `WISE_TEAM_ROLE_OVERRIDES` (env JSON) supersedes both.
...
2. WISE state cleanup via MCP tools:
   ```
   ...
   ```
3. Or run `/wise:cancel` which handles all cleanup automatically.
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.wise/worktrees/{team}/{worker}` with branch `wise-team/{teamName}/{workerName}`.
...
- **Plan id (when applicable):** Team state is session-scoped. Team handoffs at `.wise/handoffs/` are shared by design (see Wave G in the workspace plan).
