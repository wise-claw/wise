...
Run the trace autonomously using the `wise:trace` skill's behavioral contract.
...
Save to `.wise/specs/deep-dive-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".wise/specs/deep-dive-trace-{slug}.md"`
- Keep any ephemeral trace/interview scratch artifacts under `.wise/state/` or `state_write`; do not write temporary files to the repo root or arbitrary working paths.
...
- Save to `.wise/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".wise/specs/deep-dive-{slug}.md"`
...
- **Set up issue/branch/worktree first (Recommended)**
  ...
  - Action: Invoke the known project setup skill or workflow if one is named in guidance; otherwise invoke `Skill("wise:project-session-manager")` with `spec_path` and the pre-flight findings as context. After setup completes, rerun this Phase 5 pre-flight before showing execution options.
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("wise:plan")` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the wise-plan skill's interview phase (the deep-dive interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.wise/plans/`, invoke `Skill("wise:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-dive spec → wise-plan --consensus --direct → autopilot execution`

2. **Execute with autopilot (skip ralplan)**
   - Description: "Full autonomous pipeline — planning, parallel implementation, QA, validation. Faster but without consensus refinement."
   - Action: Invoke `Skill("wise:autopilot")` with the spec file path as context. The spec replaces autopilot's Phase 0 — autopilot starts at Phase 1 (Planning).
...
- Use `Agent(subagent_type="wise:explore", model="haiku")` for brownfield codebase exploration (Phase 1)
...
- Use `Write` tool to save trace result to `.wise/specs/deep-dive-trace-{slug}.md` and final spec to `.wise/specs/deep-dive-{slug}.md`; use `.wise/state/` or `state_write` for ephemeral artifacts
...
[Phase 5] Spec ready. User selects ralplan → autopilot.
  → wise-plan --consensus --direct runs on the spec
...
- [ ] Phase 3 saves trace result to `.wise/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.wise/specs/deep-dive-{slug}.md` in standard deep-interview format
...
- [ ] Phase 5 "Ralplan → Autopilot" option explicitly invokes autopilot after wise-plan consensus completes
...
- [ ] `slug`, `trace_path`, `spec_path` persisted in state for resume resilience; ephemeral artifacts stayed under `.wise/state/` or `state_write`
</Final_Checklist>
...
Deep-dive's output (`.wise/specs/deep-dive-{slug}.md`) feeds into the standard wise pipeline:

```
/deep-dive "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .wise/specs/deep-dive-{slug}.md

  → /wise-plan --consensus --direct (spec as input)
    → Planner/Architect/Critic consensus
    → Plan: .wise/plans/ralplan-*.md
