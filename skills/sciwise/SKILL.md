...
```
/wise:sciwise <goal>                    # Standard research with user checkpoints
/wise:sciwise AUTO: <goal>              # Fully autonomous until complete
/wise:sciwise status                    # Check current research session status
/wise:sciwise resume                    # Resume interrupted research session
/wise:sciwise list                      # List all research sessions
/wise:sciwise report <session-id>       # Generate report for session
```

### Quick Examples

```
/wise:sciwise What are the performance characteristics of different sorting algorithms?
/wise:sciwise AUTO: Analyze authentication patterns in this codebase
/wise:sciwise How does the error handling work across the API layer?
```
...
Task(subagent_type="wise:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="wise:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="wise:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="wise:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/wise:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/wise:sciwise AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="wise:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="wise:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="wise:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="wise:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="wise:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="wise:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="wise:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/wise:sciwise status` | Show current session progress |
| `/wise:sciwise resume` | Resume most recent interrupted session |
| `/wise:sciwise resume <session-id>` | Resume specific session |
| `/wise:sciwise list` | List all sessions with status |
| `/wise:sciwise report <session-id>` | Generate/regenerate report |
| `/wise:sciwise cancel` | Cancel current session (preserves state) |
...
Progress is preserved in `.wise/research/{session-id}/` for resume.
