...
| Command | Action |
|---------|--------|
| `/wise:writer-memory init <project-name>` | Initialize new project memory |
| `/wise:writer-memory status` | Show memory overview (character count, scene count, etc) |
| `/wise:writer-memory char add <name>` | Add new character |
| `/wise:writer-memory char <name>` | View character details |
| `/wise:writer-memory char update <name> <field> <value>` | Update character field |
| `/wise:writer-memory char list` | List all characters |
| `/wise:writer-memory rel add <char1> <char2> <type>` | Add relationship |
| `/wise:writer-memory rel <char1> <char2>` | View relationship |
| `/wise:writer-memory rel update <char1> <char2> <event>` | Add relationship event |
| `/wise:writer-memory scene add <title>` | Add new scene |
| `/wise:writer-memory scene <id>` | View scene details |
| `/wise:writer-memory scene list` | List all scenes |
| `/wise:writer-memory theme add <name>` | Add theme |
| `/wise:writer-memory world set <field> <value>` | Set world attribute |
| `/wise:writer-memory query <question>` | Query memory naturally (Korean supported) |
| `/wise:writer-memory validate <character> <dialogue>` | Check if dialogue matches character tone |
| `/wise:writer-memory synopsis` | Generate emotion-focused synopsis |
| `/wise:writer-memory export` | Export full memory as readable markdown |
| `/wise:writer-memory backup` | Create manual backup |
...
### With WISE Notepad System
Writer Memory integrates with `.wise/notepad.md`:
...
```
Task(subagent_type="wise:architect",
     model="opus",
