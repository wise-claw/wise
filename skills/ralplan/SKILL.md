...
Ralplan is a shorthand alias for `/wise:plan --consensus`. It triggers iterative planning with Planner, Architect, and Critic agents until consensus is reached, with **RALPLAN-DR structured deliberation** (short mode by default, deliberate mode for high-risk work).

## Usage

```
/wise:ralplan "task description"
```
...
```
/wise:ralplan --interactive "task description"
```
...
8. *(--interactive only)* On approval: invoke `Skill("wise:team")` for parallel team execution (recommended) or `Skill("wise:ralph")` for sequential execution -- never implement directly
