...
Meta-skill for managing wise skills via CLI-like commands.
...
**Built-in skill note:** Built-in skills are bundled with wise and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/wise-learned/<name>/SKILL.md`
   - `project` → `.wise/skills/<name>/SKILL.md`
...
✓ Created skill at ~/.claude/skills/wise-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/wise-learned/<name>/SKILL.md`
   - `.wise/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/wise-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.claude/skills/wise-learned/)
...
```
Skill: custom-logger
Scope: user (~/.claude/skills/wise-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/wise-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.claude/skills/wise-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/wise-learned/`
   - Project skills: `.wise/skills/`
...
User: yes
Assistant: ✓ Copied 'error-handler' to .wise/skills/
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/wise-learned"
...
PROJECT_SKILLS_DIR=".wise/skills"
...
echo "=== USER-LEVEL SKILLS (~/.claude/skills/wise-learned/) ==="
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/wise-learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/wise-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/wise-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.wise/skills/) ==="
if [ -d ".wise/skills" ]; then
  PROJECT_COUNT=$(find ".wise/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".wise/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.claude/skills/wise-learned/) - Available across all projects
- **Project-level** (.wise/skills/) - Only for this project
...
- `/wise:skill list` - Show detailed skill inventory
- `/wise:skill add` - Start skill creation (invoke skillify)
- `/wise:skill scan` - Scan both skill directories
...
**Version Control**: Project-level skills (`.wise/skills/`) are intended to be committed with your code so the whole team benefits. In linked worktrees, uncommitted skills remain local to that worktree and disappear if it is removed.
...
- `/wise:skillify` - Extract a skill from current conversation (`/wise:learner` is a deprecated alias)
- `/wise:note` - Save quick notes (less formal than skills)
- `/wise:deepinit` - Generate AGENTS.md codebase hierarchy
...
✓ User skills directory exists: ~/.claude/skills/wise-learned/
✓ Project skills directory exists: .wise/skills/
...
- Run `/wise:skill list` periodically to review your skill library
