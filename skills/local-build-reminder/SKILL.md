...
description: Remind the user to rebuild WISE after editing TypeScript when running from a local fork. Triggered automatically by the AI whenever it notices it (or the user) just changed a src/**/*.ts file in an WISE dev install.
level: 1
---

# Local Build Reminder

**Always-on reminder for WISE fork development.** When WISE is running in local
mode (HUD shows `[WISE#X.Y.ZL]` with an `L` suffix), Claude Code loads compiled
...
4. The user runs an WISE command and expects new behavior tied to a TS edit.
...
- The user is in a Claude Code session that isn't running WISE locally
  (no `L` in the HUD).
...
The HUD's `[WISE#X.Y.ZL]` suffix is the visible cue. Programmatically, the
...
When running inside the WISE fork repo itself, the AI is by definition in
