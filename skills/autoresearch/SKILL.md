...
- You need durable experiment logs under `.wise/autoresearch/`
...
- You want the deprecated `wise autoresearch` CLI flow — it is no longer authoritative
</Do_Not_Use_When>
...
<Required_Artifacts>
Canonical persistent storage lives under `.wise/autoresearch/<mission-slug>/` and/or `.wise/logs/autoresearch/<run-id>/`.
...
```text
.wise/autoresearch/<mission-slug>/
  mission.md
  evaluator.json
  runs/<run-id>/
    evaluations/
...
<Execution_Policy>
- Do not hand execution back to `wise autoresearch`
