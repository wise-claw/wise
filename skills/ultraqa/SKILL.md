...
| ---------------------------------------------- | --------- | -------------------------------- |
| `/wise:ultraqa --tests`            | tests     | All test suites pass             |
| `/wise:ultraqa --build`            | build     | Build succeeds with exit 0       |
| `/wise:ultraqa --lint`             | lint      | No lint errors                   |
| `/wise:ultraqa --typecheck`        | typecheck | No TypeScript errors             |
| `/wise:ultraqa --custom "pattern"` | custom    | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="wise:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ...
   ```
   Task(subagent_type="wise:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ...
   ```
   Task(subagent_type="wise:executor", model="sonnet", prompt="FIX:
...
Track state in `.wise/ultraqa-state.json`:
...
User can cancel with `/wise:cancel` which clears the state file.
...
rm -f .wise/state/ultraqa-state.json
```
