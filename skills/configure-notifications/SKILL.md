...
Set up WISE notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so WISE can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${CLAUDE_CONFIG_DIR:-~/.claude}/.wise-config.json`.
...
2. Send /newbot
3. Choose a name (e.g., "My WISE Notifier")
4. Choose a username (e.g., "my_wise_bot")
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=WISE test notification - Telegram is configured!")
...
Config saved to: ~/.claude/.wise-config.json

You can also set these via environment variables:
  WISE_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  WISE_TELEGRAM_CHAT_ID=123456789

To reconfigure: /wise:configure-notifications telegram
To configure Discord: /wise:configure-notifications discord
To configure Slack: /wise:configure-notifications slack
```
...
```bash
export WISE_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
export WISE_TELEGRAM_CHAT_ID="123456789"
```

Env vars are auto-detected by the notification system without needing `.wise-config.json`.

---

## Discord Setup

Set up Discord notifications so WISE can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **WISE (default)** - Display as "WISE"
...
Build the notifications object with the collected values and merge into `.wise-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}WISE test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  WISE_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  WISE_DISCORD_MENTION=<@1465264645320474637>

To reconfigure: /wise:configure-notifications discord
To configure Telegram: /wise:configure-notifications telegram
To configure Slack: /wise:configure-notifications slack
```
...
**Webhook method:**
```bash
export WISE_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export WISE_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export WISE_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export WISE_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export WISE_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so WISE can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "WISE Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}WISE test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  WISE_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  WISE_SLACK_MENTION=<@U1234567890>

To reconfigure: /wise:configure-notifications slack
To configure Discord: /wise:configure-notifications discord
To configure Telegram: /wise:configure-notifications telegram
```
...
```bash
export WISE_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export WISE_SLACK_MENTION="<@U1234567890>"  # optional
```
...
- `wise --telegram` — Activates Telegram notifications (sets `WISE_TELEGRAM=1`)
- `wise --discord` — Activates Discord notifications (sets `WISE_DISCORD=1`)
- `wise --slack` — Activates Slack notifications (sets `WISE_SLACK=1`)
- `wise --webhook` — Activates webhook notifications (sets `WISE_WEBHOOK=1`)
- `wise --openclaw` — Activates OpenClaw gateway integration (sets `WISE_OPENCLAW=1`)
...
**Examples:**
- `wise --telegram --discord` — Telegram + Discord active
- `wise --telegram --slack --webhook` — Telegram + Slack + Webhook active
- `wise --telegram --openclaw` — Telegram + OpenClaw active
- `wise` — No notifications sent (all platforms require explicit activation)

---

## Hook Event Templates

Customize notification messages per event and per platform using `wise_config.hook.json`.
...
Check if `${CLAUDE_CONFIG_DIR:-~/.claude}/wise_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.claude/wise_config.hook.json
```
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active WISE mode name
...
Read or create `${CLAUDE_CONFIG_DIR:-~/.claude}/wise_config.hook.json` and merge the new settings:
...
- `/wise:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.claude/wise_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/wise_config.openclaw.json"
...
If migrate:
- Read `wise_config.openclaw.json`
- Transform to custom integration format
- Save to `.wise-config.json`
- Backup legacy to `wise_config.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"WISE test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
