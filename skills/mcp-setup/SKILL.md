...
**Options:**
1. **Recommended starter setup** - Fast path for the most common WISE MCP additions
...
- Run /wise:wise-doctor to diagnose issues

MANAGING MCP SERVERS:
- Add more servers: /wise:mcp-setup or `claude mcp add ...`
...
If the custom server is meant to provide organization-specific reference material to WISE workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.
...
Then point WISE at the full tool name in `.claude/wise.jsonc` or `~/.config/claude-wise/config.jsonc`:
