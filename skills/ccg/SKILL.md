...
2. Claude runs via CLI (skill nesting not supported):
   - `wise ask codex "<codex prompt>"`
   - `wise ask gemini "<gemini prompt>"`

3. Artifacts are written under `.wise/artifacts/ask/`
...
```text
.wise/artifacts/ask/codex-*.md
.wise/artifacts/ask/gemini-*.md
```
...
```bash
/wise:ccg <task description>
```

Example:

```bash
/wise:ccg Review this PR - architecture/security via Codex and UX/readability via Gemini
```
