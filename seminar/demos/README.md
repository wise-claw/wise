# WISE Seminar Demo Scripts

This directory contains demo scripts for showcasing Wise's capabilities.

## Overview

The seminar includes 5 progressive demos that showcase different aspects of WISE:
...
### Required Setup
- WISE installed and configured (`/wise:wise-setup` completed)
- HUD statusline installed (`/wise:hud setup`)
...
# Verify WISE is installed
which wise || echo "Run: /wise:wise-setup"
...
- [ ] No active WISE operations running (`/wise:cancel --all`)
- [ ] Clean state files (`rm -rf .wise/state/*`)
...
### Opening (1 min)
"Today I'll show you Wise - a multi-agent orchestration system that transforms Claude from a single assistant into a coordinated team of specialists. Instead of doing everything yourself, you conduct a symphony of AI agents, each optimized for specific tasks."
...
### Closing (1 min)
"WISE transforms how you work with Claude - from manual coding to orchestrating specialized agents. All open source at github.com/Yeachan-Heo/wise."

## Tips for Presenters

### General Tips
- **Announce behaviors**: WISE announces what it's activating ("I'm activating autopilot...")
...
- **State cleanup**: Between demos, verify clean state with `/wise:cancel`
...
**Solution**: Check `.wise/logs/agent-lifecycle.log` for errors, or skip to fallback output

### Issue: HUD not showing
**Solution**: Mention it verbally ("The HUD would show 3 active agents here...")

### Issue: Demo taking too long
**Solution**: Jump to next phase or use `/wise:cancel` and show fallback
