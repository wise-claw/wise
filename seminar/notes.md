...
**The Reveal**
- "That's exactly what wise does. It transforms Claude Code from a single generalist assistant into a coordinated team of 28 specialists."
- "Today, I'm going to show you how this changes everything about AI-assisted development."

### Transition
"Let me start by showing you exactly what WISE is and how it thinks differently about AI assistance."
...
## Section 2: What is WISE? (8 min, Slides 4-8)

### Opening Line
"The fundamental insight behind WISE is simple: your AI should be a conductor, not a performer."
...
- "That's the shift WISE makes. Claude becomes the conductor, and 28 specialized agents are the orchestra."
...
- "AFTER: 'Build authentication for the API.' WISE automatically: explores codebase, analyzes requirements, generates plan, implements in parallel, writes tests, verifies. You're the product owner stating the goal."
...
### Opening Line
"Execution modes are where WISE's power becomes concrete. Each mode is optimized for a specific type of work. Let's explore all five."
...
### Transition
"These modes are powered by WISE's agent system. Let's look at how those 28 specialists are organized."

### Audience Engagement
- "Quick question: Which mode sounds most useful for YOUR daily work?" (Take 2-3 responses)
- "The beauty is you don't have to memorize this. Say 'fast parallel fixes' and WISE activates ultrawork. Say 'efficient batch fixes' and it activates ."
...
- "WISE organizes agents into 13 specializations:"
...
- "How does WISE decide which agent to use? Three factors:"
...
### Opening Line
"Enough talking about it. Let's see WISE in action across five different scenarios."

### Demo 1: Autopilot - Full Feature Build (5 min, Slide 29)

**Setup**
- Have terminal ready with WISE installed
...
- Could be the WISE codebase itself with intentional errors
...
### Opening Line
"Powerful technology is useless if it's hard to use. WISE is designed for zero learning curve."
...
```
[WISE] Mode: ultrawork | Agents: 3 active | executor-1: fixing auth.ts | executor-2: fixing api.ts | architect: reviewing
```
- "At a glance you know:"
- "  Which mode is active"
- "  How many agents are working"
- "  What each agent is doing"
- "Installation: `/wise:hud setup` - adds to your shell prompt"

**Notepad Wisdom** (Slide 36)
- "WISE learns from every session via the notepad system:"
...
- "These persist across sessions. Your WISE gets smarter over time."

**Cost Analytics** (Slide 37)
- "WISE tracks token usage per session:"
- "See exactly how much each mode costs"
- "Compare ultrawork vs  for your workload"
- "Audit logs at `.wise/logs/delegation-audit.jsonl`"
...
### Transition
"That's WISE. Let's recap and open for questions."
...
### Opening Line
"Let's recap what makes WISE transformative."
...
- "WISE transforms Claude Code from a single assistant into a coordinated team."
...
- "If you like it, share it with your team. WISE shines with real-world complexity."
...
**Answer:**
"WISE itself is completely free - it's MIT licensed open-source. What you pay for is the Claude API usage.
...
The key cost benefit: WISE's smart model routing saves you 30-50% on token costs compared to manually using Claude. For example, simple searches use Haiku (super cheap), complex debugging uses Opus (expensive but necessary). Without WISE, everything might default to Opus.
...
**Answer:**
"Currently WISE is designed specifically for Claude Code and leverages Claude's three model tiers - Haiku, Sonnet, and Opus.
...
**Answer:**
"Great question. Without WISE, Claude Code gives you one very smart generalist assistant. You tell it every step: 'search for this, analyze that, now implement this, now test that.'

With WISE, you get 28 specialized agents orchestrated automatically. You state the goal - 'build authentication' - and WISE:
...
Real-world impact: Tasks that took 30 minutes of back-and-forth with Claude Code now take 5 minutes of autonomous execution with WISE."

### 4. "What about security? Is my code safe?"

**Answer:**
"WISE runs entirely locally via Claude Code. Your code never leaves your machine except through the normal Claude API calls that you'd be making anyway.

Additionally, WISE includes a security-reviewer agent that can audit code for common vulnerabilities. You can invoke it explicitly: '/pipeline security' runs a security audit pipeline.

The notepad wisdom system stores data locally in `.wise/notepads/`. Nothing is sent to external servers.
...
Then invoke: `Task(subagent_type="wise:pytest-specialist")`

You can also customize execution modes, delegation categories, and model routing rules via the config file at `~/.claude/.wise-config.json`.
...
**Answer:**
"Yes. WISE works with any language that Claude Code supports - which is basically all mainstream languages.
...
**Answer:**
"Honestly? You don't need to think about it. Just describe what you want in natural language and WISE auto-detects the right mode.
...
But more seriously - WISE has built-in verification at multiple levels:
...
In practice, failures happen - maybe a test fails, maybe there's a linting error. WISE catches these and fixes them before claiming completion.
...
**Answer:**
"WISE is designed for interactive development, not CI/CD automation.
...
That said, some teams use WISE-generated tests in their CI pipeline. The tests themselves are standard - jest, pytest, etc. - they just happened to be generated via WISE.
...
WISE: Literally zero learning time. If you can describe what you want in English, you can use WISE.
...
"Let me show you the mental model shift. BEFORE WISE: [read the before section slowly]. You're the micromanager. AFTER WISE: [read the after section with rising energy]. You're the product owner. This isn't a 10% improvement - this is a complete paradigm shift."
...
3. Feel confident they could install and use WISE today
4. Be excited about the paradigm shift from interactive to autonomous

**Your Success Metric:**
"How many people install WISE in the next week?"
