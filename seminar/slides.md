...
---
# wise: Multi-Agent Orchestration for Claude Code
...
Note: This is a 60-minute seminar covering the complete wise system. We'll focus on practical usage patterns.
...
Note: These are real problems I faced building production applications with Claude Code. WISE was born from frustration with manually orchestrating AI-assisted development.
...
Note: WISE transforms Claude from a single performer into a conductor of an orchestra of specialized AI agents.
...
## Before vs After WISE

| Aspect | Before WISE | After WISE |
|--------|-----------|-----------|
...
/wise:autopilot Add OAuth2 authentication
...
Note: The tier system is central to WISE's cost efficiency. Always start low and escalate only when needed.

---

## Smart Delegation

**WISE automatically picks the right agent:**
...
Note: Composition is where WISE really shines - combine behaviors for exactly the workflow you need.
...
**Real-time visibility into WISE state:**

```
+------------------------------------------------------------+
| WISE | autopilot:exec | 3 agents | 5/12 tasks | ctx:45% | $2.34 |
+------------------------------------------------------------+
      ^               ^          ^            ^         ^
      |               |          |            |         |
...
```bash
git clone https://github.com/Yeachan-Heo/wise.git
cd wise
...
Note: Zero learning curve means you can start using WISE immediately after installation.
...
Note: Matching the right mode to the task type is key to getting the most out of WISE.
...
**GitHub Repository**
```
github.com/Yeachan-Heo/wise
```

**Website & Documentation**
```
yeachan-heo.github.io/wise-website
```
...
| Question | Answer |
|----------|--------|
| Does WISE work with Claude API keys? | Yes, both Max/Pro subscription and API keys work |
| Can I use WISE with other AI models? | No, WISE is specifically for Claude Code |
| How do I stop a runaway autopilot? | Say "stop", "cancel", or `/wise:cancel` |
| Why is my HUD not showing? | Run `/wise:hud setup` |
...
| wise-setup | Setup wizard | `/wise-setup` |
...
|----------|--------------|--------|
| `autopilot:` | `/wise:autopilot` | Full autonomous mode |
| `ralph:` | `/wise:ralph` | Persistence mode |
| `ulw` | `/wise:ultrawork` | Parallel execution |
| `eco:` | `/wise:` | Token-efficient mode |
| `plan` | `/wise:plan` | Planning interview |
