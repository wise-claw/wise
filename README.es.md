...
*No aprendas Claude Code. Solo usa WISE.*

[Comenzar](#inicio-rápido) • [Documentación](https://yeachan-heo.github.io/wise-website) • [Referencia CLI](https://yeachan-heo.github.io/wise-website/docs/#cli-reference) • [Flujos de Trabajo](https://yeachan-heo.github.io/wise-website/docs/#workflows) • [Guía de Migración](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
A partir de **v4.1.7**, **Team** es la superficie canónica de orquestación en WISE. Los puntos de entrada legados como **swarm** y **ultrapilot** siguen siendo compatibles, pero ahora **enrutan a Team internamente**.
...
> Si los equipos están desactivados, WISE te avisará y hará fallback a ejecución sin Team cuando sea posible.

### Trabajadores CLI tmux — Codex & Gemini (v4.4.0+)

**v4.4.0 elimina los servidores MCP de Codex/Gemini** (proveedores `x`, `g`). Usa `/wise-teams` para lanzar procesos CLI reales en paneles divididos de tmux:
...
|-------|---------|----------|
| `/wise-teams N:codex` | N paneles Codex CLI | Revisión de código, análisis de seguridad, arquitectura |
| `/wise-teams N:gemini` | N paneles Gemini CLI | Diseño UI/UX, docs, tareas de gran contexto |
| `/wise-teams N:claude` | N paneles Claude CLI | Tareas generales via Claude CLI en tmux |
...
> **Nota: Nombre del paquete** — El proyecto usa la marca **wise** (repositorio, plugin, comandos), pero el paquete npm se publica como [`wise-claw`](https://www.npmjs.com/package/wise-claw). Si instalas las herramientas CLI via npm/bun, usa `npm install -g wise-claw`.
...
> **Nota:** Si la actualizacion automatica del marketplace no esta activada, debes ejecutar manualmente `/plugin marketplace update wise` para sincronizar la ultima version antes de ejecutar el setup.
...
### Modos de Ejecución
Múltiples estrategias para diferentes casos de uso - desde construcciones completamente autónomas hasta refactorización eficiente en tokens. [Aprende más →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)
...
| **wise-teams** | Trabajadores CLI tmux | Tareas Codex/Gemini CLI; se inician bajo demanda, terminan al completar |
...
Aprende una vez, reutiliza para siempre. WISE extrae conocimiento valioso de depuración en archivos de habilidades portátiles que se inyectan automáticamente cuando son relevantes.

| | Alcance de Proyecto | Alcance de Usuario |
|---|---|---|
| **Ruta** | `.wise/skills/` | `~/.wise/skills/` |
...
| `wise-teams` | Trabajadores CLI tmux (codex/gemini/claude) | `/wise-teams 2:codex "security review"` |
...
```bash
wise wait          # Verificar estado, obtener orientación
wise wait --start  # Habilitar demonio de reanudación automática
wise wait --stop   # Deshabilitar demonio
```
...
**Configuración manual:** crea `~/.claude/wise_config.openclaw.json`:
...
| `WISE_OPENCLAW_DEBUG=1` | Habilitar registro de depuración |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | Ruta alternativa del archivo de configuración |
...
- **[Referencia CLI](https://yeachan-heo.github.io/wise-website/docs/#cli-reference)** - Todos los comandos, flags y herramientas de `wise`
- **[Guía de Notificaciones](https://yeachan-heo.github.io/wise-website/docs/#notifications)** - Configuración de Discord, Telegram, Slack y webhooks
- **[Flujos de Trabajo Recomendados](https://yeachan-heo.github.io/wise-website/docs/#workflows)** - Cadenas de habilidades probadas para tareas comunes
- **[Notas de Versión](https://yeachan-heo.github.io/wise-website/docs/#release-notes)** - Novedades en cada versión
- **[Sitio Web](https://yeachan-heo.github.io/wise-website)** - Guías interactivas y ejemplos
...
WISE puede opcionalmente orquestar proveedores de IA externos para validación cruzada y consistencia de diseño. **No son necesarios** — WISE funciona completamente sin ellos.
...
Si Wise ayuda a tu flujo de trabajo, considera patrocinar:
