...
- Planning is complete only after both `.wise/plans/prd-*.md` and `.wise/plans/test-spec-*.md` exist.
...
4. WISE tier env (`WISE_MODEL_MEDIUM`)
...
- Persist visual verdict JSON in `.wise/state/{scope}/ralph-progress.json` with both numeric (`score`, threshold pass/fail) and qualitative (`reasoning`, `differences`, `suggestions`, `next_actions`) feedback.
...
<state_management>
wise uses the `.wise/` directory for persistent state:
- `.wise/state/` -- Mode state files (JSON)
- `.wise/notepad.md` -- Session-persistent notes
- `.wise/project-memory.json` -- Cross-session project knowledge
- `.wise/plans/` -- Planning documents
...
Run `wise setup` to install all components. Run `wise doctor` to verify installation.
