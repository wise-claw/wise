...
# CLAUDE.md and writes WISE content to a companion file for `wise` launch.
...
CANONICAL_WISE_REFERENCE_SKILL="${ACTIVE_PLUGIN_ROOT}/skills/wise-reference/SKILL.md"

ensure_local_wise_git_exclude() {
  local exclude_path

  if ! exclude_path=$(git rev-parse --git-path info/exclude 2>/dev/null); then
    echo "Skipped WISE git exclude setup (not a git repository)"
    return 0
  fi

  mkdir -p "$(dirname "$exclude_path")"

  local block_start="# BEGIN WISE local artifacts"

  if [ -f "$exclude_path" ] && grep -Fq "$block_start" "$exclude_path"; then
    if grep -Fxq ".omx/" "$exclude_path"; then
      echo "WISE git exclude already configured"
      return 0
    fi
    ...
    printf '.omx/\n' >> "$exclude_path"
    echo "Updated WISE git exclude for local OMX artifacts"
    return 0
  fi
...
EOF
  ...
  echo "Configured git exclude for local WISE/OMX artifacts (preserving .wise/skills/)"
}

# Determine target path
CONFIG_DIR="$(resolve_claude_config_dir)"
if [ "$MODE" = "local" ]; then
  mkdir -p .claude/skills/wise-reference
  TARGET_PATH=".claude/CLAUDE.md"
  SKILL_TARGET_PATH=".claude/skills/wise-reference/SKILL.md"
elif [ "$MODE" = "global" ]; then
  mkdir -p "$CONFIG_DIR/skills/wise-reference"
  TARGET_PATH="$CONFIG_DIR/CLAUDE.md"
  SKILL_TARGET_PATH="$CONFIG_DIR/skills/wise-reference/SKILL.md"
else
...
install_wise_reference_skill() {
  local source_label=""
  local temp_skill
  temp_skill=$(mktemp /tmp/wise-reference-skill-XXXXXX.md)

  if [ -f "$CANONICAL_WISE_REFERENCE_SKILL" ]; then
    cp "$CANONICAL_WISE_REFERENCE_SKILL" "$temp_skill"
    source_label="$CANONICAL_WISE_REFERENCE_SKILL"
  elif [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -f "${CLAUDE_PLUGIN_ROOT}/skills/wise-reference/SKILL.md" ]; then
    cp "${CLAUDE_PLUGIN_ROOT}/skills/wise-reference/SKILL.md" "$temp_skill"
    source_label="${CLAUDE_PLUGIN_ROOT}/skills/wise-reference/SKILL.md"
  else
    rm -f "$temp_skill"
    echo "Skipped wise-reference skill install (canonical skill source unavailable)"
    return 0
  fi

  if [ ! -s "$temp_skill" ]; then
    rm -f "$temp_skill"
    echo "Skipped wise-reference skill install (empty canonical skill source: $source_label)"
    return 0
  fi
  ...
  rm -f "$temp_skill"
  echo "Installed wise-reference skill to $SKILL_TARGET_PATH"
}

# Extract old version before download
OLD_VERSION=$(grep -m1 'WISE:VERSION:' "$TARGET_PATH" 2>/dev/null | sed -E 's/.*WISE:VERSION:([^ ]+).*/\1/' || true)
if [ -z "$OLD_VERSION" ]; then
  OLD_VERSION=$(wise --version 2>/dev/null | head -1 || true)
fi
if [ -z "$OLD_VERSION" ]; then
  OLD_VERSION="none"
fi
...
# Load canonical WISE content to temp file
TEMP_WISE=$(mktemp /tmp/wise-claude-XXXXXX.md)
trap 'rm -f "$TEMP_WISE"' EXIT

WISE_IMPORT_START='<!-- WISE:IMPORT:START -->'
WISE_IMPORT_END='<!-- WISE:IMPORT:END -->'
COMPANION_FILENAME='CLAUDE-wise.md'

write_wrapped_wise_file() {
  local destination="$1"
...
EOF
  ...
  if grep -Fq "$WISE_IMPORT_START" "$target_path"; then
    perl -0pe 's/^<!-- WISE:IMPORT:START -->\R[\s\S]*?^<!-- WISE:IMPORT:END -->(?:\R)?//msg' "$target_path" > "${target_path}.importless"
...
if ! grep -q '<!-- WISE:START -->' "$TEMP_WISE" || ! grep -q '<!-- WISE:END -->' "$TEMP_WISE"; then
  echo "ERROR: Canonical CLAUDE.md source is missing required WISE markers: $SOURCE_LABEL" >&2
...
if grep -q '<!-- WISE:START -->' "$TEMP_WISE"; then
  awk '/<!-- WISE:END -->/{p=0} p; /<!-- WISE:START -->/{p=1}' "$TEMP_WISE" > "${TEMP_WISE}.clean"
  mv "${TEMP_WISE}.clean" "$TEMP_WISE"
fi

if [ ! -f "$TARGET_PATH" ]; then
  # Fresh install: wrap in markers
  write_wrapped_wise_file "$TARGET_PATH"
  rm -f "$TEMP_WISE"
  echo "Installed CLAUDE.md (fresh)"
else
  # Merge: preserve user content outside WISE markers
  if grep -q '<!-- WISE:START -->' "$TARGET_PATH"; then
    # Has markers: remove ALL complete WISE blocks, preserve only real user text
    # Use perl -0 for a global multiline regex replace (portable across GNU/BSD environments)
    perl -0pe 's/^<!-- WISE:START -->\R[\s\S]*?^<!-- WISE:END -->(?:\R)?//msg; s/^<!-- User customizations(?: \([^)]+\))? -->\R?//mg; s/\A(?:[ \t]*\R)+//; s/(?:\R[ \t]*)+\z//;' \
      "$TARGET_PATH" > "${TARGET_PATH}.preserved"

    if grep -Eq '^<!-- WISE:(START|END) -->$' "${TARGET_PATH}.preserved"; then
    ...
    echo "Updated WISE section (user customizations preserved)"
  elif [ "$MODE" = "global" ] && [ "$INSTALL_STYLE" = "preserve" ]; then
    COMPANION_TARGET_PATH="$CONFIG_DIR/$COMPANION_FILENAME"
    ensure_not_symlink_path "$COMPANION_TARGET_PATH" "WISE companion CLAUDE.md"
    ...
    write_wrapped_wise_file "$COMPANION_TARGET_PATH"
    ensure_managed_companion_import "$TARGET_PATH" "$COMPANION_FILENAME"
    VALIDATION_PATH="$COMPANION_TARGET_PATH"
    echo "Installed WISE companion file and preserved existing CLAUDE.md"
  else
    ...
    mv "${TARGET_PATH}.tmp" "$TARGET_PATH"
    echo "Migrated existing CLAUDE.md (added WISE markers, preserved old content)"
  fi
  rm -f "$TEMP_WISE"

  # Clean up orphaned companion file from a prior preserve-mode install.
  # If left behind, prepareWiseLaunchConfigDir reads stale companion content
  # instead of the freshly-updated CLAUDE.md during wise launches.
...
if ! grep -q '<!-- WISE:START -->' "$VALIDATION_PATH" || ! grep -q '<!-- WISE:END -->' "$VALIDATION_PATH"; then
  echo "ERROR: Installed CLAUDE.md is missing required WISE markers: $VALIDATION_PATH" >&2
  exit 1
fi
...
NEW_VERSION=$(grep -m1 'WISE:VERSION:' "$VALIDATION_PATH" 2>/dev/null | sed -E 's/.*WISE:VERSION:([^ ]+).*/\1/' || true)
if [ -z "$NEW_VERSION" ]; then
  NEW_VERSION=$(wise --version 2>/dev/null | head -1 || true)
fi
if [ -z "$NEW_VERSION" ]; then
  NEW_VERSION="unknown"
fi
...
if [ -f "$CONFIG_DIR/settings.json" ] && grep -q "wise" "$CONFIG_DIR/settings.json"; then
  echo "Plugin verified"
else
  echo "Plugin NOT found - run: claude /install-plugin wise"
fi
