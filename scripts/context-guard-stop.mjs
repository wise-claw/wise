 ...
 *
 * Configurable via WISE_CONTEXT_GUARD_THRESHOLD env var (default: 75%).
 *
 * Safety rules:
...
const THRESHOLD = parseInt(process.env.WISE_CONTEXT_GUARD_THRESHOLD || '75', 10);
...
function getGuardFilePath(sessionId) {
  const configDir = getClaudeConfigDir();
  const guardDir = join(configDir, 'projects', '.wise-guards');
  try {
...
function buildStopRecoveryAdvice(contextPercent, blockCount) {
  ...
  return `[WISE ${severity}] Context at ${contextPercent}% (threshold: ${THRESHOLD}%). ` +
    ...
    `(.wise/state, .wise/notepad.md). (Block ${blockCount}/${MAX_BLOCKS})`;
}
