#!/bin/sh
# WISE Node.js Finder (find-node.sh)
#
...
#
# Priority:
#   1. nodeBinary stored in ~/.claude/.wise-config.json (set at setup time)
...
# ---------------------------------------------------------------------------
# 1. Read stored node path from WISE config
# ---------------------------------------------------------------------------
CLAUDE_DIR="$(resolve_claude_config_dir)"
CONFIG_FILE="$CLAUDE_DIR/.wise-config.json"
...
# ---------------------------------------------------------------------------
if [ -z "$NODE_BIN" ]; then
  printf '[WISE] Error: Could not find node binary. Run /wise:wise-setup to fix.\n' >&2
