 ...
 *
 * Called on every leader hook tick. Workers skip (WISE_TEAM_WORKER set).
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'WISE_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'WISE_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
export async function drainPendingTeamDispatch(options: {
  ...
  const stateDir = options.stateDir ?? join(wiseRoot, 'state');
  const logsDir = options.logsDir ?? join(wiseRoot, 'logs');
  const maxPerTick = options.maxPerTick ?? 5;
  const injector = options.injector ?? defaultInjector;

  if (safeString(process.env.WISE_TEAM_WORKER)) {
