/**
 * WISE Orchestrator Hook
 *
 * Enforces orchestrator behavior - delegation over direct implementation.
 * When an orchestrator agent tries to directly modify files outside .wise/,
 * this hook injects reminders to delegate to subagents instead.
 *
 * Adapted from oh-my-opencode's wise-orchestrator hook for shell-based hooks.
 */
 ...
 * Checks: .wise/config.json → [$CLAUDE_CONFIG_DIR|~/.claude]/.wise-config.json → default (warn)
 */
function getEnforcementLevel(directory: string): EnforcementLevel {
  ...
  const localConfig = path.join(getWiseRoot(directory), 'config.json');
  const globalConfig = path.join(getClaudeConfigDir(), '.wise-config.json');
...
/**
 * Create wise orchestrator hook handlers
 */
export function createWiseOrchestratorHook(directory: string) {
  return {
    /**
