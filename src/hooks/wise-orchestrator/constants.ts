/**
 * WISE Orchestrator Constants
 *
 * Message templates and configuration for orchestrator behavior enforcement.
 *
 * Adapted from oh-my-opencode's wise-orchestrator hook.
 */

export const HOOK_NAME = 'wise-orchestrator';

/** @deprecated Use ALLOWED_PATH_PATTERNS instead. Legacy single prefix. */
export const ALLOWED_PATH_PREFIX = '.wise/';
...
You just performed direct file modifications outside \`.wise/\`.
...
You should NOT:
- Write code directly (except for \`.wise/\` files like plans and notepads)
- Make direct file edits outside \`.wise/\`
...
You (coordinator) are attempting to directly modify a file outside \`.wise/\`.
...
- Files inside \`.wise/\` (plans, notepads, drafts)
...
- Creating new files outside \`.wise/\`
...
- Use the notepad at .wise/notepads/{PLAN_NAME}/ to record learnings
