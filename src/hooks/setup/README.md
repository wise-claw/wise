...
### `init`
Initializes WISE directory structure and environment on first run or explicit setup.

**What it does:**
- Creates required directories: `.wise/state/`, `.wise/logs/`, `.wise/notepads/`, `.wise/state/checkpoints/`, `.wise/plans/`
- Validates existing config files (`.wise-config.json`)
- Sets environment variables (`WISE_INITIALIZED=true`) if `CLAUDE_ENV_FILE` is available
...
```json
{
  ...
  "hookSpecificOutput": {
    "hookEventName": "Setup",
    "additionalContext": "WISE initialized:\n- 5 directories created\n- 1 configs validated\n- Environment variables set: WISE_INITIALIZED"
  }
}
```

### `maintenance`
Performs periodic maintenance tasks to keep WISE state clean.
...
// => ['/path/to/project/.wise-config.json']
```

### Environment Variables

#### `setEnvironmentVariables(): string[]`
Sets environment variables for WISE initialization.
