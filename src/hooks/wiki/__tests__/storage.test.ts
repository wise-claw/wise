...
describe('Wiki Storage', () => {
  ...
  describe('getWikiDir', () => {
    it('should return .wise/wiki path', () => {
      const dir = getWikiDir(tempDir);
      expect(dir).toBe(path.join(tempDir, '.wise', 'wiki'));
    });
  });
  ...
  describe('ensureWikiDir', () => {
    ...
    it('should create .gitignore with wiki/ entry', () => {
      ...
      const gitignorePath = path.join(tempDir, '.wise', '.gitignore');
      expect(fs.existsSync(gitignorePath)).toBe(true);
      expect(fs.readFileSync(gitignorePath, 'utf-8')).toContain('wiki/');
    });

    it('should append to existing .gitignore', () => {
      const wiseDir = path.join(tempDir, '.wise');
      fs.mkdirSync(wiseDir, { recursive: true });
      fs.writeFileSync(path.join(wiseDir, '.gitignore'), 'state/\n');

      ensureWikiDir(tempDir);
      const content = fs.readFileSync(path.join(wiseDir, '.gitignore'), 'utf-8');
    ...
    it('should not duplicate wiki/ in .gitignore', () => {
      const wiseDir = path.join(tempDir, '.wise');
      fs.mkdirSync(wiseDir, { recursive: true });
      fs.writeFileSync(path.join(wiseDir, '.gitignore'), 'wiki/\n');
