...
/**
 * Get the path to progress.txt in .wise subdirectory
 */
export function getWiseProgressPath(directory: string): string {
  return join(getWiseRoot(directory), PROGRESS_FILENAME);
}

/**
 * Find progress.txt in a directory (checks both root and .wise)
 */
export function findProgressPath(directory: string): string | null {
  ...
  const wisePath = getWiseProgressPath(directory);
...
export function initProgress(directory: string): boolean {
  ...
  const progressPath = getWiseProgressPath(directory);
...
export function appendProgress(
  ...
  if (!progressPath) {
    ...
    progressPath = getWiseProgressPath(directory);
  }
