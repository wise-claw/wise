...
import { ensureSessionStateDir, getWiseRoot, getSessionStateDir } from '../../lib/worktree-paths.js';
...
/**
 * Get the path to the prd.json in .wise subdirectory
 */
export function getWisePrdPath(directory: string): string {
...
export function getLegacyStatePrdPath(directory: string): string {
  return join(getWiseRoot(directory), 'state', PRD_FILENAME);
}
...
export function findPrdPath(directory: string, sessionId?: string): string | null {
  ...
  const wisePath = getWisePrdPath(directory);
  if (existsSync(wisePath)) {
    return wisePath;
  }
...
export function writePrd(directory: string, prd: PRD, sessionId?: string): boolean {
  ...
  if (sessionId) {
    ...
    // prefer writing to an existing legacy location, or .wise by default.
    prdPath = findPrdPath(directory) ?? getWisePrdPath(directory);
  }
...
5. If ALL stories are done, run \`/wise:cancel\` to cleanly exit ralph mode and clean up all state files
