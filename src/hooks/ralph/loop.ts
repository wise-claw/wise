 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /wise:cancel.
...
export function isUltraQAActive(
  ...
  const stateFile = join(wiseDir, "state", "ultraqa-state.json");
  try {
...
export function createRalphLoopHook(directory: string): RalphLoopHook {
  const startLoop = (
    ...
    if (isUltraQAActive(directory, sessionId)) {
      console.error(
        "Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /wise:cancel.",
      );
      return false;
    }
