/**
 * Tests for issue #891: MCP state tools and stop hook resolve .wise/state/
 * differently when cwd is a subdirectory.
 *
 * processSessionEnd must normalize input.cwd to the git worktree root before
 * building any .wise/ paths, so it always operates on the same directory that
...
describe('processSessionEnd cwd normalization (issue #891)', () => {
  ...
  beforeEach(() => {
    worktreeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wise-891-root-'));
  ...
  it('reads and cleans up state written at worktree root, not subdirectory', async () => {
    ...
    const stateDir = path.join(worktreeRoot, '.wise', 'state');
  ...
  it('writes session summary to worktree root, not subdirectory', async () => {
    ...
    // Session summary should appear under worktreeRoot/.wise/sessions/
    const summaryPath = path.join(worktreeRoot, '.wise', 'sessions', 'test-session-891-summary.json');
    expect(fs.existsSync(summaryPath)).toBe(true);

    // Nothing should have been written under the subdirectory
    expect(fs.existsSync(path.join(subdirectory, '.wise'))).toBe(false);
  });
