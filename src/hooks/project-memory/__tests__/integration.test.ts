...
describe("Project Memory Integration", () => {
  ...
  describe("End-to-end SessionStart flow", () => {
    ...
    it("should persist to centralized state dir without creating local .wise when WISE_STATE_DIR is set", async () => {
      ...
      try {
        ...
        await expect(
          fs.access(path.join(tempDir, ".wise", "project-memory.json")),
        ).rejects.toThrow();
      } finally {
        delete process.env.WISE_STATE_DIR;
