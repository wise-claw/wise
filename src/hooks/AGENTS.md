...
| `wise-orchestrator/` | Orchestrator behavior |
...
- Verify state persistence in `.wise/state/`
...
| ultrapilot | `.wise/state/ultrapilot-state.json` |
| ralph | `.wise/state/ralph-state.json` |
| swarm | `.wise/state/swarm-tasks.db` (SQLite) |
