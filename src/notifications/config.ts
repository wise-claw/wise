 ...
 *
 * Reads notification config from .wise-config.json and provides
...
const CONFIG_FILE = join(getClaudeConfigDir(), ".wise-config.json");
const DEFAULT_TMUX_TAIL_LINES = 15;


/**
 * Read raw config from .wise-config.json
 */
...
export function buildConfigFromEnv(): NotificationConfig | null {
  ...
  const discordMention = validateMention(process.env.WISE_DISCORD_MENTION);

  // Discord Bot (token + channel)
  const discordBotToken = process.env.WISE_DISCORD_NOTIFIER_BOT_TOKEN;
  const discordChannel = process.env.WISE_DISCORD_NOTIFIER_CHANNEL;
  ...
  // Discord Webhook
  const discordWebhook = process.env.WISE_DISCORD_WEBHOOK_URL;
  if (discordWebhook) {
    config.discord = {
      enabled: true,
  ...
  // Telegram (support both WISE_TELEGRAM_BOT_TOKEN and WISE_TELEGRAM_NOTIFIER_BOT_TOKEN)
  const telegramToken =
    process.env.WISE_TELEGRAM_BOT_TOKEN ||
    process.env.WISE_TELEGRAM_NOTIFIER_BOT_TOKEN;
  const telegramChatId =
    process.env.WISE_TELEGRAM_CHAT_ID ||
    process.env.WISE_TELEGRAM_NOTIFIER_CHAT_ID ||
    process.env.WISE_TELEGRAM_NOTIFIER_UID;
  ...
  // Slack Webhook
  const slackWebhook = process.env.WISE_SLACK_WEBHOOK_URL;
  if (slackWebhook) {
    config.slack = {
      enabled: true,
      webhookUrl: slackWebhook,
      mention: validateSlackMention(process.env.WISE_SLACK_MENTION),
    };
    hasAnyPlatform = true;
  }

  // Slack Bot (app token + bot token + channel)
  const slackBotToken = process.env.WISE_SLACK_BOT_TOKEN;
  const slackBotChannel = process.env.WISE_SLACK_BOT_CHANNEL;
  if (slackBotToken && slackBotChannel) {
    config["slack-bot"] = {
      enabled: true,
      appToken: process.env.WISE_SLACK_APP_TOKEN,
...
function applyEnvMerge(config: NotificationConfig): NotificationConfig {
  ...
  const envMention = validateMention(process.env.WISE_DISCORD_MENTION);
  if (envMention) {
  ...
  const envSlackMention = validateSlackMention(process.env.WISE_SLACK_MENTION);
  if (envSlackMention) {
 ...
 *
 * Priority: WISE_NOTIFY_VERBOSITY env var > config.verbosity > "session" default.
...
export function getVerbosity(config: NotificationConfig): VerbosityLevel {
  const envValue = process.env.WISE_NOTIFY_VERBOSITY;
 ...
 *
 * Priority: WISE_NOTIFY_TMUX_TAIL_LINES env var > config.tmuxTailLines > 15 default.
...
export function getTmuxTailLines(config: NotificationConfig): number {
  const envValue = Number.parseInt(process.env.WISE_NOTIFY_TMUX_TAIL_LINES ?? "", 10);
 ...
 *
 * When a profile name is provided (or set via WISE_NOTIFY_PROFILE env var),
 ...
 *
 * Reads from .wise-config.json, looking for the `notifications` key.
 ...
 *
 * @param profileName - Optional profile name (overrides WISE_NOTIFY_PROFILE env var)
 */
export function getNotificationConfig(profileName?: string): NotificationConfig | null {
  const raw = readRawConfig();
  const effectiveProfile = profileName || process.env.WISE_NOTIFY_PROFILE;
  ...
  // Priority 2: Explicit notifications config in .wise-config.json
  if (raw) {
 ...
 *   --telegram  -> WISE_TELEGRAM=1
 *   --discord   -> WISE_DISCORD=1
 *   --slack     -> WISE_SLACK=1
 *   --webhook   -> WISE_WEBHOOK=1
 */
function isPlatformActivated(platform: NotificationPlatform): boolean {
  if (platform === "telegram") return process.env.WISE_TELEGRAM === "1";
  if (platform === "discord" || platform === "discord-bot")
    return process.env.WISE_DISCORD === "1";
  if (platform === "slack" || platform === "slack-bot")
    return process.env.WISE_SLACK === "1";
  if (platform === "webhook") return process.env.WISE_WEBHOOK === "1";
  return false;
}
 ...
 *
 * Reads from .wise-config.json notifications.reply section.
 * Environment variables override config file values:
 * - WISE_REPLY_ENABLED: enable reply listening (default: false)
 * - WISE_REPLY_POLL_INTERVAL_MS: polling interval in ms (default: 3000)
 * - WISE_REPLY_RATE_LIMIT: max messages per minute (default: 10)
 * - WISE_REPLY_DISCORD_USER_IDS: comma-separated authorized Discord user IDs
 * - WISE_REPLY_INCLUDE_PREFIX: include visual prefix (default: true)
 *
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
  ...
  const enabled = process.env.WISE_REPLY_ENABLED === "true" || replyRaw?.enabled === true;
  if (!enabled) return null;

  const authorizedDiscordUserIds = parseDiscordUserIds(
    process.env.WISE_REPLY_DISCORD_USER_IDS,
  ...
  if (hasDiscordBot && authorizedDiscordUserIds.length === 0) {
    console.warn(
      ...
      "Set WISE_REPLY_DISCORD_USER_IDS or add to .wise-config.json notifications.reply.authorizedDiscordUserIds"
    );
  }

  const authorizedSlackUserIds = parseSlackUserIds(
    process.env.WISE_REPLY_SLACK_USER_IDS,
    replyRaw?.authorizedSlackUserIds,
  );

  return {
    enabled: true,
    pollIntervalMs: parseIntSafe(process.env.WISE_REPLY_POLL_INTERVAL_MS) ?? replyRaw?.pollIntervalMs ?? 3000,
    maxMessageLength: replyRaw?.maxMessageLength ?? 500,
    rateLimitPerMinute: parseIntSafe(process.env.WISE_REPLY_RATE_LIMIT) ?? replyRaw?.rateLimitPerMinute ?? 10,
    includePrefix: process.env.WISE_REPLY_INCLUDE_PREFIX !== "false" && (replyRaw?.includePrefix !== false),
...
const LEGACY_OPENCLAW_CONFIG = join(getClaudeConfigDir(), "wise_config.openclaw.json");
...
/**
 * Read custom integrations configuration from .wise-config.json.
 */
