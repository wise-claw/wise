...
export interface DiscordBotNotificationConfig {
  enabled: boolean;
  /** Discord bot token (or env var: WISE_DISCORD_NOTIFIER_BOT_TOKEN) */
  botToken?: string;
  /** Channel ID to send messages to (or env var: WISE_DISCORD_NOTIFIER_CHANNEL) */
  channelId?: string;
  /** Optional mention to prepend to messages (e.g. "<@123456>" for user, "<@&789>" for role) */
  mention?: string;
}
...
/** Top-level notification configuration (stored in .wise-config.json) */
...
export interface NotificationPayload {
  ...
  projectName?: string;
  /** Active WISE modes during this session */
  modesUsed?: string[];
  ...
  agentName?: string;
  /** Agent type for agent-call events (e.g., "wise:executor") */
  agentType?: string;
