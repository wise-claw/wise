 ...
 *
 * Reads wise_config.hook.json for user-customizable message templates.
...
const DEFAULT_CONFIG_PATH = join(getClaudeConfigDir(), "wise_config.hook.json");
 ...
 * - File path overridable via WISE_HOOK_CONFIG env var (for testing)
 */
export function getHookConfig(): HookNotificationConfig | null {
  if (cachedConfig !== undefined) return cachedConfig;

  const configPath = process.env.WISE_HOOK_CONFIG || DEFAULT_CONFIG_PATH;
 ...
 * - hook event `enabled: false` overrides `.wise-config.json` event `enabled: true`
 * - Platform credentials are NOT affected (they stay in .wise-config.json)
 */
