 ...
 *
 * Multi-platform lifecycle notifications for wise.
...
export async function notify(
  ...
  // WISE_NOTIFY=0 suppresses all CCNotifier events (set by `wise --notify false`)
  if (process.env.WISE_NOTIFY === '0') {
    return null;
  }
    ...
    if (
      ...
      try {
        ...
        const rawTail = payload.projectPath
          ? getNewPaneTail(payload.tmuxPaneId, join(getWiseRoot(payload.projectPath), "state"), tailLines)
