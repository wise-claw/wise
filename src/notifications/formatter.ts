...
/** Lines starting with these characters are WISE UI chrome, not output. */
const UI_CHROME_RE = /^[●⎿✻·◼]/;

/** Matches the "ctrl+o to expand" hint injected by WISE. */
const CTRL_O_RE = /ctrl\+o to expand/i;

/** Lines composed entirely of box-drawing characters and whitespace. */
const BOX_DRAWING_RE = /^[\s─═│║┌┐└┘┬┴├┤╔╗╚╝╠╣╦╩╬╟╢╤╧╪━┃┏┓┗┛┣┫┳┻╋┠┨┯┷┿╂]+$/;

/** WISE HUD status lines: [WISE#...] or [WISE] (unversioned). */
const WISE_HUD_RE = /\[WISE[#\]]/;
 ...
 * - Drops lines starting with WISE chrome characters (●, ⎿, ✻, ·, ◼)
...
export function parseTmuxTail(raw: string, maxLines: number = DEFAULT_MAX_TAIL_LINES): string {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (WISE_HUD_RE.test(trimmed)) continue;
