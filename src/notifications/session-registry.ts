 ...
 *
 * Registry location: XDG-aware global WISE state (legacy ~/.wise/state fallback for reads)
...
import { getGlobalWiseStateCandidates, getGlobalWiseStateRoot } from '../utils/paths.js';
 ...
 * WISE_TEST_REGISTRY_DIR overrides the default global state dir so that tests
 * can redirect all I/O to a temporary directory without touching global state.
 */
function getRegistryStateDir(): string {
  return process.env['WISE_TEST_REGISTRY_DIR'] ?? getGlobalWiseStateRoot();
}
...
function getRegistryReadPaths(): string[] {
  if (process.env['WISE_TEST_REGISTRY_DIR']) {
    return [getRegistryPath()];
  }

  return getGlobalWiseStateCandidates('reply-session-registry.jsonl');
}
