...
describe("session-registry", () => {
  beforeEach(() => {
    ...
    // isolated from the real ~/.wise/state and from other parallel test runs.
    testDir = mkdtempSync(join(tmpdir(), "wise-session-registry-test-"));
    process.env["WISE_TEST_REGISTRY_DIR"] = testDir;
    REGISTRY_PATH = join(testDir, "reply-session-registry.jsonl");
    LOCK_PATH = join(testDir, "reply-session-registry.lock");
  });

  afterEach(() => {
    delete process.env["WISE_TEST_REGISTRY_DIR"];
