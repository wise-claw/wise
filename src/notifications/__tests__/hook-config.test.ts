/**
 * Tests for hook notification config reader (wise_config.hook.json).
 *
 * Covers:
 * - File missing → null
 * - File disabled → null
 ...
 * - WISE_HOOK_CONFIG env var override
 */
...
const TEST_DIR = join(tmpdir(), `wise-hook-config-test-${process.pid}`);
const TEST_CONFIG_PATH = join(TEST_DIR, "wise_config.hook.json");
...
describe("hook-config reader", () => {
  beforeEach(() => {
    resetHookConfigCache();
    vi.stubEnv("WISE_HOOK_CONFIG", TEST_CONFIG_PATH);
  });
  ...
  it("returns null when file does not exist", () => {
    vi.stubEnv("WISE_HOOK_CONFIG", join(TEST_DIR, "nonexistent.json"));
  ...
  it("WISE_HOOK_CONFIG env var overrides default path", () => {
    ...
    vi.stubEnv("WISE_HOOK_CONFIG", altPath);
    resetHookConfigCache();
    const config = getHookConfig();
