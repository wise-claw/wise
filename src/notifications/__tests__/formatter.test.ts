...
describe("formatAgentCall", () => {
  ...
  it("should include agent type when provided", () => {
    ...
    expect(result).toContain("**Type:** `wise:executor`");
  });
...
describe("parseTmuxTail noise filters", () => {
  ...
  it("drops WISE HUD versioned status lines", () => {
    expect(
      parseTmuxTail("[WISE#4.4.5] | thinking | session:510m | ctx:61% | 🔧57"),
    ).toBe("");
  });

  it("drops unversioned WISE HUD lines", () => {
    expect(parseTmuxTail("[WISE] | session:5m")).toBe("");
  });
  ...
  it("returns empty string when all lines are noise types", () => {
    const input = [
      "────────────────────────",
      "[WISE#4.4.5] | thinking | session:510m",
