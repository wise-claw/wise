...
describe("reply-listener", () => {
  ...
  describe("Configuration", () => {
    ...
    it("forwards WISE_* env vars to daemon process", () => {
      const fs = require("fs");
      const path = require("path");
      const source = fs.readFileSync(
        path.join(__dirname, "..", "reply-listener.ts"),
      ...
      // Should forward WISE_* env vars for getNotificationConfig()
      expect(source).toContain("WISE_");
      expect(source).toContain("startsWith('WISE_')");
    });
  ...
  describe("Slack user authorization", () => {
    ...
    it("getReplyConfig parses authorizedSlackUserIds from env and config", () => {
      ...
      expect(configSource).toContain("WISE_REPLY_SLACK_USER_IDS");
