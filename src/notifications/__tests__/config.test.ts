...
describe("validateSlackUsername", () => {
  it("accepts simple username", () => {
    expect(validateSlackUsername("WISE Bot")).toBe("WISE Bot");
  });

  it("accepts username with hyphens and underscores", () => {
    expect(validateSlackUsername("wise-notify_bot")).toBe("wise-notify_bot");
  });
...
describe("buildConfigFromEnv", () => {
  ...
  beforeEach(() => {
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "");
    vi.stubEnv("WISE_DISCORD_WEBHOOK_URL", "");
    vi.stubEnv("WISE_DISCORD_MENTION", "");
    vi.stubEnv("WISE_TELEGRAM_BOT_TOKEN", "");
    vi.stubEnv("WISE_TELEGRAM_NOTIFIER_BOT_TOKEN", "");
    vi.stubEnv("WISE_TELEGRAM_CHAT_ID", "");
    vi.stubEnv("WISE_TELEGRAM_NOTIFIER_CHAT_ID", "");
    vi.stubEnv("WISE_TELEGRAM_NOTIFIER_UID", "");
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "");
    vi.stubEnv("WISE_SLACK_MENTION", "");
  });
  ...
  it("builds discord-bot config from env vars", () => {
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "123456");
  ...
  it("includes validated mention in discord-bot config", () => {
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "123456");
    vi.stubEnv("WISE_DISCORD_MENTION", "<@12345678901234567>");
    const config = buildConfigFromEnv();
  ...
  it("rejects invalid mention in env var", () => {
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "123456");
    vi.stubEnv("WISE_DISCORD_MENTION", "@everyone");
    const config = buildConfigFromEnv();
    expect(config!["discord-bot"]!.mention).toBeUndefined();
  });

  it("builds discord webhook config from env var", () => {
    vi.stubEnv("WISE_DISCORD_WEBHOOK_URL", "https://discord.com/api/webhooks/test");
    const config = buildConfigFromEnv();
  ...
  it("builds telegram config from env vars", () => {
    vi.stubEnv("WISE_TELEGRAM_BOT_TOKEN", "123:abc");
    vi.stubEnv("WISE_TELEGRAM_CHAT_ID", "999");
    const config = buildConfigFromEnv();
  ...
  it("builds slack config from env var", () => {
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
    const config = buildConfigFromEnv();
  ...
  it("builds slack config with mention from env var", () => {
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
    vi.stubEnv("WISE_SLACK_MENTION", "<@U1234567890>");
    const config = buildConfigFromEnv();
  ...
  it("trims whitespace from slack mention env var", () => {
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
    vi.stubEnv("WISE_SLACK_MENTION", "  <!channel>  ");
    const config = buildConfigFromEnv();
  ...
  it("rejects invalid slack mention format in env var", () => {
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
    vi.stubEnv("WISE_SLACK_MENTION", "@everyone");
    const config = buildConfigFromEnv();
  ...
  it("uses WISE_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback", () => {
    vi.stubEnv("WISE_TELEGRAM_NOTIFIER_BOT_TOKEN", "123:fallback");
    vi.stubEnv("WISE_TELEGRAM_CHAT_ID", "999");
    const config = buildConfigFromEnv();
    expect(config!.telegram!.botToken).toBe("123:fallback");
  });

  it("uses WISE_TELEGRAM_NOTIFIER_UID as fallback for chat ID", () => {
    vi.stubEnv("WISE_TELEGRAM_BOT_TOKEN", "123:abc");
    vi.stubEnv("WISE_TELEGRAM_NOTIFIER_UID", "uid-999");
    const config = buildConfigFromEnv();
...
describe("getNotificationConfig - deep merge", () => {
  ...
  it("env provides discord-bot when file config has only discord webhook", () => {
    ...
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "env-bot-token");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "env-channel");
  ...
  it("env provides telegram when file config has only discord", () => {
    vi.stubEnv("WISE_TELEGRAM_BOT_TOKEN", "123:tg-token");
    vi.stubEnv("WISE_TELEGRAM_CHAT_ID", "tg-chat");
    const envConfig = buildConfigFromEnv();
  ...
  it("builds config with multiple platforms from env", () => {
    vi.stubEnv("WISE_DISCORD_NOTIFIER_BOT_TOKEN", "bot-token");
    vi.stubEnv("WISE_DISCORD_NOTIFIER_CHANNEL", "channel-123");
    vi.stubEnv("WISE_TELEGRAM_BOT_TOKEN", "456:tg");
    vi.stubEnv("WISE_TELEGRAM_CHAT_ID", "chat-789");
    vi.stubEnv("WISE_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
