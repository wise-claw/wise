...
import { getGlobalWiseStateRoot } from '../utils/paths.js';
import {
  capturePaneContent,
  sendToPane,
  isTmuxAvailable,
 ...
 * WISE_* notification env vars are forwarded so the daemon can call getNotificationConfig().
 */
...
/** Default paths */
const DEFAULT_STATE_DIR = getGlobalWiseStateRoot();
...
function createMinimalDaemonEnv(): NodeJS.ProcessEnv {
  ...
  // Forward WISE_* env vars so the daemon can call getNotificationConfig()
 ...
 * WISE_* env vars are forwarded so the daemon can read both file and env config.
 *
