...
export async function render(
  ...
  if (enabledElements.wiseLabel) {
    const localSuffix = isRuntimePackageLocal() ? "L" : "";
    const versionTag = context.wiseVersion
      ? `#${context.wiseVersion}${localSuffix}`
      : (localSuffix ? `#${localSuffix}` : "");
    if (enabledElements.updateNotification !== false && context.updateAvailable) {
      rendered.set(
        "wiseLabel",
        bold(`[WISE${versionTag}] -> ${context.updateAvailable} wise update`),
      );
    } else {
      rendered.set("wiseLabel", bold(`[WISE${versionTag}]`));
    }
  }
