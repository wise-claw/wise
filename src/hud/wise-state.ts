/**
 * WISE HUD - State Readers
 *
 * Read ralph, ultrawork, and PRD state from existing WISE files.
 ...
 * 1. Session-scoped paths (.wise/state/sessions/{id}/{filename}) - newest first
 * 2. Standard path (.wise/state/{filename})
 * 3. Legacy path (.wise/{filename})
 *
...
function resolveStatePath(directory: string, filename: string, sessionId?: string): string | null {
  const wiseRoot = getWiseRoot(directory);

  if (sessionId) {
    const sessionPath = join(wiseRoot, 'state', 'sessions', sessionId, filename);
  ...
  // Check standard path
  const newPath = join(wiseRoot, 'state', filename);
  ...
  // Check legacy path
  const legacyPath = join(wiseRoot, filename);
 ...
 * Checks only local .wise/state location.
 */
 ...
 * Checks both root prd.json and .wise/prd.json.
 */
export function readPrdStateForHud(directory: string): PrdStateForHud | null {
  ...
  if (!existsSync(prdPath)) {
    // Check .wise
    prdPath = join(getWiseRoot(directory), 'prd.json');
