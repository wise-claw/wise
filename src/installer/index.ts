 ...
 * Installer Module
 *
 * Handles installation of WISE agents, commands, and configuration
...
import { syncWiseLearnedUserSkillsForClaudeCode } from '../utils/user-skill-compat.js';
...
const WISE_MANAGED_SKILL_MARKER = '.wise-managed';
const PLUGIN_FULL_SKILL_BODIES_DIR = 'skill-bodies';
const PLUGIN_COMPACT_SKILL_SHIM_MARKER = '<!-- WISE:COMPACT-PLUGIN-SKILL -->';
...
const WISE_VERSION_MARKER_PATTERN = /<!-- WISE:VERSION:([^\s]+) -->/;
...
/**
 * Detects the newest installed WISE version from persistent metadata or
 * existing CLAUDE.md markers so an older CLI package cannot overwrite a
 * newer installation during `wise setup`.
 */
...
function extractWiseVersionMarker(content: string): string | null {
  const match = content.match(WISE_VERSION_MARKER_PATTERN);
  return match?.[1] ?? null;
}
...
function getNewestInstalledVersionHint(): string | null {
  ...
  for (const candidatePath of claudeCandidates) {
    ...
    try {
      const detectedVersion = extractWiseVersionMarker(readFileSync(candidatePath, 'utf-8'));
...
function buildStatusLineCommand(
  ...
  if (cacheWrapperPath) {
    if (isDefaultClaudeConfigDirPath(CLAUDE_CONFIG_DIR)) {
      return 'sh ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/wise-hud-cache.sh ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/wise-hud.mjs';
    }
  ...
  if (isDefaultClaudeConfigDirPath(CLAUDE_CONFIG_DIR)) {
    if (findNodePath) {
      return 'sh ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/find-node.sh ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/wise-hud.mjs';
    }

    return 'node ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/wise-hud.mjs';
  }
...
export interface InstallOptions {
   ...
   * `<configDir>` because the user is launching WISE via
   * `claude --plugin-dir <path>` (or `wise --plugin-dir <path>`) and the
   * plugin already provides them at runtime. HUD, hooks, CLAUDE.md, and
   * `.wise-config.json` are still installed. Mutually exclusive with
...
/**
 * Read hudEnabled from .wise-config.json without importing auto-update
 * (avoids circular dependency since auto-update imports from installer)
 */
export function isHudEnabledInConfig(): boolean {
  const configPath = join(CLAUDE_CONFIG_DIR, WISE_CONFIG_FILE_REL);
...
/**
 * Detect whether a statusLine config belongs to wise.
 *
 * Checks the command string for known WISE HUD paths so that custom
 * (non-WISE) statusLine configurations are preserved during forced
 * updates/reconciliation.
 *
 * @param statusLine - The statusLine setting object from settings.json
 * @returns true if the statusLine was set by WISE
 */
export function isWiseStatusLine(statusLine: unknown): boolean {
  if (!statusLine) return false;
  // Legacy string format (pre-v4.5): "~/.claude/hud/wise-hud.mjs"
  if (typeof statusLine === 'string') {
    return statusLine.includes('wise-hud');
  }
  // Current object format: { type: "command", command: "node ...wise-hud.mjs" }
  if (typeof statusLine === 'object') {
    const sl = statusLine as Record<string, unknown>;
    if (typeof sl.command === 'string') {
      return sl.command.includes('wise-hud');
    }
  }
  return false;
}
...
/**
 * Known WISE hook script filenames installed into .claude/hooks/.
 * Must be kept in sync with HOOKS_SETTINGS_CONFIG_NODE command entries.
 */
const WISE_HOOK_FILENAMES = new Set([
...
/**
 * Detect whether a hook command belongs to wise.
 *
 ...
 * @returns true if the command belongs to WISE
 */
export function isWiseHook(command: string): boolean {
  ...
  if (containsHooksDir && hookFilenameMatch && WISE_HOOK_FILENAMES.has(hookFilenameMatch[1])) {
    return true;
  }
  return false;
}
...
function configureInstallerSettings(
...

  {
    ...
    const enabledWisePlugin = context.runningAsPlugin || isWisePluginEnabledInSettings(settings);
    const pluginHandlesHooks = context.pluginProvidesHookFiles && enabledWisePlugin;
    ...
    if (shouldConfigureSettingsHooks) {
      ...
      for (const [eventType, newWiseGroups] of Object.entries(desiredHooks)) {
  ...
  if (context.hudScriptPath) {
    ...
    if (!isWindows()) {
      try {
        ...
        const cacheWrapperDest = join(HUD_DIR, 'wise-hud-cache.sh');
    ...
    const needsMigration = typeof settings.statusLine === 'string'
      && isWiseStatusLine(settings.statusLine);
    ...
    } else if (context.options.force && isWiseStatusLine(settings.statusLine)) {
      settings.statusLine = {
        type: 'command',
        command: statusLineCommand
      };
...
function mergeHookGroups(
  ...
  if (options.forceHooks && !options.allowPluginHookRefresh) {
    if (hasNonWiseHook) {
      log(`  Warning: Overwriting non-WISE ${eventType} hook with --force-hooks: ${nonWiseCommand}`);
      result.hookConflicts.push({ eventType, existingCommand: nonWiseCommand });
    }
    log(`  Updated ${eventType} hook (--force-hooks)`);
    return newWiseGroups;
  }

  if (options.force) {
    if (hasNonWiseHook) {
      log(`  Merged ${eventType} hooks (updated WISE hooks, preserved non-WISE hook: ${nonWiseCommand})`);
      result.hookConflicts.push({ eventType, existingCommand: nonWiseCommand });
    } else {
      log(`  Updated ${eventType} hook (--force)`);
    }
    return [...nonWiseGroups, ...newWiseGroups];
  }

  if (hasNonWiseHook) {
    log(`  Warning: ${eventType} hook has non-WISE hook. Skipping. Use --force-hooks to override.`);
    result.hookConflicts.push({ eventType, existingCommand: nonWiseCommand });
  } else {
...
/**
 * Remove stale WISE-created agent files from the config agents directory.
 *
 * When WISE drops an agent definition in a new version, the old .md file
 ...
 *   1. Are .md files (WISE agent naming convention)
 *   2. Were previously shipped by WISE (match the frontmatter `name:` pattern)
 ...
 * known WISE agent) are preserved.
 */
export function cleanupStaleAgents(log: (msg: string) => void): string[] {
  ...
  for (const file of readdirSync(agentsDir)) {
    ...
    // Check if this looks like an WISE-created agent (kebab-case .md with frontmatter)
 ...
 * ~/.claude/agents/ from a prior `wise setup` cause agent definitions to
 * appear twice. Removes standalone copies with WISE frontmatter whose
...
/**
 * Remove stale WISE-created skill directories from the config skills directory.
 *
 * Similar to cleanupStaleAgents but for skill directories. Removes directories
 * that contain a SKILL.md with WISE frontmatter but are no longer shipped by
...
export function cleanupStaleSkills(log: (msg: string) => void): string[] {
  ...
  for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
    ...
    if (entry.name === 'wise-learned') continue;

    const skillDir = join(skillsDir, entry.name);
    const skillMdPath = join(skillDir, 'SKILL.md');
    if (!existsSync(skillMdPath)) continue;
    if (!isWiseManagedSkillDir(skillDir)) continue;
 ...
 * ~/.claude/skills/ from a prior `wise setup` cause every command to appear
...
export function prunePluginDuplicateSkills(log: (msg: string) => void): string[] {
  ...
  for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    if (entry.name === 'wise-learned' || entry.name === '.wise-trash') continue;
    ...
    try {
      ...
      // plugin's copy, OR the directory is explicitly marked as WISE-owned via the
      // .wise-managed marker file. Frontmatter structure alone is not a reliable
      // ownership signal — user skills routinely use the same ---/name: format.
      const pluginContent = pluginSkillHashes.get(entry.name);
      const skillDir = join(skillsDir, entry.name);

      if (pluginContent === standaloneContent || isWiseManagedSkillDir(skillDir)) {
        rmSync(skillDir, { recursive: true, force: true });
        removed.push(entry.name);
...
export function getInstalledWisePluginRoots(): string[] {
  ...
  try {
    ...
    for (const [pluginId, entries] of Object.entries(plugins)) {
      if (!pluginId.toLowerCase().includes('wise') || !Array.isArray(entries)) {
        continue;
      }
...
function getKnownMarketplaceInstallRoots(): string[] {
  ...
  try {
    ...
    for (const [marketplaceId, entry] of Object.entries(raw)) {
      const isWiseMarketplace = marketplaceId.toLowerCase().includes('wise')
        || marketplaceId.toLowerCase().includes('wise');
      if (!isWiseMarketplace) {
        continue;
      }
...
function renderCompactPluginSkillShim(skillDirName: string, content: string): string {
  ...
  const rawDescription = getFrontmatterStringValue(parsed.metadata, 'short_description')
    ...
    ?? `Invoke the ${skillDirName} WISE skill.`;
...
export function syncInstalledPluginPayload(): {
  ...
  const targetRoots = getInstalledWisePluginRoots()
  ...
  if (!sourceRoot) {
    return {
      synced: false,
      errors: [
        'Unable to find a complete WISE package source to repair installed plugin roots',
...
/**
 * Detect whether an installed Claude Code plugin already provides WISE agent
 * markdown files, so the legacy ~/.claude/agents copy can be skipped.
 */
export function hasPluginProvidedAgentFiles(): boolean {
  return getInstalledWisePluginRoots().some(pluginRoot =>
...
export function hasEnabledWisePlugin(): boolean {
...
function isWisePluginEnabledInSettings(settings: Record<string, unknown>): boolean {
  for (const candidate of [settings.enabledPlugins, settings.plugins]) {
    if (Array.isArray(candidate)) {
      if (candidate.some(plugin =>
        typeof plugin === 'string' && plugin.toLowerCase().includes('wise')
...
function getManagedSkillMarkerPath(skillDir: string): string {
  return join(skillDir, WISE_MANAGED_SKILL_MARKER);
}

function markSkillAsWiseManaged(skillDir: string): void {
  writeFileSync(getManagedSkillMarkerPath(skillDir), 'wise-managed\n');
}

function isWiseManagedSkillDir(skillDir: string): boolean {
...
function syncBundledSkillDefinitions(log: (msg: string) => void, options?: { safeStandaloneNames?: boolean }): string[] {
  ...
  for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
    ...
    markSkillAsWiseManaged(targetDir);
...
function syncUserSkillCompatShims(log: (msg: string) => void): string[] {
  const synced = syncWiseLearnedUserSkillsForClaudeCode();
...
/**
 * Extract the embedded WISE version from a CLAUDE.md file.
 *
 * Primary source of truth is the injected `<!-- WISE:VERSION:x.y.z -->` marker.
 * Falls back to legacy headings that may include a version string inline.
 */
export function extractWiseVersionFromClaudeMd(content: string): string | null {
  const versionMarkerMatch = content.match(/<!--\s*WISE:VERSION:([^\s]+)\s*-->/i);
  ...
  const headingMatch = content.match(/^#\s+wise.*?\b(v?\d+\.\d+\.\d+(?:[-+][^\s]+)?)\b/m);
  if (headingMatch?.[1]) {
...
/**
 * Keep persisted setup metadata in sync with the installed WISE runtime version.
 *
...
export function syncPersistedSetupVersion(options?: {
  ...
  const configPath = options?.configPath ?? join(CLAUDE_CONFIG_DIR, WISE_CONFIG_FILE_REL);
  ...
  if (!detectedVersion) {
    ...
    if (existsSync(claudeMdPath)) {
      detectedVersion = extractWiseVersionFromClaudeMd(readFileSync(claudeMdPath, 'utf-8')) ?? undefined;
    }
  }
...
/**
 * Merge WISE content into existing CLAUDE.md using markers
 * @param existingContent - Existing CLAUDE.md content (null if file doesn't exist)
 * @param wiseContent - New WISE content to inject
 * @returns Merged content with markers
 */
export function mergeClaudeMd(existingContent: string | null, wiseContent: string, version?: string): string {
  const START_MARKER = '<!-- WISE:START -->';
  const END_MARKER = '<!-- WISE:END -->';
  const USER_CUSTOMIZATIONS = '<!-- User customizations -->';
  const WISE_BLOCK_PATTERN = new RegExp(
  ...
  // Idempotency guard: strip markers from wiseContent if already present
  // This handles the case where docs/CLAUDE.md ships with markers
  let cleanWiseContent = wiseContent;
  const wiseStartIdx = findLineAnchoredMarker(wiseContent, START_MARKER);
  const wiseEndIdx = findLineAnchoredMarker(wiseContent, END_MARKER, true);
  if (wiseStartIdx !== -1 && wiseEndIdx !== -1 && wiseStartIdx < wiseEndIdx) {
    // Extract content between markers, trimming any surrounding whitespace
    cleanWiseContent = wiseContent
      .substring(wiseStartIdx + START_MARKER.length, wiseEndIdx)
      .trim();
  }

  // Strip any existing version marker from content and inject current version
  cleanWiseContent = cleanWiseContent.replace(/<!-- WISE:VERSION:[^\s]*? -->\n?/, '');
  const versionMarker = version ? `<!-- WISE:VERSION:${version} -->\n` : '';

  // Case 1: No existing content - wrap wiseContent in markers
  if (!existingContent) {
    return `${START_MARKER}\n${versionMarker}${cleanWiseContent}\n${END_MARKER}\n`;
  }

  const strippedExistingContent = existingContent.replace(WISE_BLOCK_PATTERN, '');
  ...
  if (hasResidualStartMarker || hasResidualEndMarker) {
    ...
    // Strip unmatched WISE markers from recovered content to prevent unbounded
  ...
  // Case 3: Preserve only user-authored content that lives outside WISE markers
  return `${START_MARKER}\n${versionMarker}${cleanWiseContent}\n${END_MARKER}\n\n${USER_CUSTOMIZATIONS}\n${preservedUserContent}`;
}

/**
 * Install WISE agents, commands, skills, and hooks
 */
export function install(options: InstallOptions = {}): InstallResult {
  ...
  if (isComparableVersion(targetVersion)
    ...
    const message = `Skipping install: installed WISE ${installedVersionHint} is newer than CLI package ${targetVersion}. Run "wise update" to update the CLI package, then rerun "wise setup".`;
    log(message);
    result.success = true;
  ...
  if (pluginPayloadSync.errors.length > 0) {
    ...
    if (pluginPayloadSync.targetRoots.length > 0) {
      ...
      result.message = 'Installation failed: WISE plugin cache is incomplete and could not be repaired';
      return result;
    }
  }
  if (pluginPayloadSync.synced) {
    ...
    log(`Repaired installed WISE plugin payload from ${sourceSummary} -> ${targetSummary}`);
  }
  ...
  const enabledWisePlugin = hasEnabledWisePlugin();
  // Dev plugin-dir mode: user launched WISE via `claude --plugin-dir <path>` or
  // `wise --plugin-dir <path>`. The plugin already exposes agents/skills at runtime,
  // so skip copying them into <configDir>. Auto-detected via WISE_PLUGIN_ROOT in CLI.
  ...
  const shouldInstallBundledSkills =
    !pluginDirMode && (options.noPlugin === true || !enabledWisePlugin || !pluginProvidesSkillFiles);
  ...
  if (runningAsPlugin) {
    ...
    log('Detected installed WISE plugin agent definitions - skipping legacy ~/.claude/agents sync');
  }
  ...
  try {
    ...
    if (!runningAsPlugin) {
      ...
      // Clean up stale WISE-created agents from previous versions
      ...
      if (!(pluginProvidesHookFiles && enabledWisePlugin)) {
    ...
    if (shouldInstallBundledSkills) {
      log(options.noPlugin
        ...
        : !enabledWisePlugin
          ? 'Installing bundled skills from local package (no enabled WISE plugin detected)...'
      ...
      result.installedSkills.push(...syncBundledSkillDefinitions(log, {
        safeStandaloneNames: !enabledWisePlugin || options.noPlugin === true,
      }));
    ...
    // Clean up stale WISE-created skills from previous versions
    ...
    // This runs regardless of plugin context so that `wise update` (which re-execs
    // as `update-reconcile` with CLAUDE_PLUGIN_ROOT still set) always keeps the
    // version marker and WISE instructions in ~/.claude/CLAUDE.md up to date.
    ...
    if (!projectScoped) {
      ...
      const wiseContent = loadClaudeMdContent();
      ...
      // Merge WISE content with existing content
      const mergedContent = mergeClaudeMd(existingContent, wiseContent, targetVersion);
    ...
    if (projectScoped) {
      ...
      log('Skipping HUD statusline (hudEnabled is false in .wise-config.json)');
    } else {
    ...
    if (!skipHud) try {
      ...
      hudScriptPath = join(HUD_DIR, 'wise-hud.mjs').replace(/\\/g, '/');
    ...
    if (!projectScoped) {
      ...
      try {
        ...
        writeFileSync(
          join(wiseRoot, 'template-version.json'),
...
/**
 * Check if WISE is already installed
 */
