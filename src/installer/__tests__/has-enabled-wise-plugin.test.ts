/**
 * Regression tests for `hasEnabledWisePlugin()` settings format detection.
 *
 ...
 * As a result, `wise update`/`wise setup` invoked from a regular shell (where
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), 'wise-has-enabled-'));
...
describe('hasEnabledWisePlugin', () => {
  describe('CLAUDE_PLUGIN_ROOT short-circuit (highest priority)', () => {
    it('returns true when CLAUDE_PLUGIN_ROOT is set, even with no settings.json', async () => {
      ...
      const { hasEnabledWisePlugin } = await freshInstaller();
      expect(hasEnabledWisePlugin()).toBe(true);
    });
  });

  describe('Modern Claude Code 1.x format (`enabledPlugins`)', () => {
    it('returns true when enabledPlugins.wise@wise is true', async () => {
      writeSettings({
        enabledPlugins: {
          'wise@wise': true,
    ...
    it('returns true for any pluginId substring matching wise', async () => {
      writeSettings({
        enabledPlugins: {
          'wise-fork@somerepo': true,
        },
      });
      const { hasEnabledWisePlugin } = await freshInstaller();
      expect(hasEnabledWisePlugin()).toBe(true);
    ...
    it('returns false when enabledPlugins.wise@wise is explicitly false', async () => {
      writeSettings({
        enabledPlugins: {
          'wise@wise': false,
        },
    ...
    it('returns false when enabledPlugins has no wise entry', async () => {
      writeSettings({
        enabledPlugins: {
          'unrelated-plugin@foo': true,
        },
    ...
    it('handles enabledPlugins as an array of plugin id strings', async () => {
      writeSettings({
        enabledPlugins: ['wise@wise', 'other'],
      });
      const { hasEnabledWisePlugin } = await freshInstaller();
      expect(hasEnabledWisePlugin()).toBe(true);
    });
  ...
  describe('Legacy `plugins` field (backward compatibility)', () => {
    it('returns true when plugins.wise@wise is true', async () => {
      writeSettings({
        plugins: {
          'wise@wise': true,
        },
    ...
    it('returns true when plugins is an array with wise entry', async () => {
      writeSettings({
        plugins: ['wise'],
      });
      const { hasEnabledWisePlugin } = await freshInstaller();
  ...
  describe('Mixed format support', () => {
    ...
    it('returns false when neither field has an enabled WISE entry', async () => {
      writeSettings({
        enabledPlugins: { 'unrelated@foo': true },
