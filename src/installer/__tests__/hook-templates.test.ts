...
describe('keyword-detector packaged artifacts', () => {
  ...
  it('emits compact skill invocation guidance instead of full SKILL.md bodies', () => {
    ...
    for (const scriptPath of [templatePath, pluginPath]) {
      ...
      expect(context).toContain('Preferred invocation: /wise:ralph');
  ...
  it('marks packaged keyword-triggered states as awaiting confirmation', () => {
    ...
    try {
      for (const [scriptPath, statePath] of [
        [templatePath, join(tempDir, '.wise', 'state', 'sessions', 'hook-session', 'ralph-state.json')],
        [pluginPath, join(tempDir, '.wise', 'state', 'sessions', 'hook-session', 'ralph-state.json')],
      ] as const) {
        ...
        rmSync(join(tempDir, '.wise'), { recursive: true, force: true });
        rmSync(join(fakeHome, '.wise'), { recursive: true, force: true });
      }
    } finally {
  ...
  it('does not auto-trigger informational keyword questions in packaged artifacts', () => {
    ...
    for (const prompt of [
      ...
      `🦌 DeerFlow vs ⚡ WISE Ultrawork - 완전 비교!
...
WISE Ultrawork = "특수부대 작전 반"
...
결론: "순식간에 많은 작업" → WISE Ultrawork ⚡
이런대화가 한번이라면 몇번할수있을까 오픈라우터 20달러 결제기준 api로`,
      'The article said "WISE Ultrawork", but why is the answer the same?',
      'WISE Ultrawork = "special ops". how much would it cost?',
    ]) {
