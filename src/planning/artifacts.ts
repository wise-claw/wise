...
function getPlansDirCandidates(cwd: string): string[] {
  return [join(getWiseRoot(cwd), "plans"), join(cwd, ".omx", "plans")];
}
...
function launchHintPattern(mode: "team" | "ralph"): RegExp {
  return mode === "team"
    ? /(?<command>(?:(?:wise|omx)\s+team|\$team)(?:\s+ralph)?(?:\s+(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?)?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')(?<flags>(?:\s+--[\w-]+)*))/gi
    : /(?<command>(?:(?:wise|omx)\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')(?<flags>(?:\s+--[\w-]+)*))/gi;
}
