...
function buildCurrentProjectTargets(projectRoot: string): SearchTarget[] {
  ...
  const wiseRoot = getWiseRoot(projectRoot);
  const sessionSummariesDir = join(wiseRoot, 'sessions');
  for (const filePath of listJsonlFiles(sessionSummariesDir)) {
    targets.push({ filePath, sourceType: 'wise-session-summary' });
  }

  const replayDir = join(wiseRoot, 'state');
  if (existsSync(replayDir)) {
    for (const filePath of listJsonlFiles(replayDir)) {
      if (filePath.includes('agent-replay-') && filePath.endsWith('.jsonl')) {
        targets.push({ filePath, sourceType: 'wise-session-replay' });
      }
    }
  }
...
function buildJsonArtifactEntry(entry: Record<string, unknown>, sourceType: SearchTarget['sourceType']): SearchableEntry | null {
  ...
  const entryType = sourceType === 'wise-session-summary' ? 'session-summary' : 'session-replay';
...
function buildSearchableEntry(entry: Record<string, unknown>, sourceType: SearchTarget['sourceType']): SearchableEntry | null {
  if (sourceType === 'project-transcript' || sourceType === 'legacy-transcript' || sourceType === 'wise-session-replay') {
    return buildTranscriptEntry(entry) ?? (sourceType === 'wise-session-replay' ? buildJsonArtifactEntry(entry, sourceType) : null);
  }

  if (sourceType === 'wise-session-summary') {
...
async function collectMatchesFromFile(
  ...
  if (target.sourceType === 'wise-session-summary' && target.filePath.endsWith('.json')) {
    try {
