...
export function getInteropMode(env: NodeJS.ProcessEnv = process.env): InteropMode {
  const raw = (env.OMX_WISE_INTEROP_MODE || 'off').toLowerCase();
...
export function canUseOmxDirectWriteBridge(env: NodeJS.ProcessEnv = process.env): boolean {
  const interopEnabled = env.OMX_WISE_INTEROP_ENABLED === '1';
  const toolsEnabled = env.WISE_INTEROP_TOOLS_ENABLED === '1';
...
function getInteropSource(target: 'wise' | 'omx'): 'wise' | 'omx' {
  return target === 'wise' ? 'omx' : 'wise';
}
...
export const interopSendTaskTool: ToolDefinition<{
  ...
  description: 'Send a task to the other tool (WISE -> OMX or OMX -> WISE) for execution. The task will be queued in shared state for the target tool to pick up.',
  schema: {
    target: z.enum(['wise', 'omx']).describe('Target tool to send the task to'),
...
export const interopReadResultsTool: ToolDefinition<{
  source: z.ZodOptional<z.ZodEnum<['wise', 'omx']>>;
  ...
  schema: {
    source: z.enum(['wise', 'omx']).optional().describe('Filter by source tool'),
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const tasks = readSharedTasks(cwd, {
        source: source as 'wise' | 'omx' | undefined,
...
export const interopSendMessageTool: ToolDefinition<{
  ...
  schema: {
    target: z.enum(['wise', 'omx']).describe('Target tool to send the message to'),
...
export const interopSendOmxMessageTool: ToolDefinition<{
  ...
  schema: {
    ...
    fromWorker: z.string().describe('Sender worker name (e.g., "wise-bridge")'),
