...
const EXTRA_STATE_ONLY_MODES = ['ralplan', 'wise-teams', 'skill-active'] as const;
...
function pruneMissionBoardTeams(root: string, teamNames?: string[]): number {
  const missionStatePath = join(getWiseRoot(root), 'state', 'mission-state.json');
...
function cleanupTeamRuntimeState(root: string, teamNames?: string[]): number {
  const teamStateRoot = join(getWiseRoot(root), 'state', 'team');
...
function getLegacyStateFileCandidates(mode: StateToolMode, root: string): string[] {
  ...
  const candidates = [
    ...
    join(getWiseRoot(root), `${normalizedName}.json`),
  ];

  return [...new Set(candidates)];
}

function getWorkingDirectoryLocalWiseRoot(root: string): string {
  return join(root, WisePaths.ROOT);
}

function shouldCheckWorkingDirectoryLocalState(root: string): boolean {
  return getWorkingDirectoryLocalWiseRoot(root) !== getWiseRoot(root);
}

function getWorkingDirectoryLocalSessionStatePath(mode: StateToolMode, root: string, sessionId: string): string {
  const normalizedName = mode.endsWith('-state') ? mode : `${mode}-state`;
  return join(getWorkingDirectoryLocalWiseRoot(root), 'state', 'sessions', sessionId, `${normalizedName}.json`);
}

function getWorkingDirectoryLocalLegacyStateFileCandidates(mode: StateToolMode, root: string): string[] {
  const normalizedName = mode.endsWith('-state') ? mode : `${mode}-state`;
  return [
    join(getWorkingDirectoryLocalWiseRoot(root), 'state', `${normalizedName}.json`),
    join(getWorkingDirectoryLocalWiseRoot(root), `${normalizedName}.json`),
  ];
}
...
function clearModeRuntimeArtifacts(
  ...
  let cleared = 0;
  let hadFailure = false;
  const stateRoot = join(getWiseRoot(root), 'state');
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // Mirrors the session_id path at line ~403. (patch: fix missing cancel signal)
      {
        ...
        const legacySignalPath = join(getWiseRoot(root), 'state', 'cancel-signal-state.json');
...
export const stateListActiveTool: ToolDefinition<{
  ...
  description: 'List all currently active modes. By default, scopes to the current session (WISE_SESSION_ID). Pass all:true to list active modes across all sessions.',
  annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  schema: {
    workingDirectory: z.string().optional().describe('Working directory (defaults to cwd)'),
    session_id: z.string().optional().describe('Explicit session ID to scope the listing. Overrides WISE_SESSION_ID when provided.'),
