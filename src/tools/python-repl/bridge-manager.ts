...
function getBridgeScriptPath(): string {
  // Check for WISE_BRIDGE_SCRIPT environment variable first (set by MCP server context)
  if (process.env.WISE_BRIDGE_SCRIPT) {
    const override = path.resolve(process.env.WISE_BRIDGE_SCRIPT);
    const overrideBasename = path.basename(override);
    if (overrideBasename !== 'gyoshu_bridge.py') {
      throw new Error(`WISE_BRIDGE_SCRIPT must point to gyoshu_bridge.py, got: ${overrideBasename}`);
    }
    if (!fs.existsSync(override)) {
      throw new Error(`WISE_BRIDGE_SCRIPT file not found: ${override}`);
    }
    return override;
  }
...
export async function spawnBridgeServer(
  ...
  const proc: ChildProcess = spawn(pythonEnv.pythonPath, bridgeArgs, {
    ...
    env: {
      ...process.env,
      PYTHONUNBUFFERED: '1',
      WISE_PARENT_PID: String(process.pid),
      ...(isPythonSandboxEnabled() ? { WISE_PYTHON_SANDBOX: '1' } : {}),
    },
    detached: true,
  });
