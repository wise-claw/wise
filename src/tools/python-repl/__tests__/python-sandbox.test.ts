...
describe('python-repl sandbox env propagation', () => {
  ...
  it('sandbox enabled with WISE_SECURITY=strict', () => {
...
function executeBridgeCode(code: string, sandboxEnv = false): { success: boolean; stdout: string; error?: { type: string; message: string } } {
  ...
  const tmpScript = join(tmpdir(), `wise-bridge-exec-test-${process.pid}-${Date.now()}.py`);
  const script = [
    'import importlib.util, json, os',
    sandboxEnv ? 'os.environ["WISE_PYTHON_SANDBOX"] = "1"' : 'os.environ.pop("WISE_PYTHON_SANDBOX", None)',
...
describe('gyoshu bridge execution builtins hardening', () => {
  ...
  it('uses the same locked-down execution namespace when WISE_PYTHON_SANDBOX=1', () => {
