...
describe('wiki tools workingDirectory', () => {
  ...
  let originalWiseStateDir: string | undefined;

  beforeEach(() => {
    originalCwd = process.cwd();
    originalWiseStateDir = process.env.WISE_STATE_DIR;
  ...
  afterEach(() => {
    ...
    clearWorktreeCache();
    if (originalWiseStateDir === undefined) {
      delete process.env.WISE_STATE_DIR;
    } else {
      process.env.WISE_STATE_DIR = originalWiseStateDir;
    }
  ...
  it('writes and reads wiki pages in the provided linked worktree instead of process cwd', async () => {
    ...
    expect(existsSync(join(linked, '.wise', 'wiki', 'linked-worktree-page.md'))).toBe(true);
    expect(existsSync(join(primary, '.wise', 'wiki'))).toBe(false);
