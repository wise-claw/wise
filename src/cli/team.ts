...
Usage:
  wise team api <operation> --input '<json>' [--json] [--cwd DIR]
...
function resolveJobsDir(env: NodeJS.ProcessEnv = process.env): string {
  return env.WISE_JOBS_DIR || getGlobalWiseStatePath('team-jobs');
}

function resolveRuntimeCliPath(env: NodeJS.ProcessEnv = process.env): string {
  if (env.WISE_RUNTIME_CLI_PATH) {
    return env.WISE_RUNTIME_CLI_PATH;
  }
...
function teamStateRoot(cwd: string, teamName: string): string {
  return join(getWiseRoot(cwd), 'state', 'team', teamName);
}
...
export function generateJobId(now = Date.now()): string {
  return `wise-${now.toString(36)}${randomUUID().slice(0, 8)}`;
}
...
function autoTeamName(task: string): string {
  const slug = task
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 24) || 'task';
  return `wise-${slug}-${Date.now().toString(36).slice(-4)}`;
}
  ...
  if (!canonicalOperation || !SUPPORTED_API_OPERATIONS.has(canonicalOperation as SupportedApiOperation)) {
    return {
      ...
      error: {
        ...
        message: `Unsupported wise team api operation: ${operation}`,
      },
    };
  }
...
Usage:
  wise team start --agent <claude|codex|gemini|cursor|grok>[,<agent>...] --task "<task>" [--count N] [--name TEAM] [--cwd DIR] [--new-window] [--auto-merge] [--json]
  wise team status <job_id|team_name> [--json] [--cwd DIR]
  wise team wait <job_id> [--timeout-ms MS] [--json]
  wise team cleanup <job_id> [--grace-ms MS] [--json]
  wise team resume <team_name> [--json] [--cwd DIR]
  wise team shutdown <team_name> [--force] [--json] [--cwd DIR]
  wise team api <operation> [--input '<json>'] [--json] [--cwd DIR]
  wise team [ralph] <N:agent-type[:role]> "task" [--json] [--cwd DIR] [--new-window]

Worktrees:
  Native per-worker git worktree mode is opt-in/config-gated with team.ops.worktreeMode or WISE_TEAM_WORKTREE_MODE=detached|named.
...
Examples:
  wise team start --agent codex --count 2 --task "review auth flow" --new-window
  wise team status wise-abc123
  wise team status auth-review
  wise team resume auth-review
  wise team shutdown auth-review --force
  wise team api list-tasks --input '{"teamName":"auth-review"}' --json
  wise team 3:codex "refactor launch command"
...
function parseStartArgs(args: string[]): StartArgsParsed {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "wise team start": ${token}`);
  }
...
function parseCommonJobArgs(args: string[], command: 'status' | 'wait' | 'cleanup'): {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "wise team ${command}": ${token}`);
  }

  if (!target) {
    throw new Error(`Missing required target for "wise team ${command}".`);
  }
...
function parseTeamTargetArgs(args: string[], command: 'resume' | 'shutdown'): {
  ...
  if (!teamName) {
    throw new Error(`Missing required <team_name> for "wise team ${command}".`);
  }
...
function parseApiArgs(args: string[]): {
  ...
  if (!operation) {
    throw new Error(`Missing required <operation> for "wise team api"\n\n${TEAM_API_USAGE}`);
  }
...
function parseLegacyStartAlias(args: string[]): TeamLegacyStartArgs | null {
  ...
  if (shortFollowup) {
    ...
    const command = `wise team ${ralph ? 'ralph ' : ''}${spec} ${JSON.stringify(task)}`;
    const approvedHintOutcome = readApprovedExecutionLaunchHintOutcome(cwd, 'team', {
      task,
      command,
    });
