/**
 * Native tmux shell launch for wise
...
/**
 * Extract the WISE-specific --notify flag from launch args.
 * --notify false  → disable notifications (WISE_NOTIFY=0)
...
/**
 * Extract the WISE-specific --openclaw flag from launch args.
 * Purely presence-based (like --madmax/--yolo):
 *   --openclaw        -> enable OpenClaw (WISE_OPENCLAW=1)
...
/**
 * Extract the WISE-specific --telegram flag from launch args.
 * Purely presence-based:
 *   --telegram        -> enable Telegram notifications (WISE_TELEGRAM=1)
...
/**
 * Extract the WISE-specific --discord flag from launch args.
 * Purely presence-based:
 *   --discord        -> enable Discord notifications (WISE_DISCORD=1)
 *   --discord=true   -> enable
...
/**
 * Extract the WISE-specific --slack flag from launch args.
 * Purely presence-based:
 *   --slack        -> enable Slack notifications (WISE_SLACK=1)
 *   --slack=true   -> enable
 *   --slack=false  -> disable
 *   --slack=1      -> enable
 *   --slack=0      -> disable
...
/**
 * Extract the WISE-specific --webhook flag from launch args.
 * Purely presence-based:
 *   --webhook        -> enable Webhook notifications (WISE_WEBHOOK=1)
 *   --webhook=true   -> enable
 ...
 * normalizeClaudeLaunchArgs strips them so we can apply WISE-specific
...
function runClaudeInsideTmux(cwd: string, args: string[]): void {
  ...
  try {
    ...
    // Propagate Claude's exit code so wise does not swallow failures
...
export async function launchCommand(args: string[]): Promise<void> {
  ...
  // child of Claude Code) can resolve the active plugin root via WISE_PLUGIN_ROOT.
  ...
  // Extract WISE-specific --notify flag before passing remaining args to Claude CLI
  const { notifyEnabled, remainingArgs } = extractNotifyFlag(args);
  if (!notifyEnabled) {
    process.env.WISE_NOTIFY = '0';
  }

  // Extract WISE-specific --openclaw flag (presence-based, no value consumption)
  ...
  // Extract WISE-specific --telegram flag (presence-based)
  ...
  // Extract WISE-specific --discord flag (presence-based)
  ...
  // Extract WISE-specific --slack flag (presence-based)
  ...
  // Extract WISE-specific --webhook flag (presence-based)
