 ...
 * - No WISE HUD pane spawning in tmux launch paths
 */
...
import { runClaude, launchCommand, extractNotifyFlag, extractOpenClawFlag, extractTelegramFlag, extractDiscordFlag, extractSlackFlag, extractWebhookFlag, normalizeClaudeLaunchArgs, isPrintMode, prepareWiseLaunchConfigDir, buildEnvExportPrefix, hasMadmaxFlag, TMUX_ENV_FORWARD } from '../launch.js';
import {
  resolveLaunchPolicy,
...
// ---------------------------------------------------------------------------
// runClaude — WISE HUD pane spawning disabled
// ---------------------------------------------------------------------------
describe('runClaude WISE HUD behavior', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    (execFileSync as ReturnType<typeof vi.fn>).mockReturnValue(Buffer.from(''));
  });

  it('does not build an wise hud --watch command inside tmux', () => {
    (resolveLaunchPolicy as ReturnType<typeof vi.fn>).mockReturnValue('inside-tmux');

    runClaude('/tmp/cwd', [], 'test-session');

    const calls = vi.mocked(buildTmuxShellCommand).mock.calls;
    const wiseHudCall = calls.find(
      ([cmd, args]) => cmd === 'node' && Array.isArray(args) && args.includes('hud'),
    );
    expect(wiseHudCall).toBeUndefined();
  });
...
describe('launchCommand — env var propagation', () => {
  ...
  const envKeys = ['WISE_NOTIFY', 'WISE_OPENCLAW', 'WISE_TELEGRAM', 'WISE_DISCORD', 'WISE_SLACK', 'WISE_WEBHOOK', 'CLAUDECODE'] as const;
  ...
  it('bare --telegram sets WISE_TELEGRAM to 1', async () => {
    await launchCommand(['--telegram']);
    expect(process.env.WISE_TELEGRAM).toBe('1');
  });

  it('bare --discord sets WISE_DISCORD to 1', async () => {
    await launchCommand(['--discord']);
    expect(process.env.WISE_DISCORD).toBe('1');
  });

  it('bare --slack sets WISE_SLACK to 1', async () => {
    await launchCommand(['--slack']);
    expect(process.env.WISE_SLACK).toBe('1');
  });

  it('bare --webhook sets WISE_WEBHOOK to 1', async () => {
    await launchCommand(['--webhook']);
    expect(process.env.WISE_WEBHOOK).toBe('1');
  });

  it('bare --openclaw sets WISE_OPENCLAW to 1', async () => {
    await launchCommand(['--openclaw']);
    expect(process.env.WISE_OPENCLAW).toBe('1');
  });

  it('--telegram=false overrides inherited WISE_TELEGRAM=1', async () => {
    process.env.WISE_TELEGRAM = '1';
    await launchCommand(['--telegram=false']);
    expect(process.env.WISE_TELEGRAM).toBe('0');
  });

  it('--discord=false overrides inherited WISE_DISCORD=1', async () => {
    process.env.WISE_DISCORD = '1';
    await launchCommand(['--discord=false']);
    expect(process.env.WISE_DISCORD).toBe('0');
  });

  it('--slack=false overrides inherited WISE_SLACK=1', async () => {
    process.env.WISE_SLACK = '1';
    await launchCommand(['--slack=false']);
    expect(process.env.WISE_SLACK).toBe('0');
  });

  it('--webhook=false overrides inherited WISE_WEBHOOK=1', async () => {
    process.env.WISE_WEBHOOK = '1';
    await launchCommand(['--webhook=false']);
    expect(process.env.WISE_WEBHOOK).toBe('0');
  });

  it('--openclaw=false overrides inherited WISE_OPENCLAW=1', async () => {
    process.env.WISE_OPENCLAW = '1';
    await launchCommand(['--openclaw=false']);
    expect(process.env.WISE_OPENCLAW).toBe('0');
  });

  it('--telegram=0 overrides inherited WISE_TELEGRAM=1', async () => {
    process.env.WISE_TELEGRAM = '1';
  ...
  it('WISE flags are stripped from args passed to Claude', async () => {
...
describe('prepareWiseLaunchConfigDir / launchCommand WISE companion loading', () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = mkdtempSync(join(tmpdir(), 'wise-launch-profile-'));
  ...
  it('uses a runtime launch profile when a preserved CLAUDE-wise.md companion exists', async () => {
    const configDir = join(tempRoot!, '.claude');
    mkdirSync(join(configDir, 'skills'), { recursive: true });
    writeFileSync(join(configDir, 'CLAUDE.md'), '# User base config\n');
    writeFileSync(join(configDir, 'CLAUDE-wise.md'), '<!-- WISE:START -->\n# WISE companion\n<!-- WISE:END -->\n');
    ...
    const runtimeDir = join(configDir, '.wise-launch');
    expect(process.env.CLAUDE_CONFIG_DIR).toBe(runtimeDir);
    expect(existsSync(join(runtimeDir, 'CLAUDE.md'))).toBe(true);
    expect(readFileSync(join(runtimeDir, 'CLAUDE.md'), 'utf-8')).toContain('# WISE companion');
  ...
  it('repairs retired team MCP entries in the runtime settings copy', () => {
    ...
    const runtimeDir = prepareWiseLaunchConfigDir(configDir);
  ...
  it('does not copy credential content when credential symlink creation fails', async () => {
    ...
    try {
      const { prepareWiseLaunchConfigDir: prepareWithFailedSymlink } = await import('../launch.js');
      const configDir = join(tempRoot!, '.claude');
      mkdirSync(configDir, { recursive: true });
      const credentialsPath = join(configDir, '.credentials.json');
      writeFileSync(join(configDir, 'CLAUDE-wise.md'), '<!-- WISE:START -->\n# WISE\n<!-- WISE:END -->\n');
  ...
  it('preserves runtime .claude.json across runtime config dir rebuilds', () => {
    ...
    const rebuiltRuntimeDir = prepareWiseLaunchConfigDir(configDir);
  ...
  it('preserves runtime .claude.json when source .claude.json is absent, invalid, or has no mcpServers', () => {
    ...
    prepareWiseLaunchConfigDir(configDir);
  ...
  it('leaves CLAUDE_CONFIG_DIR unchanged when no preserved companion exists', () => {
    const configDir = join(tempRoot!, '.claude');
    mkdirSync(configDir, { recursive: true });
    writeFileSync(join(configDir, 'CLAUDE.md'), '<!-- WISE:START -->\n# WISE base\n<!-- WISE:END -->\n');

    expect(prepareWiseLaunchConfigDir(configDir)).toBe(configDir);
    expect(existsSync(join(configDir, '.wise-launch'))).toBe(false);
  });
...
describe('TMUX_ENV_FORWARD allowlist', () => {
  ...
  it('includes all WISE launch flags', () => {
    for (const name of ['WISE_NOTIFY', 'WISE_OPENCLAW', 'WISE_TELEGRAM', 'WISE_DISCORD', 'WISE_SLACK', 'WISE_WEBHOOK']) {
...
describe('runClaude outside-tmux — env forwarding', () => {
  ...
  it('does not inject exports when no forwarded vars are set', () => {
    ...
    delete process.env.WISE_TELEGRAM;
    delete process.env.WISE_DISCORD;
    delete process.env.WISE_SLACK;
    delete process.env.WISE_WEBHOOK;
