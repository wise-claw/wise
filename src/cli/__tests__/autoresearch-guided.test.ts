...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'wise-autoresearch-guided-test-'));
...
describe('runAutoresearchNoviceBridge', () => {
  it('loops through refine further before launching and writes draft + mission files', async () => {
    ...
    try {
      ...
      const draftContent = await readFile(join(repo, '.wise', 'specs', 'deep-interview-autoresearch-ux-eval.md'), 'utf-8');
      const resultContent = await readFile(join(repo, '.wise', 'specs', 'autoresearch-ux-eval', 'result.json'), 'utf-8');
      const missionContent = await readFile(join(result.missionDir, 'mission.md'), 'utf-8');
      const sandboxContent = await readFile(join(result.missionDir, 'sandbox.md'), 'utf-8');
  ...
  it('uses explicit cwd, login-shell wrapping, and verifies startup before logging success', () => {
    ...
    tmuxExecMock.mockImplementation((args: string[]) => {
      ...
      if (args[0] === 'new-session') {
        expect(args.slice(0, 6)).toEqual(['new-session', '-d', '-s', 'wise-autoresearch-demo', '-c', '/repo']);
        expect(args[6]).toBe('wrapped:' + `${process.execPath} ${process.cwd()}/bin/wise.js autoresearch /repo/missions/demo`);
        return '';
      }
    ...
    expect(buildTmuxShellCommandMock).toHaveBeenCalledWith(process.execPath, [expect.stringMatching(/bin\/wise\.js$/), 'autoresearch', '/repo/missions/demo']);
    expect(wrapWithLoginShellMock).toHaveBeenCalledWith(`${process.execPath} ${process.cwd()}/bin/wise.js autoresearch /repo/missions/demo`);
    expect(logSpy).toHaveBeenCalledWith('\nAutoresearch launched in background tmux session.');
    expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-t', 'wise-autoresearch-demo', 'set-clipboard', 'on'], { stripTmux: true, stdio: 'ignore' });
    expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-at', 'wise-autoresearch-demo', 'terminal-features', ',*:clipboard'], { stripTmux: true, stdio: 'ignore' });
    expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t wise-autoresearch-demo');
  });
});
  ...
  it('launches a detached claude setup session and seeds deep-interview autoresearch mode', async () => {
    ...
    try {
      ...
      tmuxExecMock.mockImplementation((args: string[]) => {
        if (args[0] === 'new-session') {
          expect(args.slice(0, 9)).toEqual([
            'new-session', '-d', '-P', '-F', '#{pane_id}', '-s', 'wise-autoresearch-setup-kf12oi', '-c', repo,
          ]);
          expect(typeof args[9]).toBe('string');
          expect(String(args[9])).toContain('wrapped:CODEX_HOME=');
          expect(String(args[9])).toContain(`CODEX_HOME=${repo}/.omx/tmp/wise-autoresearch-setup-kf12oi/codex-home`);
        ...
        if (args[0] === 'has-session') {
          hasSessionCalls += 1;
          expect(args).toEqual(['has-session', '-t', 'wise-autoresearch-setup-kf12oi']);
          return '';
        }
        if (args[0] === 'send-keys') {
          return '';
      ...
      expect(buildTmuxShellCommandWithEnvMock).toHaveBeenCalledWith(
        ...
        { CODEX_HOME: `${repo}/.omx/tmp/wise-autoresearch-setup-kf12oi/codex-home` },
      );
      expect(wrapWithLoginShellMock).toHaveBeenCalledWith(`CODEX_HOME=${repo}/.omx/tmp/wise-autoresearch-setup-kf12oi/codex-home claude --dangerously-skip-permissions`);
      ...
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-t', 'wise-autoresearch-setup-kf12oi', 'set-clipboard', 'on'], { stripTmux: true, stdio: 'ignore' });
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-at', 'wise-autoresearch-setup-kf12oi', 'terminal-features', ',*:clipboard'], { stripTmux: true, stdio: 'ignore' });
      expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t wise-autoresearch-setup-kf12oi');
  ...
  it('builds native Windows-friendly setup commands without env-prefix argv hacks', async () => {
    ...
    try {
      ...
      expect(wrapWithLoginShellMock).toHaveBeenCalledWith(
        `CODEX_HOME=${repo}/.omx/tmp/wise-autoresearch-setup-kf12oi/codex-home claude --dangerously-skip-permissions`,
      );
    } finally {
