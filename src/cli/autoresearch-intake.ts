...
export const AUTORESEARCH_DEEP_INTERVIEW_RESULT_KIND = 'wise.autoresearch.deep-interview/v1';
...
function buildArtifactDir(repoRoot: string, slug: string): string {
  return join(getWiseRoot(repoRoot), 'specs', `${AUTORESEARCH_ARTIFACT_DIR_PREFIX}${slug}`);
}

function buildDraftArtifactPath(repoRoot: string, slug: string): string {
  return join(getWiseRoot(repoRoot), 'specs', `${DEEP_INTERVIEW_DRAFT_PREFIX}${slug}.md`);
}
...
export async function writeAutoresearchDraftArtifact(input: {
  ...
  const specsDir = join(getWiseRoot(input.repoRoot), 'specs');
...
async function listMarkdownDraftPaths(repoRoot: string): Promise<string[]> {
  const specsDir = join(getWiseRoot(repoRoot), 'specs');
