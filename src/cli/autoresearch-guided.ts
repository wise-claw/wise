...
export async function runAutoresearchNoviceBridge(
  ...
  try {
    while (true) {
      ...
      const evaluatorIntent = await promptWithDefault(io, '\nHow should WISE judge success? Describe it in plain language', topic);
...
export async function guidedAutoresearchSetupInference(
  ...
  try {
    ...
    const explicitEvaluator = await askQuestion(
      rl,
      '\nOptional evaluator command (leave blank and WISE will infer one if confidence is high)\n> ',
    );
...
export function spawnAutoresearchTmux(missionDir: string, slug: string): void {
  ...
  const sessionName = `wise-autoresearch-${slug}`;
  ...
  const wisePath = resolve(join(__dirname, '..', '..', 'bin', 'wise.js'));
  const command = buildTmuxShellCommand(process.execPath, [wisePath, 'autoresearch', missionDir]);
...
export function spawnAutoresearchSetupTmux(repoRoot: string): void {
  ...
  const sessionName = `wise-autoresearch-setup-${Date.now().toString(36)}`;
