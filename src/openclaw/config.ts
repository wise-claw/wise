 ...
 *
 * Reads OpenClaw config from ~/.claude/wise_config.openclaw.json.
 * Config is cached after first read (env vars don't change during process lifetime).
 * Config file path can be overridden via WISE_OPENCLAW_CONFIG env var.
 */
...
const CONFIG_FILE = process.env.WISE_OPENCLAW_CONFIG
  || join(getClaudeConfigDir(), "wise_config.openclaw.json");
 ...
 *
 * Returns null when:
 * - WISE_OPENCLAW env var is not "1"
...
export function getOpenClawConfig(): OpenClawConfig | null {
  ...
  if (process.env.WISE_OPENCLAW !== "1") {
    return null;
  }
