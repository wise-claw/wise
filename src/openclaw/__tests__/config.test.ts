...
describe("getOpenClawConfig", () => {
  ...
  it("returns null when WISE_OPENCLAW is not set", () => {
    vi.stubEnv("WISE_OPENCLAW", "");
    expect(getOpenClawConfig()).toBeNull();
  });

  it("returns null when WISE_OPENCLAW is not '1'", () => {
  ...
  it("returns valid config when file exists and WISE_OPENCLAW=1", () => {
    vi.stubEnv("WISE_OPENCLAW", "1");
  ...
  it("respects WISE_OPENCLAW_CONFIG env var for custom config path", () => {
    vi.stubEnv("WISE_OPENCLAW", "1");
    vi.stubEnv("WISE_OPENCLAW_CONFIG", "/custom/path/config.json");
