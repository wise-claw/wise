...
TypeScript source code for wise - the core library that powers multi-agent orchestration.
...
| File | Description |
|------|-------------|
| `index.ts` | Main entry point - exports `createWiseSession()` |
...
2. **Entry Point Pattern**:
   ...
   export { createWiseSession } from './session';
   export { lspTools, astTools, allCustomTools } from './tools';
   export { getAgentDefinitions, wiseSystemPrompt } from './agents/definitions';
   ```
