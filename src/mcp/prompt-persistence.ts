 ...
 *
 * Writes assembled prompts and model responses to .wise/prompts/ before/after
...
const PROMPT_PERSISTENCE_PRODUCER = { system: 'wise', component: 'prompt-persistence' } as const;
...
export function getPromptsDir(workingDirectory?: string): string {
  const root = getWorktreeRoot(workingDirectory) || workingDirectory || process.cwd();
  return join(getWiseRoot(root), 'prompts');
}
