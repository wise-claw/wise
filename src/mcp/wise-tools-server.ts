/**
 * WISE Tools Server - In-process MCP server for custom tools
 *
...
/**
 * Map from user-facing WISE_DISABLE_TOOLS group names to ToolCategory values.
...
/**
 * Parse WISE_DISABLE_TOOLS env var value into a Set of disabled ToolCategory values.
 *
 * Accepts a comma-separated list of group names (case-insensitive).
 * Unknown names are silently ignored.
 *
 * @param envValue - The env var value to parse. Defaults to process.env.WISE_DISABLE_TOOLS.
 * @returns Set of ToolCategory values that should be disabled.
 *
 * @example
 * // WISE_DISABLE_TOOLS=lsp,python-repl,project-memory
...
export function parseDisabledGroups(envValue?: string): Set<ToolCategory> {
  ...
  const value = envValue ?? process.env.WISE_DISABLE_TOOLS;
...
const interopToolsEnabled = process.env.WISE_INTEROP_TOOLS_ENABLED === '1';
...
// Read WISE_DISABLE_TOOLS once at startup and filter tools accordingly
...
/**
 * In-process MCP server exposing all WISE custom tools
 *
 * Tools will be available as mcp__t__<tool_name>.
 * Tools in disabled groups (via WISE_DISABLE_TOOLS) are excluded at startup.
 */
export const wiseToolsServer = createSdkMcpServer({
  name: "t",
  version: "1.0.0",
  tools: sdkTools
});
 ...
 * Only includes tools that are enabled (not disabled via WISE_DISABLE_TOOLS).
 */
export const wiseToolNames = enabledTools.map(t => `mcp__t__${t.name}`);

// Build a map from MCP tool name to category for efficient lookup
// Built from allTools so getWiseToolNames() category filtering works correctly
 ...
 */
export function getWiseToolNames(options?: ToolNameFilterOptions): string[] {
  return filterToolNames(wiseToolNames, toolCategoryMap, options);
}
