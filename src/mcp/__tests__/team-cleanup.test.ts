...
describe('killWorkerPanes', () => {
  ...
  it('writes shutdown sentinel before force-killing', async () => {
    const cwd = join(tmpdir(), `wise-cleanup-test-${process.pid}`);
    const stateDir = join(cwd, '.wise', 'state', 'team', 'myteam');
  ...
  it('does not throw when sentinel directory does not exist (non-fatal)', async () => {
    await expect(
      killWorkerPanes({
        ...
        cwd: '/tmp/does-not-exist-wise-test',
        graceMs: 0,
      })
...
describe('killTeamSession', () => {
  ...
  it('calls kill-session for session-mode sessions (no ":" in name)', async () => {
    await killTeamSession('wise-team-myteam-worker1');
    expect(killedSessions).toContain('wise-team-myteam-worker1');
  });
});

// ─── validateJobId regex ──────────────────────────────────────────────────────

// Re-test the regex rule from team-server.ts (spec: /^wise-[a-z0-9]{1,16}$/)
const JOB_ID_RE = /^wise-[a-z0-9]{1,16}$/;

describe('validateJobId regex (/^wise-[a-z0-9]{1,16}$/)', () => {
  it('accepts valid job IDs', () => {
    expect(JOB_ID_RE.test('wise-abc123')).toBe(true);
    expect(JOB_ID_RE.test('wise-a')).toBe(true);
    expect(JOB_ID_RE.test('wise-mlytzz5w')).toBe(true);
  });

  it('rejects path traversal attempts', () => {
    expect(JOB_ID_RE.test('wise-../../etc/passwd')).toBe(false);
    expect(JOB_ID_RE.test('../wise-abc')).toBe(false);
    expect(JOB_ID_RE.test('wise-abc/../../x')).toBe(false);
  });

  it('rejects IDs without the wise- prefix', () => {
  ...
  it('rejects IDs longer than 16 chars after prefix', () => {
    expect(JOB_ID_RE.test('wise-' + 'a'.repeat(17))).toBe(false);
  });

  it('rejects empty suffix', () => {
    expect(JOB_ID_RE.test('wise-')).toBe(false);
  });
});

describe('team start validation wiring', () => {
  it('validates teamName at wise_run_team_start API boundary', () => {
  ...
  it('contains timeoutSeconds deprecation guard in wise_run_team_start', () => {
    const source = readFileSync(join(__dirname, '..', 'team-server.ts'), 'utf-8');
...
describe('wise_run_team_start timeoutSeconds rejection', () => {
  ...
  it('error message includes migration guidance (wise_run_team_wait + wise_run_team_cleanup)', () => {
    expect(() => handleStartGuard({
      teamName: 'test',
      agentTypes: ['claude'],
      tasks: [],
      cwd: '/tmp',
      timeoutSeconds: 30,
    })).toThrow('wise_run_team_wait timeout_ms');
  });
