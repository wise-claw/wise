...
import { getWiseRoot } from './worktree-paths.js';
...
function getNamespaceDir(namespace: string, worktreeRoot?: string): string {
  ...
  return join(wiseRoot, SHARED_MEMORY_DIR, namespace);
}
...
export function cleanupExpired(
  ...
  const wiseRoot = getWiseRoot(worktreeRoot);
  const sharedMemDir = join(wiseRoot, SHARED_MEMORY_DIR);
