...
describe('renderSkills', () => {
  ...
  describe('namespaced skill names', () => {
    it('displays only last segment for namespaced skills (wise:plan)', () => {
      const lastSkill: SkillInvocation = { name: 'wise:plan', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain('skill:plan');
    ...
    it('displays only last segment for namespaced skills with args', () => {
      const lastSkill: SkillInvocation = { name: 'wise:autopilot', args: 'build app', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
