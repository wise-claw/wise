...
describe("readHudConfig", () => {
  ...
  describe("priority order", () => {
    ...
    it("reads from settings.json wiseHud key first", () => {
    ...
    it("falls back to legacy hud-config.json when settings.json has no wiseHud", () => {
      mockExistsSync.mockImplementation((path) => {
        const s = String(path);
        return (
          /[\\/]Users[\\/]testuser[\\/]\.claude[\\/]settings\.json$/.test(s) ||
          /[\\/]Users[\\/]testuser[\\/]\.claude[\\/]\.wise[\\/]hud-config\.json$/.test(
            s,
          )
        );
      });
  ...
  describe("merging with defaults", () => {
    ...
    it("merges partial config with defaults", () => {
      ...
      expect(config.elements.wiseLabel).toBe(
        DEFAULT_HUD_CONFIG.elements.wiseLabel,
      );
...
describe("writeHudConfig", () => {
  ...
  it("preserves unrelated settings.json keys while writing wiseHud", () => {
    mockExistsSync.mockImplementation((path) =>
    ...
    expect(written.wiseHud.elements.gitRepo).toBe(true);
  });

  it("merges legacy hud-config defaults into the written wiseHud payload", () => {
    mockExistsSync.mockImplementation((path) => {
      const s = String(path);
      return s.endsWith("settings.json") || s.endsWith(".wise/hud-config.json");
    });
    mockReadFileSync.mockImplementation((path) => {
      const s = String(path);
    ...
    const [, raw] = mockAtomicWriteFileSync.mock.calls[0] as [string, string];
    const written = JSON.parse(raw);
    expect(written.wiseHud.elements.cwd).toBe(true);
    expect(written.wiseHud.elements.gitBranch).toBe(true);
    expect(written.wiseHud.wrapMode).toBe("truncate");
  });
});
...
describe("layout config round-trip", () => {
  ...
  it("readHudConfig preserves elementOrder from settings.json", () => {
    ...
    mockReadFileSync.mockReturnValue(
      JSON.stringify({
        wiseHud: {
          elementOrder: ["contextBar", "wiseLabel", "session"],
        },
      }),
    );
  ...
  it("writeHudConfig persists layout to settings.json", () => {
    ...
    const ok = writeHudConfig({
      ...DEFAULT_HUD_CONFIG,
      layout: {
        main: ["contextBar", "wiseLabel", "ralph"],
      },
    });
    ...
    const [, raw] = mockAtomicWriteFileSync.mock.calls[0] as [string, string];
    const written = JSON.parse(raw);
    expect(written.wiseHud.layout).toEqual({
      main: ["contextBar", "wiseLabel", "ralph"],
    });
  });
  ...
  it("writeHudConfig persists elementOrder to settings.json", () => {
    ...
    const [, raw] = mockAtomicWriteFileSync.mock.calls[0] as [string, string];
    const written = JSON.parse(raw);
    expect(written.wiseHud.elementOrder).toEqual([
      "contextBar",
      "wiseLabel",
      "session",
    ]);
