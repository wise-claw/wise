...
} from '../hooks/wise-orchestrator/index.js';
import type { AuditEntry } from '../hooks/wise-orchestrator/audit.js';
...
describe('delegation-enforcement-levels', () => {
  ...
  describe('getEnforcementLevel via processOrchestratorPreTool', () => {
    ...
    it('local config overrides global config', () => {
      ...
      mockExistsSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (/[\\/]tmp[\\/]test-project[\\/]\.wise[\\/]config\.json$/.test(s)) return true;
        if (/[\\/]mock[\\/]home[\\/]\.claude[\\/]\.wise-config\.json$/.test(s)) return true;
        return false;
      });
      mockReadFileSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (/[\\/]tmp[\\/]test-project[\\/]\.wise[\\/]config\.json$/.test(s)) {
          return JSON.stringify({ delegationEnforcementLevel: 'off' });
        }
        if (/[\\/]mock[\\/]home[\\/]\.claude[\\/]\.wise-config\.json$/.test(s)) {
  ...
  describe('processOrchestratorPreTool enforcement levels', () => {
    function setEnforcement(level: string) {
      mockExistsSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (/[\\/]\.wise[\\/]config\.json$/.test(s)) return true;
        return false;
      });
      mockReadFileSync.mockImplementation(() => {
  ...
  describe('isAllowedPath', () => {
    it('returns true for .wise/ paths', () => {
      expect(isAllowedPath('.wise/plans/test.md')).toBe(true);
    });
    ...
    it('rejects .wise/../src/file.ts traversal', () => {
      expect(isAllowedPath('.wise/../src/file.ts')).toBe(false);
    });
    ...
    it('handles Windows-style .wise paths', () => {
      expect(isAllowedPath('.wise\\plans\\test.md')).toBe(true);
    });

    it('rejects Windows traversal .wise\\..\\src\\file.ts', () => {
      expect(isAllowedPath('.wise\\..\\src\\file.ts')).toBe(false);
    });

    // Nested .wise in non-root position (should be rejected for relative paths)
    it('rejects foo/.wise/bar.ts as relative path', () => {
      expect(isAllowedPath('foo/.wise/bar.ts')).toBe(false);
    });

    // Windows mixed-separator edge cases
    it('rejects mixed separator traversal .wise\\..\\..\\secret', () => {
      expect(isAllowedPath('.wise\\..\\..\\secret')).toBe(false);
    });

    it('rejects double-dot with mixed separators .wise/..\\src', () => {
      expect(isAllowedPath('.wise/..\\src')).toBe(false);
    });

    it('rejects UNC paths as not relative to project', () => {
      expect(isAllowedPath('\\\\server\\share\\.wise\\file')).toBe(false);
    });

    it('rejects absolute Windows drive paths without worktree root', () => {
      expect(isAllowedPath('C:\\repo\\.wise\\file')).toBe(false);
    });
  });
