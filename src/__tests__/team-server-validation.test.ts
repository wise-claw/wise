...
const VALID_JOB_ID_RE = /^wise-[a-z0-9]{1,16}$/;
...
describe('validateJobId', () => {
  describe('rejects path traversal and invalid inputs', () => {
    const traversalPayloads = [
      ...
      'wise-UPPERCASE',
      'wise-has spaces',
      'wise-' + 'a'.repeat(17), // 17 chars — exceeds 16-char limit
      'notprefixed',
      'wise_underscore',
      'wise-abc!@#',
    ];
  ...
  describe('accepts valid job IDs', () => {
    const validIds = [
      'wise-abc123',
      'wise-a',
      'wise-123456789012', // 12 chars
      'wise-1',
      'wise-abcdefghijkl', // 12 lowercase letters
      'wise-abcdefghijklmnop', // exactly 16 chars
    ];
...
describe('team-server handler validation integration', () => {
  ...
  it('production validateJobId regex matches test regex', async () => {
    ...
    expect(src).toContain('/^wise-[a-z0-9]{1,16}$/');
  });
