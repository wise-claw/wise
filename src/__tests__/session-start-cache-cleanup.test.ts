 ...
 *
 * The script's cleanup block scans ~/.claude/plugins/cache/wise/wise/
...
describe('session-start.mjs — plugin cache cleanup uses symlinks', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'wise-cache-test-'));
    fakeHome = join(tmpDir, 'home');
    fakeCacheBase = join(fakeHome, '.claude', 'plugins', 'cache', 'wise', 'wise');
    fakeProject = join(tmpDir, 'project');

    // Create fake project directory with .wise
  ...
  it('keeps explicit external plugin roots authoritative for update checks', () => {
    ...
    const updateCache = join(fakeHome, '.claude', '.wise', 'update-check.json');
    mkdirSync(join(fakeHome, '.claude', '.wise'), { recursive: true });
    ...
    expect(parsed.systemMessage).toContain('[WISE UPDATE AVAILABLE]');
