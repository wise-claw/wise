...
// Mock getWiseRoot to use our test directory
...
describe('Shared Memory', () => {
  ...
  beforeEach(() => {
    ...
    tildeConfigDir = join(homedir(), `.wise-test-shared-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('writeEntry / readEntry', () => {
    ...
    it('should create namespace directory automatically', () => {
      ...
      const nsDir = join(wiseDir, 'state', 'shared-memory', 'auto-ns');
      expect(existsSync(nsDir)).toBe(true);
    });

    it('should store entry as JSON file', () => {
      writeEntry('ns', 'mykey', { x: 1 });
      const filePath = join(wiseDir, 'state', 'shared-memory', 'ns', 'mykey.json');
  ...
  describe('TTL support', () => {
    ...
    it('should auto-delete expired entries on read', () => {
      ...
      const filePath = join(wiseDir, 'state', 'shared-memory', 'ns');
  ...
  describe('listEntries', () => {
    ...
    it('should filter out expired entries', () => {
      ...
      const nsDir = join(wiseDir, 'state', 'shared-memory', 'ns');
      const expiredEntry = {
        key: 'dead',
        value: 'expired',
        namespace: 'ns',
  ...
  describe('isSharedMemoryEnabled', () => {
    ...
    it('should read config from the active CLAUDE_CONFIG_DIR', () => {
      ...
      writeFileSync(join(claudeConfigDir, '.wise-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
    ...
    it('should expand ~-prefixed CLAUDE_CONFIG_DIR values', () => {
      ...
      writeFileSync(join(tildeConfigDir, '.wise-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
  ...
  describe('atomic writes', () => {
    it('should not leave temp file after successful write', () => {
      ...
      const filePath = join(wiseDir, 'state', 'shared-memory', 'ns', 'clean-test.json');
      expect(existsSync(filePath)).toBe(true);
    ...
    it('should preserve original file when a leftover .tmp exists from a prior crash', () => {
      ...
      const filePath = join(wiseDir, 'state', 'shared-memory', 'ns', 'crash-test.json');
