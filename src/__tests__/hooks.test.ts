...
describe('Keyword Detector', () => {
  ...
  describe('detectKeywordsWithType', () => {
    ...
    it('should detect cancel keyword', () => {
      const detected = detectKeywordsWithType('cancelwise this task');
      expect(detected).toHaveLength(1);
      expect(detected[0].type).toBe('cancel');
      expect(detected[0].keyword).toBe('cancelwise');
    });

    it('should detect cancel keyword variations', () => {
      const cancelTerms = ['cancelwise', 'stopwise'];
  ...
  describe('getPrimaryKeyword', () => {
    ...
    it('should give cancel highest priority', () => {
      const primary = getPrimaryKeyword('stopwise searching for files');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });

    it('should give cancel priority over analyze', () => {
      const primary = getPrimaryKeyword('cancelwise this investigation');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });

    it('should prioritize cancel over all other keywords', () => {
      const primary = getPrimaryKeyword('stopwise ultrawork and search');
      expect(primary).not.toBeNull();
      expect(primary!.type).toBe('cancel');
    });
    ...
    it('should not detect ralph in /wise:ralph-init', () => {
      const primary = getPrimaryKeyword('/wise:ralph-init "my project"');
...
describe('Team staged workflow integration', () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `wise-team-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(testDir, '.wise', 'state', 'sessions', sessionId), { recursive: true });
  ...
  it('restores active Team stage on session-start', async () => {
    writeFileSync(
      join(testDir, '.wise', 'state', 'sessions', sessionId, 'team-state.json'),
      JSON.stringify({
        active: true,
  ...
  it('compacts WISE-style root AGENTS guidance on session-start without dropping key sections', async () => {
    const agentsContent = `# wise - Intelligent Multi-Agent Orchestration
...
- preserve verification
  ...
  it('keeps session-start under budget when only a tiny omission remainder remains', async () => {
    writeFileSync(
      join(testDir, '.wise', 'state', 'sessions', sessionId, 'ultrawork-state.json'),
      JSON.stringify({
        active: true,
        session_id: sessionId,
...
<skills>${'- drop skills\n'.repeat(500)}</skills>`
  ...
  it('trips Team continuation circuit breaker after max stop reinforcements', async () => {
    ...
    writeFileSync(
      join(testDir, '.wise', 'state', 'sessions', sessionId, 'team-pipeline-stop-breaker.json'),
  ...
  it('bypasses autopilot continuation when transcript context is critically exhausted', async () => {
    ...
    writeFileSync(
      join(testDir, '.wise', 'state', 'sessions', sessionId, 'autopilot-state.json'),
      JSON.stringify({
        active: true,
        phase: 'execution',
        session_id: sessionId,
...
describe('Persistent-mode reply cleanup behavior', () => {
  ...
  beforeEach(() => {
    testDir = join(tmpdir(), `wise-reply-cleanup-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    tempHome = join(tmpdir(), `wise-reply-home-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('does not remove reply-session registry on idle Stop/persistent-mode', async () => {
    const registryPath = join(homedir(), '.wise', 'state', 'reply-session-registry.jsonl');
    mkdirSync(join(homedir(), '.wise', 'state'), { recursive: true });
    writeFileSync(
      registryPath,
      `${JSON.stringify({
        platform: 'telegram',
...
describe('Mutual Exclusion - UltraQA and Ralph', () => {
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `wise-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(testDir, { recursive: true });
  ...
  describe('isUltraQAActive', () => {
    ...
    it('should return true when ultraqa is active', () => {
      const stateFile = join(testDir, '.wise', 'state', 'ultraqa-state.json');
  ...
  describe('isRalphLoopActive', () => {
    ...
    it('should return true when ralph is active', () => {
      const stateFile = join(testDir, '.wise', 'state', 'ralph-state.json');
      writeFileSync(stateFile, JSON.stringify({ active: true }));
  ...
  describe('UltraQA mutual exclusion', () => {
    it('should fail to start UltraQA when Ralph is active', () => {
      ...
      const sessionDir = join(testDir, '.wise', 'state', 'sessions', 'test-session');
    ...
    it('should succeed starting UltraQA when ralph state exists but inactive', () => {
      const ralphStateFile = join(testDir, '.wise', 'state', 'ralph-state.json');
  ...
  describe('Ralph mutual exclusion', () => {
    ...
    it('should succeed starting Ralph when ultraqa state exists but inactive', () => {
      const ultraqaStateFile = join(testDir, '.wise', 'state', 'ultraqa-state.json');
