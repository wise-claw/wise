...
describe('Ralph Progress Module', () => {
  ...
  describe('initProgress', () => {
    it('should create progress.txt in .wise directory', () => {
      expect(initProgress(testDir)).toBe(true);
      expect(existsSync(join(testDir, '.wise', PROGRESS_FILENAME))).toBe(true);
    });
  ...
  describe('readProgressRaw / readProgress', () => {
    ...
    it('should read progress from .wise directory', () => {
      const wiseDir = join(testDir, '.wise');
      mkdirSync(wiseDir, { recursive: true });
      writeFileSync(join(wiseDir, PROGRESS_FILENAME), '# Test');
      expect(readProgressRaw(testDir)).toBe('# Test');
    });
  });
