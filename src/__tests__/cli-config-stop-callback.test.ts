...
describe('wise config-stop-callback tag options', () => {
  it('updates telegram tagList options and preserves existing config fields', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'wise-cli-stop-callback-home-'));
    const configPath = join(homeDir, '.claude', '.wise-config.json');
  ...
  it('uses CLAUDE_CONFIG_DIR for the default file callback path', () => {
    ...
    const configPath = join(claudeConfigDir, '.wise-config.json');
