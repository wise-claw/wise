 ...
 * model instead of using WISE's per-agent model routing.
 */
...
describe('routing.forceInherit (issue #1135)', () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.WISE_ROUTING_FORCE_INHERIT;
    vi.clearAllMocks();
  });
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.WISE_ROUTING_FORCE_INHERIT = originalEnv;
    }
  });
  ...
  describe('processPreToolUse with forceInherit', () => {
    it('strips model from Task calls when forceInherit is true', () => {
      ...
      expect(modified.subagent_type).toBe('wise:executor');
    });
