...
import {
  ...
  extractWiseVersionFromClaudeMd,
...
describe('Installer Constants', () => {
  ...
  describe('VERSION', () => {
    ...
    it('should keep docs/CLAUDE.md version marker in sync with package version', () => {
      const versionMatch = CLAUDE_MD_CONTENT.match(/<!-- WISE:VERSION:([^\s]*?) -->/);
      expect(versionMatch?.[1]).toBe(VERSION);
    });
  });


  describe('extractWiseVersionFromClaudeMd()', () => {
    it('prefers the WISE version marker', () => {
      const content = `<!-- WISE:VERSION:4.7.7 -->
# wise - Intelligent Multi-Agent Orchestration`;
      expect(extractWiseVersionFromClaudeMd(content)).toBe('v4.7.7');
    });

    it('falls back to legacy heading versions', () => {
      const content = '# wise v4.6.0 - Intelligent Multi-Agent Orchestration';
      expect(extractWiseVersionFromClaudeMd(content)).toBe('v4.6.0');
    });
  });

  describe('syncPersistedSetupVersion()', () => {
    it('updates setupVersion for already-configured installs', () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'wise-installer-test-'));
      const configPath = join(tempDir, '.wise-config.json');
  ...
  describe('Plugin Detection', () => {
    ...
    it('should return true when CLAUDE_PLUGIN_ROOT is set', () => {
      process.env.CLAUDE_PLUGIN_ROOT = '/home/user/.claude/plugins/marketplaces/wise';
  ...
  describe('Project-Scoped Plugin Detection', () => {
    ...
    it('should return false for global plugin installation', () => {
      ...
      process.env.CLAUDE_PLUGIN_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '3.9.0');
      expect(isProjectScopedPlugin()).toBe(false);
    });
    ...
    it('should return true for project-scoped plugin installation', () => {
      ...
      process.env.CLAUDE_PLUGIN_ROOT = '/home/user/myproject/.claude/plugins/wise';
    ...
    it('should return true when plugin is outside global plugin directory', () => {
      ...
      process.env.CLAUDE_PLUGIN_ROOT = '/var/projects/app/.claude/plugins/wise';
      expect(isProjectScopedPlugin()).toBe(true);
    });

    it('should handle Windows-style paths', () => {
      // Windows paths with backslashes should be normalized
      process.env.CLAUDE_PLUGIN_ROOT = 'C:\\Users\\user\\project\\.claude\\plugins\\wise';
      expect(isProjectScopedPlugin()).toBe(true);
    });

    it('should handle trailing slashes in paths', () => {
      process.env.CLAUDE_PLUGIN_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise') + '/';
      expect(isProjectScopedPlugin()).toBe(false);
    });
  });
