...
beforeEach(() => {
  savedPluginRoot = process.env[WISE_PLUGIN_ROOT_ENV];
  delete process.env[WISE_PLUGIN_ROOT_ENV];
});

afterEach(() => {
  if (savedPluginRoot === undefined) delete process.env[WISE_PLUGIN_ROOT_ENV];
  else process.env[WISE_PLUGIN_ROOT_ENV] = savedPluginRoot;
});
...
describe('HUD marketplace resolution', () => {
  it('wise-hud.mjs converts absolute HUD paths to file URLs before dynamic imports', () => {
    const configDir = mkdtempSync(join(tmpdir(), 'wise-hud-wrapper-'));
    ...
    const hudScriptPath = join(configDir, 'hud', 'wise-hud.mjs');
    ...
    expect(settings.statusLine?.command).toContain(`${join(configDir, 'hud', 'wise-hud.mjs').replace(/\\/g, '/')}`);
    if (process.platform !== 'win32') {
      expect(settings.statusLine?.command).toContain('wise-hud-cache.sh');
      expect(existsSync(join(configDir, 'hud', 'wise-hud-cache.sh'))).toBe(true);
      expect(existsSync(join(configDir, 'hud', 'find-node.sh'))).toBe(true);
    }
    expect(existsSync(join(configDir, '.wise-config.json'))).toBe(true);
    ...
    // WISE_PLUGIN_ROOT replaced the legacy devPath branch (binary-weaving-mountain).
  ...
  it('wise-hud.mjs loads a marketplace install when plugin cache is unavailable', () => {
    const configDir = mkdtempSync(join(tmpdir(), 'wise-hud-marketplace-'));
    tempDirs.push(configDir);
    ...
    const marketplaceRoot = join(configDir, 'plugins', 'marketplaces', 'wise');
  ...
  it('wise-hud.mjs surfaces dynamic import errors from WISE_PLUGIN_ROOT HUD paths', () => {
    const configDir = mkdtempSync(join(tmpdir(), 'wise-hud-import-error-'));
    tempDirs.push(configDir);
    ...
    const output = execFileSync(process.execPath, [hudScriptPath], {
      ...
      env: {
        ...
        CLAUDE_CONFIG_DIR: configDir,
        HOME: fakeHome,
        WISE_PLUGIN_ROOT: pluginRoot,
        WISE_HUD_DISABLE_NPM_FALLBACK: '1',
      },
      encoding: 'utf8',
    ...
    expect(normalized).toContain('[WISE HUD] HUD import failed from');
    expect(normalized).toContain('/broken-plugin-root/dist/hud/index.js');
  });

  it('wise-hud.mjs loads a global npm install outside a Node project via npm prefix resolution', () => {
    const configDir = mkdtempSync(join(tmpdir(), 'wise-hud-global-prefix-'));
    tempDirs.push(configDir);
  ...
  it('wise-hud.mjs loads the published npm package name before the branded fallback', () => {
    const configDir = mkdtempSync(join(tmpdir(), 'wise-hud-npm-package-'));
    tempDirs.push(configDir);
