...
describe('Notepad Module', () => {
  ...
  describe('initNotepad', () => {
    it('should create notepad.md with correct structure', () => {
      ...
      expect(content).toContain('Auto-managed by WISE');
    });

    it('should create .wise directory if not exists', () => {
      const wiseDir = join(testDir, '.wise');
      expect(existsSync(wiseDir)).toBe(false);

      initNotepad(testDir);

      expect(existsSync(wiseDir)).toBe(true);
    });
