...
describe('auto-update reconciliation', () => {
    ...
    mockedReaddirSync.mockImplementation((path: Parameters<typeof readdirSync>[0], options?: Parameters<typeof readdirSync>[1]) => {
      ...
      if (normalized.endsWith('/commands')) {
        return options && typeof options === 'object' && 'withFileTypes' in options
          ? [{ name: 'wise-setup.md', isFile: () => true, isDirectory: () => false }] as any
          : ['wise-setup.md'] as any;
      }
  ...
  afterEach(() => {
    vi.unstubAllGlobals();
    delete process.env.WISE_UPDATE_RECONCILE;
  ...
  it('reconciles runtime state without re-injecting settings hooks', () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith('/tmp/wise-test-hooks', { recursive: true });
  ...
  it('syncs active plugin cache roots and logs when copy occurs', () => {
    ...
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '4.1.5');
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('[wise update] Synced plugin cache');
  });



  it('fails reconciliation when active plugin cache repair reports validation errors', () => {
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '4.14.1');
  ...
  it('skips plugin cache sync silently when no active plugin roots exist', () => {
    ...
    expect(consoleLogSpy).not.toHaveBeenCalledWith('[wise update] Synced plugin cache');
  });


  it('updates installed_plugins.json to the new cache version after plugin cache sync succeeds', () => {
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise');
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'wise@wise': [{ installPath: oldRoot, version: '4.14.0', enabled: true }],
          },
        });
      }
      return '';
  ...
  it('preserves Windows-style installPath separators when rewriting installed_plugins.json', () => {
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise');
    const oldWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\wise\\wise\\4.14.0';
    const newWindowsRoot = 'C:\\Users\\bellman\\.claude\\plugins\\cache\\wise\\wise\\4.14.1';
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'wise@wise': [{ installPath: oldWindowsRoot, version: '4.14.0' }],
          },
        });
      }
      return '';
    ...
    expect(result.errors).toEqual([]);
    const written = String(mockedWriteFileSync.mock.calls.find(([path]) => String(path).includes('installed_plugins.json.tmp-'))?.[1]);
    expect(JSON.parse(written).plugins['wise@wise'][0]).toMatchObject({
      version: '4.14.1',
    ...
    mockedReadFileSync.mockImplementation((path: Parameters<typeof readFileSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/installed_plugins.json')) {
        return JSON.stringify({
          version: 2,
          plugins: {
            'wise@wise': [{ installPath: join(cacheRoot, '4.14.0'), version: '4.14.0' }],
          },
        });
      }
      return '';
  ...
  it('syncs the plugin cache directory when cache root exists', () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.startsWith('/usr/lib/node_modules/wise/')) {
        return normalized.endsWith('/dist')
          ...
          || normalized.endsWith('/commands/wise-setup.md')
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('[wise update] Plugin cache synced');
  });
  ...
  it('handles plugin cache sync errors non-fatally', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      `[wise update] Plugin cache sync warning: Failed to sync dist to ${versionedCacheRoot}: copy failed`,
    );
  });
  ...
  it('only blocks standalone update inside an active plugin session', () => {
    ...
    process.env.CLAUDE_PLUGIN_ROOT = '/tmp/.claude/plugins/cache/wise/wise/4.1.5';
    expect(shouldBlockStandaloneUpdateInCurrentSession()).toBe(false);
  ...
  it('dedupes plugin roots and ignores missing targets during sync', () => {
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const activeRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '4.1.5');
    const staleRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '4.1.4');
  ...
  it('allows standalone update when CLAUDE_PLUGIN_ROOT is inherited without an active Claude session', async () => {
    const pluginRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise', '4.1.5');
    const cacheRoot = join(CLAUDE_CONFIG_DIR, 'plugins', 'cache', 'wise', 'wise');
  ...
  it('restores global Claude Code when npm removes an existing global install during update', async () => {
    ...
    mockedExistsSync.mockImplementation((path: Parameters<typeof existsSync>[0]) => {
      ...
      if (normalized.endsWith('/plugins/marketplaces/wise')) {
        return false;
      }
      if (normalized.endsWith('/plugins/cache/wise/wise')) {
        return false;
      }
      return true;
    });
    ...
    try {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith('[wise update] Restoring global @anthropic-ai/claude-code@1.2.3 after npm update...');
      expect(consoleLogSpy).toHaveBeenCalledWith('[wise update] Restored global @anthropic-ai/claude-code');
    } finally {
  ...
  it('skips marketplace auto-sync when the marketplace clone has local modifications', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/wise'), 'status', '--porcelain', '--untracked-files=normal'],
      expect.any(Object)
    );
  ...
  it('skips marketplace auto-sync when the marketplace clone has local commits', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/wise'), 'rev-list', '--left-right', '--count', 'HEAD...origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('fast-forwards a clean marketplace clone when origin/main is ahead', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      'git',
      ['-C', expect.stringContaining('/plugins/marketplaces/wise'), 'merge', '--ff-only', 'origin/main'],
      expect.any(Object)
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'git',
  ...
  it('re-execs with wise.cmd on Windows and persists metadata after reconciliation', async () => {
    mockPlatform('win32');
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith('C:\\Users\\bellman\\AppData\\Roaming\\npm\\wise.cmd', ['update-reconcile'], expect.objectContaining({
      encoding: 'utf-8',
      stdio: 'pipe',
      timeout: 60000,
      shell: true,
      windowsHide: true,
      env: expect.objectContaining({ WISE_UPDATE_RECONCILE: '1' }),
    }));
    expect(mockedWriteFileSync).toHaveBeenCalledWith(expect.stringContaining('.wise-version.json'), expect.stringContaining('"version": "4.1.6"'));
  });
