 ...
 * and contains a trusted subpath (Claude config dir or ~/.wise/).
...
export function validateConfigPath(configPath: string, homeDir: string, claudeConfigDir: string): boolean {
  ...
  const normalizedWiseDir = resolve(homeDir, '.wise');
  const hasWiseComponent = resolved.includes('/.wise/') || resolved.endsWith('/.wise');
  const isTrustedSubpath =
    resolved === normalizedConfigDir ||
    resolved.startsWith(normalizedConfigDir + '/') ||
    resolved === normalizedWiseDir ||
    resolved.startsWith(normalizedWiseDir + '/') ||
    hasWiseComponent;
...
function main(): void {
  ...
  if (!validateConfigPath(configPath, home, claudeConfigDir)) {
    console.error(`Config path must be under ~/ with ${claudeConfigDir} or ~/.wise/ subpath: ${configPath}`);
    process.exit(1);
  }
