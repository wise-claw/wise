 ...
 * Gated behind the WISE_TEAM_SCALING_ENABLED environment variable.
 *
...
const WISE_TEAM_SCALING_ENABLED_ENV = 'WISE_TEAM_SCALING_ENABLED';
...
export function isScalingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[WISE_TEAM_SCALING_ENABLED_ENV];
  if (!raw) return false;
...
function assertScalingEnabled(env: NodeJS.ProcessEnv = process.env): void {
  if (!isScalingEnabled(env)) {
    throw new Error(
      `Dynamic scaling is disabled. Set ${WISE_TEAM_SCALING_ENABLED_ENV}=1 to enable.`,
    );
  }
}
...
export async function scaleUp(
  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
    ...
    const teamStateRoot = config.team_state_root ?? `${leaderCwd}/.wise/state/team/${sanitized}`;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const extraEnv: Record<string, string> = {
        ...
        WISE_TEAM_STATE_ROOT: teamStateRoot,
        WISE_TEAM_LEADER_CWD: leaderCwd,
        ...(worktree ? { WISE_TEAM_WORKTREE_PATH: worktree.path, WISE_TEAM_WORKER_CWD: workerCwd } : {}),
      };
      ...
      if (worktree) {
        try {
          const workerOverlayParams = {
            ...
            cwd: leaderCwd,
            instructionStateRoot: '$WISE_TEAM_STATE_ROOT',
          };
      ...
      const skipReadyWait = env.WISE_TEAM_SKIP_READY_WAIT === '1';
      if (!skipReadyWait) {
        try {
...
function resolveWorkerReadyTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.WISE_TEAM_READY_TIMEOUT_MS;
