...
beforeEach(() => {
  mocks.reset();
  process.env.WISE_RUNTIME_V2 = '1';
});

afterEach(() => {
  delete process.env.WISE_RUNTIME_V2;
});
...
describe('M5 v2 gate', () => {
  it('allows unset WISE_RUNTIME_V2 because runtime v2 is default-on', async () => {
    delete process.env.WISE_RUNTIME_V2;
    const repoRoot = makeRepoRoot();
    try {
      const cfg = defaultConfig(repoRoot);
  ...
  it('throws when WISE_RUNTIME_V2=0', async () => {
    process.env.WISE_RUNTIME_V2 = '0';
    const repoRoot = makeRepoRoot();
    try {
      const cfg = defaultConfig(repoRoot);
...
describe('commit watcher + auto-merge', () => {
  it('detects a SHA change and triggers a merge', async () => {
    ...
    try {
      ...
      const branchName = `wise-team/demo-team/${sanitizeName(workerName)}`;
      let revParseCount = 0;
      on(
  ...
  it('handles multiple workers independently', async () => {
    ...
    try {
      ...
      const branchA = `wise-team/demo-team/${sanitizeName('alice')}`;
      const branchB = `wise-team/demo-team/${sanitizeName('bob')}`;
      let aCount = 0;
      let bCount = 0;
      on(
  ...
  it('runs preflight reset before each merge', async () => {
    ...
    try {
      ...
      const branchName = `wise-team/demo-team/${sanitizeName('alice')}`;
      let count = 0;
      on(
        (args) => args[0] === 'rev-parse' && args[1] === `refs/heads/${branchName}`,
        () => {
...
describe('M6 recoverFromRestart', () => {
  it('loads persisted SHA state and reports orphaned rebases', async () => {
    ...
    try {
      ...
      mkdirSync(join(repoRoot, '.wise', 'state', 'team', sanitizeName(cfg.teamName)), {
        recursive: true,
      });
      ...
      atomicWriteJson(worktreesMetaPath, [
        {
          path: aliceWtPath,
          branch: 'wise-team/demo-team/alice',
          workerName: 'alice',
        ...
        {
          path: bobWtPath,
          branch: 'wise-team/demo-team/bob',
          workerName: 'bob',
          teamName: cfg.teamName,
          createdAt: new Date().toISOString(),
        },
...
describe('worker registration', () => {
  ...
  it('is idempotent — registering twice is a no-op', async () => {
    ...
    try {
      ...
      on(
        (args) => args[0] === 'rev-parse' && args[1].startsWith('refs/heads/wise-team'),
        () => 'sha\n',
      );
