...
describe('model-contract', () => {
  describe('backward-compat API shims', () => {
    ...
    it('isTrustedPrefix enforces directory boundaries (no sibling-prefix bypass)', () => {
      ...
      try {
        ...
        // custom trusted dirs (WISE_TRUSTED_CLI_DIRS) get the same boundary check
        const origCustom = process.env.WISE_TRUSTED_CLI_DIRS;
        process.env.WISE_TRUSTED_CLI_DIRS = '/opt/mybins';
        try {
          expect(isTrustedPrefix('/opt/mybins/grok')).toBe(true);
          expect(isTrustedPrefix('/opt/mybins-evil/grok')).toBe(false);
        } finally {
          if (origCustom === undefined) delete process.env.WISE_TRUSTED_CLI_DIRS;
          else process.env.WISE_TRUSTED_CLI_DIRS = origCustom;
        }
      } finally {
  ...
  describe('getContract', () => {
    ...
    it('blocks codex when external LLM is disabled', async () => {
      const origSecurity = process.env.WISE_SECURITY;
      ...
      try {
        ...
        if (origSecurity === undefined) {
          ...
          process.env.WISE_SECURITY = origSecurity;
        }
        const { clearSecurityConfigCache } = await import('../../lib/security-config.js');
        clearSecurityConfigCache();
      }
  ...
  describe('getWorkerEnv', () => {
    it('returns correct env vars', () => {
      ...
      expect(env.WISE_TEAM_WORKER).toBe('my-team/worker-1');
      expect(env.WISE_TEAM_NAME).toBe('my-team');
      expect(env.WISE_WORKER_AGENT_TYPE).toBe('codex');
    });
    ...
    it('propagates allowlisted model selection env vars into worker startup env', () => {
      const env = getWorkerEnv('my-team', 'worker-1', 'claude', {
        ...
        WISE_MODEL_HIGH: 'claude-opus-4-6-override',
        WISE_MODEL_MEDIUM: 'claude-sonnet-4-6-override',
        WISE_MODEL_LOW: 'claude-haiku-4-5-override',
        WISE_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL: 'gpt-5',
        WISE_GEMINI_DEFAULT_MODEL: 'gemini-2.5-pro',
      ...
      expect(env.WISE_MODEL_HIGH).toBe('claude-opus-4-6-override');
      expect(env.WISE_MODEL_MEDIUM).toBe('claude-sonnet-4-6-override');
      expect(env.WISE_MODEL_LOW).toBe('claude-haiku-4-5-override');
      expect(env.WISE_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL).toBe('gpt-5');
      expect(env.WISE_GEMINI_DEFAULT_MODEL).toBe('gemini-2.5-pro');
  ...
  describe('resolveClaudeWorkerModel (issue #1695)', () => {
    it('returns undefined when WISE_ROUTING_FORCE_INHERIT=true even if Bedrock model env vars are set', () => {
      vi.stubEnv('WISE_ROUTING_FORCE_INHERIT', 'true');
      ...
      vi.stubEnv('WISE_MODEL_MEDIUM', 'us.anthropic.claude-sonnet-4-5-20250929-v1:0');
      expect(resolveClaudeWorkerModel()).toBeUndefined();
      vi.unstubAllEnvs();
    });

    it('returns undefined when WISE_ROUTING_FORCE_INHERIT=true on Vertex', () => {
      vi.stubEnv('WISE_ROUTING_FORCE_INHERIT', 'true');
    ...
    it('falls back to WISE_MODEL_MEDIUM tier env var', () => {
      vi.stubEnv('CLAUDE_CODE_USE_BEDROCK', '1');
      vi.stubEnv('ANTHROPIC_MODEL', '');
      vi.stubEnv('CLAUDE_MODEL', '');
