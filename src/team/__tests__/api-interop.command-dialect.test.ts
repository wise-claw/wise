...
describe('team api command dialect resolution', () => {
  it('defaults to wise team api', () => {
    expect(resolveTeamApiCliCommand({} as NodeJS.ProcessEnv)).toBe('wise team api');
  });
  ...
  it('prefers wise team api when both contexts are present', () => {
    expect(resolveTeamApiCliCommand({
      WISE_TEAM_WORKER: 'demo-team/worker-1',
      OMX_TEAM_WORKER: 'demo-team/worker-2',
    } as NodeJS.ProcessEnv)).toBe('wise team api');
  });
