...
const BASE_MERGE_ARGS = {
  workerName: 'writer',
  workerBranch: 'wise-team/test-team/writer',
  leaderBranch: 'wise-team/test-team-leader',
...
const BASE_REBASE_ARGS = {
  ...
  conflictingFiles: CONFLICTS,
  baseSha: 'def5678',
  worktreePath: '/repo/.wise/team/test-team/worktrees/writer',
...
describe('formatMergeConflictForLeader', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Merge conflict: writer → wise-team/test-team-leader

      **Worker branch:** \`wise-team/test-team/writer\`
      **Leader branch:** \`wise-team/test-team-leader\`
      **Merge base:** \`abc1234\`
      ...
      **Leader: choose strategy.** To resolve, run:
      ...
      \`\`\`sh
      git checkout wise-team/test-team-leader && git merge --no-ff wise-team/test-team/writer
  ...
  it('includes resolution recipe with correct branches', () => {
    const result = formatMergeConflictForLeader(BASE_MERGE_ARGS);
    expect(result).toContain('git checkout wise-team/test-team-leader && git merge --no-ff wise-team/test-team/writer');
  });
...
describe('formatRebaseConflictForWorker', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Rebase conflict: writer onto wise-team/test-team-leader

      **Worker branch:** \`wise-team/test-team/writer\`
      **Base branch:** \`wise-team/test-team-leader\`
      **Base SHA:** \`def5678\`
      **Worktree:** \`/repo/.wise/team/test-team/worktrees/writer\`
      **Observed at:** 2001-09-09T01:46:40.000Z
  ...
  it('includes worktree path', () => {
    const result = formatRebaseConflictForWorker(BASE_REBASE_ARGS);
    expect(result).toContain('/repo/.wise/team/test-team/worktrees/writer');
  });
...
const TEST_CWD = join(tmpdir(), `wise-test-conflict-mailbox-${process.pid}`);
...
describe('deliverMergeConflictToLeader', () => {
  it('writes to .wise/state/team/{team}/leader/inbox.md', async () => {
    ...
    const expectedPath = join(TEST_CWD, `.wise/state/team/${TEST_TEAM}/leader/inbox.md`);
...
describe('deliverRebaseConflictToWorker', () => {
  it('writes to .wise/state/team/{team}/workers/{worker}/inbox.md', async () => {
    ...
    const expectedPath = join(TEST_CWD, `.wise/state/team/${TEST_TEAM}/workers/writer/inbox.md`);
    expect(existsSync(expectedPath)).toBe(true);
    const content = readFileSync(expectedPath, 'utf-8');
    expect(content).toContain(message);
  });
