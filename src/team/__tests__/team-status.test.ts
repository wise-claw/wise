...
let WORK_DIR: string;
// Canonical tasks dir: {WORK_DIR}/.wise/state/team/{TEST_TEAM}/tasks/
let TASKS_DIR: string;

beforeEach(() => {
  WORK_DIR = join(realpathSync(tmpdir()), `wise-team-status-test-${Date.now()}`);
  TASKS_DIR = join(WORK_DIR, '.wise', 'state', 'team', TEST_TEAM, 'tasks');
  mkdirSync(TASKS_DIR, { recursive: true });
  mkdirSync(join(WORK_DIR, '.wise', 'state', 'team-bridge', TEST_TEAM), { recursive: true });
  mkdirSync(join(WORK_DIR, '.wise', 'state'), { recursive: true });
});
...
function writeWorkerRegistry(workers: McpWorkerMember[]): void {
  const registryPath = join(WORK_DIR, '.wise', 'state', 'team-mcp-workers.json');
...
function writeHeartbeatFile(data: HeartbeatData): void {
  const hbPath = join(WORK_DIR, '.wise', 'state', 'team-bridge', TEST_TEAM, `${data.workerName}.heartbeat.json`);
...
function makeWorker(name: string, provider: 'codex' | 'gemini' = 'codex'): McpWorkerMember {
  return {
    ...
    model: 'test-model',
    joinedAt: Date.now(),
    tmuxPaneId: `wise-team-${TEST_TEAM}-${name}`,
    cwd: WORK_DIR,
    backendType: 'tmux',
    subscriptions: [],
  };
