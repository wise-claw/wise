 ...
 * Path: .wise/reports/team-{teamName}-{timestamp}.md
...
export function saveTeamReport(
  ...
  const dir = join(getWiseRoot(workingDirectory), 'reports');
  ensureDirWithMode(dir);
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const filePath = join(dir, `team-${teamName}-${timestamp}.md`);
  // filePath lives under getWiseRoot(...)/reports, which in a .wise-workspace
