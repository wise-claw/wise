 ...
 *
 * Storage: append-only JSONL at .wise/logs/team-usage-{team}.jsonl
 */
...
function getUsageLogPath(workingDirectory: string, teamName: string): string {
  return join(getWiseRoot(workingDirectory), 'logs', `team-usage-${teamName}.jsonl`);
}
...
export function recordTaskUsage(
  ...
  const dir = join(getWiseRoot(workingDirectory), 'logs');
  // logPath lives under getWiseRoot(...)/logs, which in a .wise-workspace layout
