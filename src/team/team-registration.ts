...
function shadowRegistryPath(workingDirectory: string): string {
  const result = join(getWiseRoot(workingDirectory), 'state', 'team-mcp-workers.json');
  validateResolvedPath(result, join(getWiseRoot(workingDirectory), 'state'));
  return result;
}

function probeResultPath(workingDirectory: string): string {
  return join(getWiseRoot(workingDirectory), 'state', 'config-probe-result.json');
}
 ...
 * - 'shadow': Write member to .wise/state/team-mcp-workers.json (fallback)
 *
