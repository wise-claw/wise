 ...
 * Sessions are named "wise-team-{teamName}-{workerName}".
 */
...
const TMUX_SESSION_PREFIX = 'wise-team';
...
async function waitForShellReady(paneId: string, opts: WaitForShellReadyOptions = {}): Promise<boolean> {
  ...
  const envTimeout = Number.parseInt(process.env.WISE_TEAM_SHELL_READY_TIMEOUT_MS ?? '', 10);
...
export function buildWorkerStartCommand(config: WorkerPaneConfig): string {
  ...
  const shouldSourceRc = process.env.WISE_TEAM_NO_RC !== '1';
...
/** Build session name: "wise-team-{teamName}-{workerName}" */
 ...
 *   <current-js-runtime> dist/team/bridge-entry.js --config /tmp/wise-bridge-{worker}.json
 */
...
export async function createTeamSession(
  ...
  if (useDedicatedWindow) {
    ...
    const windowName = `wise-${sanitizeName(teamName)}`.slice(0, 32);
 ...
 * Worker startup: env WISE_TEAM_WORKER={teamName}/workerName shell -lc "exec agentCmd"
 */
...
export async function waitForPaneReady(
  ...
  const envTimeout = Number.parseInt(process.env.WISE_SHELL_READY_TIMEOUT_MS ?? '', 10);
  const timeoutMs = Number.isFinite(opts.timeoutMs) && (opts.timeoutMs ?? 0) > 0
    ? Number(opts.timeoutMs)
  ...
  console.warn(
    ...
    `(set WISE_SHELL_READY_TIMEOUT_MS to tune)`
  );
  return false;
}
...
export function shouldAttemptAdaptiveRetry(args: {
  ...
  if (process.env.WISE_TEAM_AUTO_INTERRUPT_RETRY === '0') return false;
 ...
 * Prefixes with [WISE_TMUX_INJECT] marker to distinguish from user input.
 * Returns false on error (does not throw).
 */
export async function injectToLeaderPane(
  ...
  const prefixed = `[WISE_TMUX_INJECT] ${message}`.slice(0, 200);

  // If the leader is running a blocking tool (e.g. wise_run_team_wait shows
...
export async function killWorkerPanes(opts: {
  ...
  const shutdownPath = join(getWiseRoot(cwd), 'state', 'team', teamName, 'shutdown.json');
  try {
...
export async function killTeamSession(
  ...
  if (process.env.WISE_TEAM_ALLOW_KILL_CURRENT_SESSION !== '1' && process.env.TMUX) {
    try {
