...
//      `.wise/state/team/{team}/`, NOT to the shared `events.jsonl` used by the
...
function mergerWorktreePathFor(repoRoot: string, teamName: string): string {
  return join(getWiseRoot(repoRoot), 'team', sanitizeName(teamName), 'merger');
}

function persistedStatePath(repoRoot: string, teamName: string): string {
  return join(
    getWiseRoot(repoRoot),
    'state',
    'team',
...
function assertRuntimeV2Gate(): void {
  if (!isRuntimeV2Enabled()) {
    throw new Error('auto-merge requires runtime v2 (WISE_RUNTIME_V2 is explicitly disabled).');
  }
}
...
export async function startMergeOrchestrator(
  ...
  // Validate paths stay under the shared WISE team root (defence-in-depth).
  // mergerPath lives under getWiseRoot(...)/team, which in a .wise-workspace
  // layout is ABOVE repoRoot — validating against repoRoot would false-positive.
  validateResolvedPath(mergerPath, join(getWiseRoot(config.repoRoot), 'team'));
