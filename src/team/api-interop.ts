...
function teamStateExists(teamName: string, candidateCwd: string): boolean {
  ...
  const teamRoot = join(getWiseRoot(candidateCwd), 'state', 'team', teamName);
...
function parseTeamWorkerContextFromEnv(env: NodeJS.ProcessEnv = process.env): { teamName: string; workerName: string } | null {
  return parseTeamWorkerEnv(env.WISE_TEAM_WORKER) ?? parseTeamWorkerEnv(env.OMX_TEAM_WORKER);
}

function readTeamStateRootFromEnv(env: NodeJS.ProcessEnv = process.env): string | null {
  const candidate = typeof env.WISE_TEAM_STATE_ROOT === 'string' && env.WISE_TEAM_STATE_ROOT.trim() !== ''
    ? env.WISE_TEAM_STATE_ROOT.trim()
...
export function resolveTeamApiCliCommand(env: NodeJS.ProcessEnv = process.env): 'wise team api' | 'omx team api' {
  const hasWiseContext = (
    (typeof env.WISE_TEAM_WORKER === 'string' && env.WISE_TEAM_WORKER.trim() !== '')
    || (typeof env.WISE_TEAM_STATE_ROOT === 'string' && env.WISE_TEAM_STATE_ROOT.trim() !== '')
  );
  if (hasWiseContext) return 'wise team api';
...
function stateRootToWorkingDirectory(stateRoot: string): string {
  ...
  for (const marker of ['/.wise/state/team/', '/.omx/state/team/']) {
  ...
  for (const marker of ['/.wise/state', '/.omx/state']) {
    const idx = normalized.lastIndexOf(marker);
    if (idx >= 0) {
      const workspaceRoot = absolute.slice(0, idx);
      if (workspaceRoot && workspaceRoot !== '/') return workspaceRoot;
...
const WORKTREE_TRIGGER_STATE_ROOT = '$WISE_TEAM_STATE_ROOT';
