...
function getTrustedPrefixes(): string[] {
  ...
  const custom = (process.env.WISE_TRUSTED_CLI_DIRS ?? '')
    .split(':')
    .map(part => part.trim())
    .filter(Boolean)
...
export function resolveCliBinaryPath(binary: string): string {
  ...
  if (!isTrustedPrefix(resolvedPath)) {
    console.warn(`[wise:cli-security] CLI binary '${binary}' resolved to non-standard path: ${resolvedPath}`);
  }
...
const WORKER_MODEL_ENV_ALLOWLIST = [
  ...
  'WISE_MODEL_MEDIUM',
  'WISE_MODEL_LOW',
  'WISE_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL',
  'WISE_CODEX_DEFAULT_MODEL',
  'WISE_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL',
  'WISE_GEMINI_DEFAULT_MODEL',
  'WISE_EXTERNAL_MODELS_DEFAULT_GROK_MODEL',
  'WISE_GROK_DEFAULT_MODEL',
] as const;
...
export function resolveClaudeWorkerModel(
  ...
  if (env.WISE_ROUTING_FORCE_INHERIT === 'true') {
    return undefined;
  }
  ...
  // WISE tier env vars
  const wiseModel = env.WISE_MODEL_MEDIUM || '';
  if (wiseModel) {
    return wiseModel;
  }
