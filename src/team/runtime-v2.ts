 ...
 * Runtime selection:
 * - Default: v2 enabled
 * - Opt-out: set WISE_RUNTIME_V2=0|false|no|off to force legacy v1
 ...
 * Workers use `wise team api` CLI commands for all lifecycle transitions.
 */
function buildV2TaskInstruction(
  ...
  const failTaskCommand = formatWiseCliInvocation(
...
async function spawnV2Worker(opts: SpawnV2WorkerOptions): Promise<SpawnV2WorkerResult> {
  ...
  const instructionStateRoot = opts.worktreePath ? '$WISE_TEAM_STATE_ROOT' : undefined;
  ...
  const envVars = {
    ...
    WISE_TEAM_STATE_ROOT: teamStateRoot(opts.cwd, opts.teamName),
    WISE_TEAM_LEADER_CWD: opts.cwd,
    ...(opts.worktreePath ? { WISE_TEAM_WORKTREE_PATH: opts.worktreePath } : {}),
    ...(opts.workerCwd ? { WISE_TEAM_WORKER_CWD: opts.workerCwd } : {}),
  };
  ...
  const modelForAgent = opts.model ?? (() => {
    if (opts.agentType === 'codex') {
      return process.env.WISE_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL
        || process.env.WISE_CODEX_DEFAULT_MODEL
        || undefined;
    }
    if (opts.agentType === 'gemini') {
      return process.env.WISE_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL
        || process.env.WISE_GEMINI_DEFAULT_MODEL
        || undefined;
    }
    if (opts.agentType === 'grok') {
      return process.env.WISE_EXTERNAL_MODELS_DEFAULT_GROK_MODEL
        || process.env.WISE_GROK_DEFAULT_MODEL
        || undefined;
    }
...
export async function startTeamV2(config: StartTeamV2Config): Promise<TeamRuntimeV2> {
  ...
  let worktreeMode: TeamWorktreeMode = normalizeTeamWorktreeMode(
    process.env.WISE_TEAM_WORKTREE_MODE ?? pluginCfg.team?.ops?.worktreeMode,
  );
  ...
  if (config.autoMerge) {
    if (!isRuntimeV2Enabled()) {
      throw new Error('auto-merge requires WISE_RUNTIME_V2=1 (this feature is v2-only).');
    }
  ...
  await mkdir(join(getWiseRoot(leaderCwd), 'state', 'team', sanitized, 'mailbox'), { recursive: true });
  ...
  try {
    for (let i = 0; i < workerNames.length; i++) {
      ...
      const overlayPath = await writeWorkerOverlay({
        ...
        ...(workerWorktrees.has(wName) ? { instructionStateRoot: '$WISE_TEAM_STATE_ROOT' } : {}),
      });
  ...
  const permissionsSnapshot = {
    approval_mode: process.env.WISE_APPROVAL_MODE || 'default',
    sandbox_mode: process.env.WISE_SANDBOX_MODE || 'default',
    network_access: process.env.WISE_NETWORK_ACCESS === '1',
  };
  ...
  for (const w of config.workers) {
    try {
      ...
      const shutdownAckPath = w.worktree_path
        ? `$WISE_TEAM_STATE_ROOT/workers/${w.name}/shutdown-ack.json`
...
export async function resumeTeamV2(
  ...
  try {
    const sessionName = config.tmux_session || `wise-team-${sanitized}`;
...
export async function findActiveTeamsV2(cwd: string): Promise<string[]> {
  const root = join(getWiseRoot(cwd), 'state', 'team');
