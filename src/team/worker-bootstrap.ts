...
export interface WorkerBootstrapParams {
   ...
   * relative global state root (`.wise/state`); non-default values are treated as
   * a team-specific root (`.../.wise/state/team/<team>`), matching
   * `WISE_TEAM_STATE_ROOT` and `teamStateRoot()` semantics.
   */
  instructionStateRoot?: string;
}

const DEFAULT_INSTRUCTION_STATE_ROOT = '.wise/state';
...
function agentTypeGuidance(agentType: CliAgentType): string {
  const teamApiCommand = formatWiseCliInvocation('team api');
  const claimTaskCommand = formatWiseCliInvocation('team api claim-task');
  const transitionTaskStatusCommand = formatWiseCliInvocation('team api transition-task-status');
  switch (agentType) {
    case 'codex':
      return [
...
export function generateWorkerOverlay(params: WorkerBootstrapParams): string {
  ...
  const claimTaskCommand = formatWiseCliInvocation(`team api claim-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const sendAckCommand = formatWiseCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"ACK: ${workerName} initialized\\"}" --json`);
  const completeTaskCommand = formatWiseCliInvocation(`team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"completed\\",\\"claim_token\\":\\"<claim_token>\\",\\"result\\":\\"Summary: <what changed>\\\\nVerification: <tests/checks run>\\\\nSubagent skip reason: worker protocol forbids nested subagents; completed focused probe in-session\\"}" --json`);
  const failTaskCommand = formatWiseCliInvocation(`team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"failed\\",\\"claim_token\\":\\"<claim_token>\\"}" --json`);
  const readTaskCommand = formatWiseCliInvocation(`team api read-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\"}" --json`);
  const releaseClaimCommand = formatWiseCliInvocation(`team api release-task-claim --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"claim_token\\":\\"<claim_token>\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const mailboxListCommand = formatWiseCliInvocation(`team api mailbox-list --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const mailboxDeliveredCommand = formatWiseCliInvocation(`team api mailbox-mark-delivered --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\",\\"message_id\\":\\"<id>\\"}" --json`);
  const teamApiCommand = formatWiseCliInvocation('team api');
  const teamCommand = formatWiseCliInvocation('team');
...
- **Environment**: WISE_TEAM_WORKER=${teamName}/${workerName}
...
- Resolve the team state root in this order: \`WISE_TEAM_STATE_ROOT\` env -> worker identity \`team_state_root\` -> config/manifest \`team_state_root\` -> ${params.cwd}/.wise/state/team/${teamName}.
- \`WISE_TEAM_STATE_ROOT\` is the team-specific root (\`.../.wise/state/team/${teamName}\`). When it is set, append worker/mailbox paths directly below it; do not append another \`team/${teamName}\` segment.
- Worktree-backed workers MUST use the canonical leader-owned state root for inbox, mailbox, task lifecycle, status, heartbeat, and shutdown files; do not use a local worktree \`.wise/state\` when \`WISE_TEAM_STATE_ROOT\` is set.
...
- To leader: \`${formatWiseCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"<message>\\"}" --json`)}\`
...
## BEFORE YOU EXIT
You MUST call \`${formatWiseCliInvocation('team api transition-task-status')}\` to mark your task as "completed" or "failed" before exiting.
...
export async function appendToInbox(
  ...
  const inboxPath = join(cwd, `.wise/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`);
...
export async function ensureWorkerStateDir(
  ...
  const workerDir = join(cwd, `.wise/state/team/${teamName}/workers/${workerName}`);
  await mkdir(workerDir, { recursive: true });

  // Also ensure mailbox dir
  const mailboxDir = join(cwd, `.wise/state/team/${teamName}/mailbox`);
  await mkdir(mailboxDir, { recursive: true });

  // And tasks dir
  const tasksDir = join(cwd, `.wise/state/team/${teamName}/tasks`);
...
export async function writeWorkerOverlay(
  ...
  const overlayPath = join(cwd, `.wise/state/team/${teamName}/workers/${workerName}/AGENTS.md`);
