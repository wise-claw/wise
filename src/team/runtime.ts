...
function stateRoot(cwd: string, teamName: string): string {
  ...
  return join(cwd, `.wise/state/team/${teamName}`);
}
...
function buildInitialTaskInstruction(
  ...
  const donePath = `.wise/state/team/${teamName}/workers/${workerName}/done.json`;
  return [
...
export async function assignTask(
  ...
  const msg = `\n\n---\n## New Task Assignment\nTask ID: ${taskId}\nClaim and execute task from: .wise/state/team/${teamName}/tasks/${taskId}.json\n`;
...
export async function resumeTeam(teamName: string, cwd: string): Promise<TeamRuntime | null> {
  ...
  const sName = configData.tmuxSession || `wise-team-${teamName}`;
