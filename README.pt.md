...
*Não aprenda Claude Code. Só use WISE.*

[Começar Rápido](#início-rápido) • [Documentação](https://yeachan-heo.github.io/wise-website) • [Referência CLI](https://yeachan-heo.github.io/wise-website/docs/#cli-reference) • [Workflows](https://yeachan-heo.github.io/wise-website/docs/#workflows) • [Guia de Migração](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
Se você executar WISE via `wise --plugin-dir <path>` ou `claude --plugin-dir <path>`, adicione `--plugin-dir-mode` a `wise setup` (ou exporte `WISE_PLUGIN_ROOT` antes) para evitar duplicar habilidades/agentes que o plugin já fornece em tempo de execução. Consulte a [seção Plugin directory flags em REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags) para uma matriz de decisão completa e todos os sinalizadores disponíveis.
...
A partir da **v4.1.7**, o **Team** é a superfície canônica de orquestração no WISE. Entrypoints legados como **swarm** e **ultrapilot** continuam com suporte, mas agora **roteiam para Team por baixo dos panos**.
...
> Se os times estiverem desativados, o WISE vai avisar você e fazer fallback para execução sem Team quando possível.

### Trabalhadores CLI tmux — Codex & Gemini (v4.4.0+)

**v4.4.0 remove os servidores MCP de Codex/Gemini** (provedores `x`, `g`). Use `/wise-teams` para lançar processos CLI reais em painéis divididos do tmux:

```bash
/wise-teams 2:codex   "review auth module for security issues"
/wise-teams 2:gemini  "redesign UI components for accessibility"
/wise-teams 1:claude  "implement the payment flow"
```
...
|-------|---------|----------|
| `/wise-teams N:codex` | N painéis Codex CLI | Revisão de código, análise de segurança, arquitetura |
| `/wise-teams N:gemini` | N painéis Gemini CLI | Design UI/UX, docs, tarefas de grande contexto |
| `/wise-teams N:claude` | N painéis Claude CLI | Tarefas gerais via Claude CLI no tmux |
...
> **Observação: Nome do pacote** — O projeto usa a marca **wise** (repo, plugin, comandos), mas o pacote npm é publicado como [`wise-claw`](https://www.npmjs.com/package/wise-claw). Se você instalar as ferramentas de CLI via npm/bun, use `npm install -g wise-claw`.
...
> **Observação:** Se a atualização automática do marketplace não estiver habilitada, você precisa executar manualmente `/plugin marketplace update wise` para sincronizar a versão mais recente antes de executar o setup.
...
Múltiplas estratégias para diferentes casos de uso — da orquestração com Team até refatoração com eficiência de tokens. [Saiba mais →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)
...
| **wise-teams** | Trabalhadores CLI tmux — processos reais `claude`/`codex`/`gemini` em painéis divididos | Tarefas Codex/Gemini CLI; criados sob demanda, encerrados ao terminar |
...
Quer contribuir para o WISE? Veja [CONTRIBUTING.md](./CONTRIBUTING.md) para o guia completo do desenvolvedor, incluindo como fazer fork, configurar um checkout local, vinculá-lo como seu plugin ativo, executar testes e enviar PRs.

<!-- TODO(i18n): verify translation -->

### Skills Personalizadas

Aprenda uma vez, reutilize para sempre. O WISE extrai conhecimento valioso de depuração em arquivos de skills portáteis que são auto-injetados quando relevantes.

| | Escopo de Projeto | Escopo de Usuário |
|---|---|---|
| **Caminho** | `.wise/skills/` | `~/.wise/skills/` |
...
| `wise-teams` | Trabalhadores CLI tmux (codex/gemini/claude) | `/wise-teams 2:codex "security review"` |
...
**Configuração manual:** crie `~/.claude/wise_config.openclaw.json`:
...
| Variável | Descrição |
|----------|-----------|
| `WISE_OPENCLAW=1` | Habilitar OpenClaw |
| `WISE_OPENCLAW_DEBUG=1` | Habilitar logs de depuração |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | Caminho alternativo do arquivo de configuração |
...
- **[Referência CLI](https://yeachan-heo.github.io/wise-website/docs/#cli-reference)** - Todos os comandos, flags e ferramentas do `wise`
- **[Guia de Notificações](https://yeachan-heo.github.io/wise-website/docs/#notifications)** - Configuração de Discord, Telegram, Slack e webhooks
- **[Workflows Recomendados](https://yeachan-heo.github.io/wise-website/docs/#workflows)** - Cadeias de skills testadas em batalha para tarefas comuns
- **[Notas de Lançamento](https://yeachan-heo.github.io/wise-website/docs/#release-notes)** - Novidades em cada versão
- **[Website](https://yeachan-heo.github.io/wise-website)** - Guias interativos e exemplos
...
O WISE pode opcionalmente orquestrar provedores externos de IA para validação cruzada e consistência de design. Eles **não são obrigatórios** — o WISE funciona completamente sem eles.
...
Se o Wise ajuda no seu fluxo de trabalho, considere patrocinar:
