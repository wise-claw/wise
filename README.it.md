...
_Non imparare Claude Code. Usa semplicemente WISE._

[Inizia](#avvio-rapido) • [Documentazione](https://yeachan-heo.github.io/wise-website) • [Guida alla migrazione](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
A partire dalla **v4.1.7**, **Team** è la superficie di orchestrazione canonica in WISE. I punti di ingresso legacy come **swarm** e **ultrapilot** sono ancora supportati, ma ora **vengono instradati a Team dietro le quinte**.
...
> Se i team sono disabilitati, WISE ti avviserà e passerà all'esecuzione senza Team quando possibile.

> **Nota: Nome del pacchetto** — Il progetto utilizza il brand **wise** (repo, plugin, comandi), ma il pacchetto npm è pubblicato come [`wise-claw`](https://www.npmjs.com/package/wise-claw). Se installi gli strumenti CLI tramite npm/bun, usa `npm install -g wise-claw`.
...
Strategie multiple per diversi casi d'uso — dall'orchestrazione basata su Team al refactoring efficiente in termini di token. [Scopri di più →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)
...
Impara una volta, riutilizza per sempre. WISE estrae le conoscenze di debug duramente acquisite in file di competenze portabili che si iniettano automaticamente quando pertinenti.

| | Ambito Progetto | Ambito Utente |
|---|---|---|
| **Percorso** | `.wise/skills/` | `~/.wise/skills/` |
...
| ------------- | ----------------------------------------- | --------------------------------------------------------------- |
| `team`        | Orchestrazione Team canonica              | `/wise:team 3:executor "fix all TypeScript errors"` |
...
```bash
wise wait          # Controlla lo stato, ottieni indicazioni
wise wait --start  # Abilita il daemon di ripristino automatico
wise wait --stop   # Disabilita il daemon
```
...
**Configurazione manuale:** crea `~/.claude/wise_config.openclaw.json`:
...
|-----------|-------------|
| `WISE_OPENCLAW=1` | Abilita OpenClaw |
| `WISE_OPENCLAW_DEBUG=1` | Abilita il logging di debug |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | Percorso alternativo del file di configurazione |
...
- **[Sito web](https://yeachan-heo.github.io/wise-website)** — Guide interattive ed esempi
...
WISE può opzionalmente orchestrare provider AI esterni per la validazione incrociata e la coerenza del design. Non sono **richiesti** — WISE funziona completamente senza di essi.
...
Se Wise migliora il tuo workflow, considera di diventare sponsor:
