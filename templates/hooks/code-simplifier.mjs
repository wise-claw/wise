...
/**
 * WISE Code Simplifier Stop Hook (Node.js)
 *
 ...
 *
 * Opt-in via ~/.wise/config.json: { "codeSimplifier": { "enabled": true } }
...
function readWiseConfig() {
  return readJsonFile(join(homedir(), '.wise', 'config.json'));
}
...
function buildMessage(files) {
  ...
  return (
    ...
    `${fileList}\n\n` +
    `Use: Task(subagent_type="wise:code-simplifier", ` +
...
async function main() {
  try {
    ...
    const stateDir = join(await resolveWiseStateRoot(cwd), 'state');
