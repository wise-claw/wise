...
>
> 相关文档：[架构](../架构.md) · [Hooks系统.md](../Hooks系统.md) · [参考.md](../参考.md) · [本地插件安装.md](../本地插件安装.md)
...
- 不直接修改 Claude Code 内部状态（例如 `/goal` 适配器仅渲染 handoff 文本，见 [Claude-Code目标适配器.md](./Claude-Code目标适配器.md)）
...
同一时刻只能有一个**主循环权威**（ralph、team、autopilot、ultraqa、Claude `/goal` 等）。冲突时采用确定性策略：`refuse` | `adopt_existing` | `artifact_only`（见 [Claude-Code目标适配器.md](./Claude-Code目标适配器.md)）。
...
|------|------|
| [架构](../架构.md) | 面向用户的架构概览 |
| [Hooks系统.md](../Hooks系统.md) | Hook 事件与禁用方式 |
| [参考.md](../参考.md) | 完整 API、agents、skills 列表 |
| [功能参考.md](../功能参考.md) | 内部特性开发者参考 |
| [层级智能体v2架构.md](./层级智能体v2架构.md) | 分层 agent 与模型路由设计 |
| [Claude-Code目标适配器.md](./Claude-Code目标适配器.md) | `/goal` 适配器契约 |
| [元数据同步系统.md](../元数据同步系统.md) | 状态同步 |
| [委派强制器.md](../委派强制器.md) | 委派强制协议 |
| [本地插件安装.md](../本地插件安装.md) | 本地插件开发 |
| [团队Worktree模式.md](../团队Worktree模式.md) | Team worktree 模式 |
