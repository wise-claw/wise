...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Run complete benchmark comparison between vanilla and WISE modes."
            echo ""
            echo "Options:"
            ...
            echo "  --skip-wise        Skip WISE benchmark run"
...
# Step 2: Run WISE benchmark
if [ "$SKIP_WISE" = false ]; then
    log_step "Step 2/4: Running WISE-enhanced benchmark..."
    echo ""
    "$SCRIPT_DIR/run_wise.sh" $ARGS
    if [ $? -ne 0 ]; then
        log_error "WISE benchmark failed. Aborting."
        exit 1
    fi
    echo ""
else
    log_warn "Skipping WISE benchmark (--skip-wise)"
    echo ""
fi
...
if [ "$SKIP_EVAL" = false ]; then
    ...
    log_step "Step 4/4: Evaluating WISE predictions..."
    echo ""
    if [ -f "$SCRIPT_DIR/evaluate.py" ]; then
        python3 "$SCRIPT_DIR/evaluate.py" \
            --predictions "$SCRIPT_DIR/predictions/wise" \
            --output "$SCRIPT_DIR/results/wise_results.json"
        if [ $? -ne 0 ]; then
            log_warn "WISE evaluation had issues (continuing...)"
        fi
    else
        log_warn "evaluate.py not found, skipping evaluation"
    fi
...
if [ -f "$SCRIPT_DIR/compare_results.py" ]; then
    python3 "$SCRIPT_DIR/compare_results.py" \
        ...
        --wise "$SCRIPT_DIR/predictions/wise/predictions.jsonl" \
...
### WISE-Enhanced
Location: \`predictions/wise/\`
Results: \`results/wise_results.json\`
...
log_info "  WISE predictions:     $SCRIPT_DIR/predictions/wise/"
