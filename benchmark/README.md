...
Automated benchmark comparison between vanilla Claude Code and WISE-enhanced Claude Code.
...
- Tests both vanilla and WISE modes
...
- Standard Claude Code without WISE
...
### run_wise.sh
Run WISE-enhanced benchmark:
- Claude Code with wise orchestration
- Saves predictions to `predictions/wise/`
- Logs to `logs/wise_*.log`
...
**Examples:**
```bash
./run_wise.sh                    # Full benchmark
./run_wise.sh --limit 100        # First 100 instances
```
...
- `--skip-wise` - Skip WISE benchmark run
...
./run_full_comparison.sh --skip-vanilla     # Only run WISE (reuse vanilla results)
```
...
├── run_wise.sh                 # Run WISE benchmark
...
│   └── wise/                  # WISE predictions
├── logs/
│   ├── vanilla_*.log         # Vanilla run logs
│   └── wise_*.log            # WISE run logs
├── results/
│   ├── vanilla_results.json  # Vanilla evaluation
│   ├── wise_results.json      # WISE evaluation
...
4. **Review Results**:
   ...
   - Inspect predictions in `predictions/vanilla/` and `predictions/wise/`
   - Review logs in `logs/`
...
./run_wise.sh --model claude-opus-4-6-20260205
```
...
2. **Parallel Runs**: Don't run vanilla and WISE in parallel (share API rate limits)
