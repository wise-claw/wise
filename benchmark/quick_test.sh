...
# Step 2: Run quick WISE test
log_step "Step 2/2: Quick WISE test ($TEST_LIMIT instances)..."
echo ""
"$SCRIPT_DIR/run_wise.sh" --limit $TEST_LIMIT --model "$MODEL" --timeout $TIMEOUT
WISE_STATUS=$?
echo ""
...
if [ $VANILLA_STATUS -eq 0 ] && [ $WISE_STATUS -eq 0 ]; then
    ...
    [ $WISE_STATUS -ne 0 ] && log_error "  WISE test: FAILED (exit code $WISE_STATUS)"
    echo ""
