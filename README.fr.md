...
_N'apprenez pas Claude Code. Utilisez simplement WISE._

[Démarrer](#démarrage-rapide) • [Documentation](https://yeachan-heo.github.io/wise-website) • [Guide de migration](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
Si vous exécutez WISE via `wise --plugin-dir <path>` ou `claude --plugin-dir <path>`, ajoutez `--plugin-dir-mode` à `wise setup` (ou exportez `WISE_PLUGIN_ROOT` auparavant) pour éviter de dupliquer les compétences/agents que le plugin fournit déjà au moment de l'exécution. Consultez la [section Plugin directory flags dans REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags) pour une matrice de décision complète et tous les indicateurs disponibles.
...
À partir de la **v4.1.7**, **Team** est la surface d'orchestration canonique dans WISE. Les anciens points d'entrée comme **swarm** et **ultrapilot** sont toujours supportés, mais **redirigent désormais vers Team en coulisses**.
...
> Si les teams sont désactivés, WISE vous avertira et basculera vers une exécution sans Team lorsque possible.

> **Note : Nom du package** — Le projet utilise la marque **wise** (repo, plugin, commandes), mais le package npm est publié sous le nom [`wise-claw`](https://www.npmjs.com/package/wise-claw). Si vous installez les outils CLI via npm/bun, utilisez `npm install -g wise-claw`.
...
Plusieurs stratégies pour différents cas d'utilisation — de l'orchestration Team au refactoring économe en tokens. [En savoir plus →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)
...
Vous souhaitez contribuer à WISE ? Voir [CONTRIBUTING.md](./CONTRIBUTING.md) pour le guide complet du développeur, incluant comment faire un fork, configurer un checkout local, le lier en tant que plugin actif, exécuter des tests et soumettre des PR.

<!-- TODO(i18n): verify translation -->

### Compétences Personnalisées

Apprenez une fois, réutilisez à jamais. WISE extrait les connaissances durement acquises lors du débogage en fichiers de compétences portables qui s'injectent automatiquement quand pertinent.

| | Portée Projet | Portée Utilisateur |
|---|---|---|
| **Chemin** | `.wise/skills/` | `~/.wise/skills/` |
...
| ------------ | ----------------------------------- | --------------------------------------------------------------- |
| `team`       | Orchestration Team canonique        | `/wise:team 3:executor "fix all TypeScript errors"` |
...
```bash
wise wait          # Vérifier le statut, obtenir des conseils
wise wait --start  # Activer le daemon de reprise automatique
wise wait --stop   # Désactiver le daemon
```
...
**Configuration manuelle :** créez `~/.claude/wise_config.openclaw.json` :
...
|----------|-------------|
| `WISE_OPENCLAW=1` | Activer OpenClaw |
| `WISE_OPENCLAW_DEBUG=1` | Activer la journalisation de débogage |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | Chemin alternatif du fichier de configuration |
...
- **[Site web](https://yeachan-heo.github.io/wise-website)** — Guides interactifs et exemples
...
WISE peut optionnellement orchestrer des fournisseurs d'IA externes pour la validation croisée et la cohérence du design. Ils ne sont **pas requis** — WISE fonctionne pleinement sans eux.
...
Si Wise améliore votre workflow, envisagez de devenir sponsor :
