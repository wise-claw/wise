...
_Lernen Sie nicht Claude Code. Nutzen Sie einfach WISE._

[Loslegen](#schnellstart) • [Dokumentation](https://yeachan-heo.github.io/wise-website) • [Migrationsleitfaden](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
Wenn Sie WISE über `wise --plugin-dir <path>` oder `claude --plugin-dir <path>` ausführen, fügen Sie `--plugin-dir-mode` zu `wise setup` hinzu (oder exportieren Sie `WISE_PLUGIN_ROOT` vorher), um zu vermeiden, dass Fähigkeiten/Agenten dupliziert werden, die das Plugin bereits zur Laufzeit bereitstellt. Siehe [Plugin directory flags Abschnitt in REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags) für eine vollständige Entscheidungsmatrix und alle verfügbaren Flags.
...
Ab **v4.1.7** ist **Team** die kanonische Orchestrierungsoberfläche in WISE. Legacy-Einstiegspunkte wie **swarm** und **ultrapilot** werden weiterhin unterstützt, **leiten aber im Hintergrund an Team weiter**.
...
> Wenn Teams deaktiviert sind, warnt WISE Sie und fällt auf Ausführung ohne Team zurück, wenn möglich.

> **Hinweis: Paketbenennung** — Das Projekt nutzt die Marke **wise** (Repo, Plugin, Befehle), aber das npm-Paket wird als [`wise-claw`](https://www.npmjs.com/package/wise-claw) veröffentlicht. Wenn Sie die CLI-Tools über npm/bun installieren, verwenden Sie `npm install -g wise-claw`.
...
Mehrere Strategien für verschiedene Anwendungsfälle — von Team-gestützter Orchestrierung bis token-effizientem Refactoring. [Mehr erfahren →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)
...
- **HUD statusline** — Echtzeit-Orchestrierungsmetriken in Ihrer Statusleiste
  - Wenn Sie Claude Code direkt mit `claude --plugin-dir <path>` starten (Umgehen des `wise` Shim), exportieren Sie `WISE_PLUGIN_ROOT=<path>` in Ihrer Shell, damit die HUD-Sammlung zu demselben Checkout wie der Plugin-Loader aufgelöst wird. Siehe [Plugin directory flags Abschnitt in REFERENCE.md](./docs/REFERENCE.md#plugin-directory-flags) für Details.
...
Möchten Sie zu WISE beitragen? Siehe [CONTRIBUTING.md](./CONTRIBUTING.md) für den vollständigen Entwicklerleitfaden, einschließlich Forking, Einrichten eines lokalen Checkouts, Verlinken als aktives Plugin, Ausführen von Tests und Einreichen von PRs.

<!-- TODO(i18n): verify translation -->

### Benutzerdefinierte Skills

Einmal lernen, für immer wiederverwenden. WISE extrahiert hart erarbeitetes Debugging-Wissen in portable Skill-Dateien, die bei Bedarf automatisch injiziert werden.

| | Projektbereich | Benutzerbereich |
|---|---|---|
| **Pfad** | `.wise/skills/` | `~/.wise/skills/` |
...
| ------------- | ------------------------------------------------ | --------------------------------------------------------------- |
| `team`        | Kanonische Team-Orchestrierung                   | `/wise:team 3:executor "fix all TypeScript errors"` |
...
```bash
wise wait          # Status prüfen, Anleitung erhalten
wise wait --start  # Auto-Resume-Daemon aktivieren
wise wait --stop   # Daemon deaktivieren
```
...
**Manuelle Einrichtung:** Erstellen Sie `~/.claude/wise_config.openclaw.json`:
...
|----------|-------------|
| `WISE_OPENCLAW=1` | OpenClaw aktivieren |
| `WISE_OPENCLAW_DEBUG=1` | Debug-Protokollierung aktivieren |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | Konfigurationsdatei-Pfad überschreiben |
...
- **[Website](https://yeachan-heo.github.io/wise-website)** — Interaktive Anleitungen und Beispiele
...
WISE kann optional externe AI-Anbieter für Kreuzvalidierung und Design-Konsistenz orchestrieren. Diese sind **nicht erforderlich** — WISE funktioniert vollständig ohne sie.
...
Wenn Wise Ihren Workflow verbessert, erwägen Sie ein Sponsoring:
