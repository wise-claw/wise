...
[![npm version](https://img.shields.io/npm/v/wise?color=cb3837)](https://www.npmjs.com/package/wise)
[![npm downloads](https://img.shields.io/npm/dm/wise?color=blue)](https://www.npmjs.com/package/wise)
...
*无需学习 Claude Code，直接使用 WISE。*

[快速开始](#快速开始) • [文档](https://yeachan-heo.github.io/wise-website) • [CLI 参考](https://yeachan-heo.github.io/wise-website/docs/#cli-reference) • [工作流](https://yeachan-heo.github.io/wise-website/docs/#workflows) • [迁移指南](docs/MIGRATION.md) • [Discord](https://discord.gg/sj4exxQ9v)
...
从 **v4.1.7** 开始，**Team** 是 WISE 的标准编排方式。**swarm** 和 **ultrapilot** 等旧版入口仍受支持，但现在**在底层路由到 Team**。
...
> 如果团队被禁用，WISE 会发出警告并在可能的情况下回退到非 Team 执行模式。

### tmux CLI 工作者 — Codex & Gemini (v4.4.0+)

**v4.4.0 移除了 Codex/Gemini MCP 服务器**（`x`、`g` 提供商）。请改用 `/wise-teams` 在 tmux 分屏中启动真实的 CLI 进程：
...
| 技能 | 工作者 | 最适合 |
|-------|---------|----------|
| `/wise-teams N:codex` | N 个 Codex CLI 窗格 | 代码审查、安全分析、架构 |
| `/wise-teams N:gemini` | N 个 Gemini CLI 窗格 | UI/UX 设计、文档、大上下文任务 |
| `/wise-teams N:claude` | N 个 Claude CLI 窗格 | 通过 tmux 中的 Claude CLI 处理通用任务 |
| `/ccg` | 1 个 Codex + 1 个 Gemini | 并行三模型编排 |

工作者按需生成，任务完成后自动退出 — 无空闲资源浪费。需要安装 `codex` / `gemini` CLI 并有活跃的 tmux 会话。

> **注意：包命名** — 项目品牌名为 **wise**（仓库、插件、命令），但 npm 包以 [`wise`](https://www.npmjs.com/package/wise) 发布。通过 npm/bun 安装 CLI 工具时，请使用 `npm install -g wise`。
...
> **注意：** 如果 marketplace 自动更新未启用，您需要在运行设置之前手动执行 `/plugin marketplace update wise` 来同步最新版本。
...
### 执行模式
针对不同场景的多种策略 - 从全自动构建到 token 高效重构。[了解更多 →](https://yeachan-heo.github.io/wise-website/docs/#execution-modes)

| 模式 | 特点 | 适用场景 |
|------|---------|---------|
| **Team（推荐）** | 阶段化流水线 | 在共享任务列表上协作的 Claude 智能体 |
| **wise-teams** | tmux CLI 工作者 | Codex/Gemini CLI 任务；按需生成，完成后退出 |
...
- **HUD 状态栏** - 状态栏实时显示编排指标
  - 如果你直接使用 `claude --plugin-dir <path>` 启动 Claude Code（绕过 `wise` shim），请在 shell 中导出 `WISE_PLUGIN_ROOT=<path>`，以便 HUD bundle 解析到与插件加载器相同的 checkout。详情见 [REFERENCE.md 中的 Plugin directory flags 部分](./docs/REFERENCE.md#plugin-directory-flags)。
...
想为 WISE 做贡献？请参阅 [CONTRIBUTING.md](./CONTRIBUTING.md) 了解完整的开发者指南，包括如何 fork、设置本地 checkout、将其链接为活跃插件、运行测试和提交 PR。

<!-- TODO(i18n): verify translation -->

### 自定义技能

一次学习，永久复用。WISE 将调试过程中获得的实战知识提取为可移植的技能文件，并在相关场景中自动注入。

| | 项目作用域 | 用户作用域 |
|---|---|---|
| **路径** | `.wise/skills/` | `~/.wise/skills/` |
...
| `wise-teams` | tmux CLI 工作者 (codex/gemini/claude) | `/wise-teams 2:codex "security review"` |
...
```bash
wise wait          # 检查状态，获取指导
wise wait --start  # 启用自动恢复守护进程
wise wait --stop   # 禁用守护进程
```
...
**手动设置：** 创建 `~/.claude/wise_config.openclaw.json`：
...
| 变量 | 说明 |
|------|------|
| `WISE_OPENCLAW=1` | 启用 OpenClaw |
| `WISE_OPENCLAW_DEBUG=1` | 启用调试日志 |
| `WISE_OPENCLAW_CONFIG=/path/to/config.json` | 覆盖配置文件路径 |
...
- **[CLI 参考](https://yeachan-heo.github.io/wise-website/docs/#cli-reference)** - 所有 `wise` 命令、标志和工具
- **[通知指南](https://yeachan-heo.github.io/wise-website/docs/#notifications)** - Discord、Telegram、Slack 和 webhook 设置
- **[推荐工作流](https://yeachan-heo.github.io/wise-website/docs/#workflows)** - 常见任务的经过实战检验的技能链
- **[发布说明](https://yeachan-heo.github.io/wise-website/docs/#release-notes)** - 每个版本的新内容
- **[网站](https://yeachan-heo.github.io/wise-website)** - 交互式指南和示例
...
WISE 可以选择性地调用外部 AI 提供商进行交叉验证和设计一致性检查。**非必需** — 没有它们 WISE 也能完整运行。
